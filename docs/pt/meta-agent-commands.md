...
Guia de referência completo para todos os comandos do meta-agente SINAPSE.
...
```bash
SINAPSE_AI_PROVIDER=openai          # Provedor de IA
SINAPSE_AI_MODEL=gpt-4             # Modelo de IA
SINAPSE_LOG_LEVEL=debug            # Nível de log
SINAPSE_TELEMETRY=disabled         # Configuração de telemetria
SINAPSE_TIMEOUT=30000             # Timeout de comando (ms)
SINAPSE_MEMORY_CACHE=true         # Cache de memória
```
