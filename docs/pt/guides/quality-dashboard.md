...
```
.sinapse/data/quality-metrics.json
```
...
```bash
# Sincronizar de .sinapse/data para o dashboard
npm run sync-metrics
...
Isso copia `.sinapse/data/quality-metrics.json` para `tools/quality-dashboard/public/.sinapse/data/`.
...
ls -la .sinapse/data/quality-metrics.json

# Se estiver faltando, popule com dados de exemplo
npx sinapse metrics seed
```
