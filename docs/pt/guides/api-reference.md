...
Referência completa de API para o SINAPSE - o Sistema Orquestrado por IA para Desenvolvimento Full Stack.
...
O SINAPSE fornece uma API unificada para interagir com agentes de IA especializados através de dois mecanismos principais:
...
1. O arquivo de definição do agente é carregado de `.sinapse-ai/development/agents/{id}.md`
...
| Comando                | Argumentos      | Descrição                                           |
| ---------------------- | --------------- | --------------------------------------------------- |
| `*create`              | `{type} {name}` | Criar componente SINAPSE (agent/task/workflow)         |
...
| `*kb`   | -                          | Alternar modo KB (conhecimento do SINAPSE Method)   |
...
| -------------------- | ------------------------------- | --------------------------------------------- |
| `AGENT_NOT_FOUND`    | Definição de agente ausente     | Verificar `.sinapse-ai/development/agents/`    |
...
- [Guia do Usuário](./user-guide.md) - Começando com SINAPSE
...
- [Guia de Squads](./squads-guide.md) - Estendendo SINAPSE com squads
