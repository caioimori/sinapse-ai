...
> Controle o nível de autonomia que os agentes SINAPSE têm sobre seu sistema.

---

## Visão Geral

Os Modos de Permissão permitem controlar o nível de autonomia dos agentes SINAPSE. Seja explorando um novo codebase ou executando builds totalmente autônomos, há um modo para seu workflow.
...
O modo é persistido em `.sinapse/config.yaml`:
...
Verifique se `.sinapse/config.yaml` existe e é gravável:
