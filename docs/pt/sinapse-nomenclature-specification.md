...
Este documento estabelece nomenclatura clara para o SINAPSE diferenciando entre:
...
**Características:**
- **Escopo:** Interno a um único arquivo de task (`.sinapse-ai/tasks/*.md`)
...
- **Localização:** Definido em `.sinapse-ai/workflows/*.yaml` ou seções de workflow de stories
...
**Conceito:** Inspirado pelo protocolo Organizer-Worker do AsyncThink, workflows do SINAPSE podem usar um **Organizer Agent** que coordena **Worker Agents** executando tasks em paralelo.

**Aplicação ao SINAPSE:**
...
- Exemplo: `.sinapse-ai/tasks/security-scan.md`

**Arquivos de Workflow:**
- Localização: `.sinapse-ai/workflows/{workflow-name}.yaml`
- Contém: Definição de orquestração multi-task
- Exemplo: `.sinapse-ai/workflows/story-development-workflow.yaml`
...
3. **Gerenciamento de Workflow SINAPSE:** Padrões de workflow existentes no SINAPSE
