...
Este documento descreve as melhores práticas para gerenciar chaves de API usadas por servidores MCP (Model Context Protocol) no SINAPSE. O gerenciamento adequado de chaves de API é crítico para segurança e integridade operacional.

---

## Arquitetura MCP no SINAPSE

SINAPSE utiliza Docker MCP Toolkit como a infraestrutura principal de MCP:
