...
Este documento especifica as capacidades de auto-recuperação do Sistema de Verificação de Saúde do SINAPSE (HCS). A auto-recuperação permite que o sistema corrija automaticamente certos problemas enquanto mantém a segurança e o controle do usuário.
...
| ---------------------- | -------------------------------------------------------- | ------------- |
| `recreate_config`      | Recriar `.sinapse/config.yaml` ausente a partir do template | Sim           |
| `clear_cache`          | Limpar arquivos de cache obsoletos em `.sinapse/cache/`     | Sim           |
...
- id: PC-001
  description: '.sinapse/config.yaml ausente'
  severity: CRITICAL
  tier: 1
...
✅ 3 problemas auto-corrigidos:
   • Recriado .sinapse/config.yaml (backup: .sinapse/backups/config.yaml.1735564800)
   • Reiniciado servidor MCP context7
   • Adicionado .sinapse/cache/ ao .gitignore
```
...
- id: PC-002
  ...
  guide:
    ...
    steps:
      ...
      - 'Verificar com: npx sinapse task validate deploy'
...
Passos para resolver:
  ...
  4. Verificar com: npx sinapse task validate deploy
...
neverAutoFix:
  files:
    ...
    - '**/*.{json,yaml,yml}' # Config do usuário (fora de .sinapse/)
...
safeToAutoFix:
  paths:
    - '.sinapse/**' # Arquivos de workspace do SINAPSE
    - '.sinapse-ai/**/*.yaml' # YAML do framework (cuidado)
