...
**Propósito:** Verificar se os arquivos do framework SINAPSE estão corretamente configurados e consistentes.

### Verificações

| ID     | Nome                           | Severidade | Nível | Modo  | Timeout | Descrição                                            |
| ------ | ------------------------------ | ---------- | ----- | ----- | ------- | ---------------------------------------------------- |
| PC-001 | Config existe                  | CRITICAL   | 1     | quick | 100ms   | `.sinapse/config.yaml` existe e é YAML válido           |
...
| PC-006 | Sem arquivos órfãos            | LOW        | 3     | full  | 5s      | Todos os arquivos em `.sinapse-ai/` são referenciados |
...
class ConfigExistsCheck extends BaseCheck {
  ...
  async check(context) {
    ...
    if (!(await fs.pathExists(configPath))) {
      return {
        passed: false,
        message: '.sinapse/config.yaml não encontrado',
...
const requiredSections = [
  'Project Overview', // ou 'SINAPSE-FULLSTACK Development Rules'
...
Usuários podem definir verificações específicas do projeto em `.sinapse/custom-checks.yaml`:
