...
SINAPSE é construído em JavaScript/TypeScript moderno com runtime Node.js, otimizado para desenvolvimento de CLI cross-platform com UX interativa e capacidades de orquestração de agentes.
...
SINAPSE integra com servidores Model Context Protocol (MCP):
...
```bash
# Configuração SINAPSE
SINAPSE_DEBUG=true                    # Habilitar logging de debug
SINAPSE_CONFIG_PATH=/custom/path      # Localização customizada de config
SINAPSE_YOLO_MODE=true               # Forçar modo yolo
...
| 1.1    | 2025-12-14 | Atualizado aviso de migração para caioimori/sinapse-ai, semantic-release para v25.0.2 [Story 6.10] | Dex (dev)        |

---

_Este é um padrão oficial do framework SINAPSE. Todas as escolhas de tecnologia devem estar alinhadas com este stack._
