...
1. **Git Pre-Push Hook** (Aplicação Primária)
   ...
   - Verificações: Variável de ambiente `$SINAPSE_ACTIVE_AGENT`
...
- [ ] **Criar Git Pre-Push Hook**
  - Localização: `.git/hooks/pre-push`
  - Conteúdo: Verificar `$SINAPSE_ACTIVE_AGENT`, bloquear se != "github-devops"
...
- [ ] **Atualizar Scripts de Ativação de Agentes**
  - Adicionar configuração de variável de ambiente: `SINAPSE_ACTIVE_AGENT={agent_id}`
  - Configurar `SINAPSE_GIT_PUSH_ALLOWED` apropriadamente
