# 🏛️ SINAPSE Dashboard - Arquitetura Completa
...
O SINAPSE Dashboard é uma aplicação web Next.js que fornece uma interface visual para monitorar e gerenciar o sistema SINAPSE. Ele se comunica com o CLI/SINAPSE através de arquivos de status no filesystem e Server-Sent Events (SSE).

### Princípios Arquiteturais

1. **CLI-First**: Dashboard é complementar ao CLI, não substituto
2. **File-Based Communication**: Status via `.sinapse/dashboard/status.json`
...
│   │   │   ├── status/route.ts       # SINAPSE status polling
...
│   │   ├── use-sinapse-status.ts        # Status with SWR
...
| ---------------- | ------------------------- | -------------------------------- |
| `story-store`    | `sinapse-stories`            | `storyOrder` (ordem das colunas) |
| `ui-store`       | `sinapse-ui`                 | `sidebarCollapsed`, `activeView` |
| `projects-store` | `sinapse-projects`           | `projects`, `activeProjectId`    |
| `settings-store` | `sinapse-dashboard-settings` | Todo o objeto `settings`         |
...
```typescript
// Retorna status atual do SINAPSE
// Lê de: .sinapse/dashboard/status.json
...
└──────┬──────┘                              └────────┬─────────┘
       ...
       │  1. Agent ativado                            │
       │  ─────────────────────────────────────────▶  │
       │     Escreve .sinapse/dashboard/status.json      │
       │                                              │
       │                                              │ 2. Dashboard detecta
       │                                              │    (SSE poll 2s)
       │                                              │
