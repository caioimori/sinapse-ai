...
Todo o estado do ADE é persistido em `.sinapse/` para:
...
Arquivo de Estado: .sinapse/project-status.yaml
```
...
| ------------------- | ---------------------------------- | --------------------------------- |
| Padrões de Código   | Padrões reutilizáveis da base     | .sinapse/patterns/code-patterns.json |
| Gotchas             | Problemas conhecidos e soluções   | .sinapse/patterns/gotchas.json       |
| Insights de Sessão  | Descobertas durante sessões       | .sinapse/sessions/                   |
| Mapa de Codebase    | Análise de estrutura do projeto   | .sinapse/codebase-map.json           |
...
ADE integra-se com agentes SINAPSE através de:
...
Todos os componentes do ADE atualizam `.sinapse/status.json`:
...
Localizada em `.sinapse-ai/core-config.yaml`:
