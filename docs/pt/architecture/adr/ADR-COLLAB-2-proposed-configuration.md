...
Seguindo a [Auditoria do Estado Atual](./ADR-COLLAB-1-current-state-audit.md), este documento propõe alterações de configuração específicas para habilitar contribuições externas seguras ao SINAPSE.
...
# Configuração CodeRabbit do SINAPSE
# Story: COLLAB-1
...
reviews:
  ...
  path_instructions:
    ...
    '.sinapse-ai/development/agents/**':
      - 'Verificar se agente segue estrutura YAML de agente SINAPSE (persona_profile, commands, dependencies)'
    ...
    '.sinapse-ai/development/tasks/**':
      - 'Verificar se tarefa segue formato de tarefa SINAPSE com pontos de elicitação claros'
    ...
    '.sinapse-ai/product/templates/**':
      - 'Garantir que template segue convenções de template SINAPSE'
    ...
    '**/*.js':
      ...
      - 'Garantir que código segue padrões de codificação SINAPSE'
...
- [ ] Agente segue estrutura YAML de agente SINAPSE
...
- [ ] Tarefa segue formato de tarefa SINAPSE
