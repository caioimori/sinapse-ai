...
Bem-vindo ao SINAPSE! Este guia irá orientá-lo nos seus primeiros passos com o framework de desenvolvimento auto-modificável alimentado por IA.
...
Antes de instalar o SINAPSE, certifique-se de ter:
...
O SINAPSE oferece opções flexíveis de instalação:
...
```
my-first-project/
├── .sinapse/                    # Configuração do framework
...
O arquivo principal de configuração (`.sinapse/config.json`) contém:
...
O meta-agent é o núcleo do SINAPSE - uma IA que entende e pode modificar sua própria base de código.
...
- **[Tutoriais em Vídeo](https://sinapse-ai.dev/tutorials)**: Aprendizado visual
...
- **Twitter**: Siga [@sinapsefullstack](https://twitter.com/sinapsefullstack)
