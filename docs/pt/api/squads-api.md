...
API REST para sincronizar squads com SINAPSE e descobrir squads do marketplace.

## Visão Geral

A API de Squads permite:

- **Sync**: Enviar squads locais para a nuvem SINAPSE
...
Obtenha sua chave de API em: https://sinapse.ai/settings/api-keys

## Endpoints

### Sincronizar Squad

Enviar uma definição de squad para SINAPSE.
...
```json
{
  ...
  "data": {
    "valid": true,
    "errors": [],
    "warnings": ["Missing recommended field: description", "Missing sinapse.minVersion field"]
  }
}
```
...
O comando `*sync-squad-sinapse` usa esta API:
...
{
  "info": {
    "name": "SINAPSE Squads API",
    "description": "API REST para o Marketplace de Squads SINAPSE",
...
Salve o JSON acima como `sinapse-squads-api.postman_collection.json` e importe no Postman.
