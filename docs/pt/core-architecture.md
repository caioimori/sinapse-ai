...
Todo o ecossistema do Método SINAPSE é projetado em torno do diretório `sinapse-ai` instalado, que atua como o cérebro da operação. O diretório `tools` fornece os meios para processar e empacotar este cérebro para diferentes ambientes.
...
O diretório `sinapse-ai` contém todas as definições e recursos que dão aos agentes suas capacidades.
...
### 3.3. Workflows (`sinapse-ai/workflows/`)

- **Propósito**: Workflows são arquivos YAML (ex: `greenfield-fullstack.yaml`) que definem uma sequência prescrita de etapas e interações de agentes para um tipo específico de projeto. Eles atuam como um guia estratégico para o usuário e o agente `sinapse-orchestrator`.
...
- **Propósito**: Estas pastas abrigam os componentes modulares que são carregados dinamicamente pelos agentes com base em suas dependências.
  ...
  - **`data/`**: Contém a base de conhecimento central (`sinapse-kb.md`), preferências técnicas (`technical-preferences.md`) e outros arquivos de dados importantes.

#### 3.4.1. Sistema de Processamento de Templates

Um princípio arquitetural chave do SINAPSE é que templates são auto-contidos e interativos - eles incorporam tanto a saída desejada do documento quanto as instruções do LLM necessárias para trabalhar com os usuários. Isso significa que, em muitos casos, nenhuma task separada é necessária para criação de documentos, pois o próprio template contém toda a lógica de processamento.

O framework SINAPSE emprega um sistema sofisticado de processamento de templates orquestrado por três componentes-chave:

- **`template-format.md`** (`sinapse-ai/utils/`): Define a linguagem de marcação fundamental usada em todos os templates do SINAPSE. Esta especificação estabelece regras de sintaxe para substituição de variáveis (`{{placeholders}}`), diretivas de processamento exclusivas para IA (`[[LLM: instructions]]`) e blocos de lógica condicional. Templates seguem este formato para garantir processamento consistente em todo o sistema.

- **`create-doc.md`** (`sinapse-ai/tasks/`): Atua como o motor de orquestração que gerencia todo o workflow de geração de documentos. Esta task coordena a seleção de templates, gerencia modos de interação com o usuário (geração incremental vs. rápida), aplica regras de processamento de template-format e lida com validação. Serve como a interface principal entre usuários e o sistema de templates.

- **`advanced-elicitation.md`** (`sinapse-ai/tasks/`): Fornece uma camada de refinamento interativo que pode ser incorporada dentro de templates através de blocos `[[LLM: instructions]]`. Este componente oferece 10 ações estruturadas de brainstorming, capacidades de revisão seção por seção e workflows de melhoria iterativa para aprimorar a qualidade do conteúdo.
...
O SINAPSE inclui uma camada de personalização através do arquivo `technical-preferences.md` em `sinapse-ai/data/`. Este arquivo serve como um perfil técnico persistente que influencia o comportamento dos agentes em todos os projetos.
...
- **Para IDEs**: Usuários interagem com os agentes diretamente via seus arquivos markdown em `sinapse-ai/agents/`. A integração do IDE (para Cursor, Claude Code, etc.) sabe como chamar estes agentes.
...
## 5. Workflows do SINAPSE

### 5.1. O Workflow de Planejamento

Antes do desenvolvimento começar, o SINAPSE segue um workflow de planejamento estruturado que estabelece a fundação para execução bem-sucedida do projeto:
...
**Orquestração de Workflow**: O agente `sinapse-orchestrator` usa estas definições de workflow para guiar usuários através do processo completo, garantindo transições adequadas entre fases de planejamento (UI web) e desenvolvimento (IDE).

### 5.2. O Ciclo Principal de Desenvolvimento

Uma vez que as fases iniciais de planejamento e arquitetura estejam completas, o projeto move para um workflow de desenvolvimento cíclico, conforme detalhado no `sinapse-kb.md`. Isso garante um processo de implementação estável, sequencial e controlado por qualidade.
