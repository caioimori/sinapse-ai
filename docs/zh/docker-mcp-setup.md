...
使用 SINAPSE 设置基于 Docker 的 MCP（模型上下文协议）服务器的指南。
...
- `~/.sinapse/mcp/` - MCP 配置目录
- `~/.sinapse/mcp/global-config.json` - 主配置文件
- `~/.sinapse/mcp/servers/` - 单个服务器配置
- `~/.sinapse/credentials/` - 安全凭证存储
...
SINAPSE 使用 Docker MCP 工具包作为主要 MCP 基础设施。
...
_SINAPSE Docker MCP 设置指南 v4.2.11_
