...
- [概述](#概述)
- [模块化架构](#模块化架构)
- [框架核心 (.sinapse-ai/)](#框架核心-sinapse-ai)
- [模块详情](#模块详情)
- [文档 (docs/)](#文档-docs)
- [Squads 系统](#squads-系统)
...
SINAPSE 使用**模块化架构**，具有清晰的关注点分离：

1. **框架核心** (`.sinapse-ai/`) - 按领域组织的可移植框架组件
...
│
├── data/                              # 共享数据
│   ├── sinapse-kb.md                     # SINAPSE 知识库（@sinapse-master，延迟加载）
...
### 共享数据文件 (.sinapse-ai/data/)

| 文件 | 所有者 | 填充规则 | 更新触发器 | 使用者 |
|------|--------|----------|------------|--------|
| `sinapse-kb.md` | @sinapse-master | 框架重大变更时更新 | 手动编辑 | @sinapse-master (延迟) |
...
### 产品数据文件 (.sinapse-ai/product/data/)
...
│
├── stories/                           # 开发故事
│   ├── sinapse migration/                # SINAPSE 迁移故事
│   │   ├── story-6.1.2.1.md
│   │   ├── story-6.1.2.2.md
│   │   ├── story-6.1.2.3.md
│   │   ├── story-6.1.2.4.md
...
Squads 是为 SINAPSE 添加专门功能的模块化扩展。与已弃用的 Squads 不同，Squads 遵循标准化的模板结构。
...
### 仓库1: SinapseAI/sinapse-ai (MIT)

```
sinapse-ai/
├── .sinapse-ai/                        # 框架资产（模块化 v2.0）
│   ├── cli/                           # CLI 命令和实用工具
│   ├── core/                          # 框架基础
│   │   ├── config/                    # 配置系统
│   │   ├── quality-gates/             # 质量验证器
...
### 仓库2: SinapseAI/squads (MIT)

```
squads/
├── verified/                          # SINAPSE 认证的 squads
│   ├── github-devops/
│   ├── db-sage/
│   └── coderabbit-workflow/
│
...
### 仓库3: SinapseAI/mcp-ecosystem (Apache 2.0)
...
### 仓库4: SinapseAI/certified-partners (私有)
...
```yaml
# 我正在创建一个新代理：
Location: .sinapse-ai/development/agents/{agent-name}.md
Example: .sinapse-ai/development/agents/security-expert.md

# 我正在创建一个新任务：
Location: .sinapse-ai/development/tasks/{task-name}.md
Example: .sinapse-ai/development/tasks/deploy-to-production.md

# 我正在创建一个新工作流：
Location: .sinapse-ai/development/workflows/{workflow-name}.yaml
Example: .sinapse-ai/development/workflows/continuous-deployment.yaml

# 我正在创建一个新模板：
Location: .sinapse-ai/product/templates/{template-name}-tmpl.{yaml|md}
Example: .sinapse-ai/product/templates/deployment-plan-tmpl.yaml

# 我正在创建一个新检查清单：
Location: .sinapse-ai/product/checklists/{checklist-name}-checklist.md
Example: .sinapse-ai/product/checklists/security-review-checklist.md

# 我正在创建一个 CLI 命令：
Location: .sinapse-ai/cli/commands/{category}/{command-name}.js
Example: .sinapse-ai/cli/commands/generate/workflow.js

# 我正在创建一个核心实用工具：
Location: .sinapse-ai/core/utils/{utility-name}.js
Example: .sinapse-ai/core/utils/performance-monitor.js

# 我正在创建一个基础设施脚本：
Location: .sinapse-ai/infrastructure/scripts/{category}/{script-name}.js
Example: .sinapse-ai/infrastructure/scripts/llm-routing/router.js

# 我正在添加一个 PM 工具适配器：
Location: .sinapse-ai/infrastructure/integrations/pm-adapters/{adapter-name}.js
Example: .sinapse-ai/infrastructure/integrations/pm-adapters/monday-adapter.js
...
ADE 通过 `.sinapse-ai/core-config.yaml` 配置：
...
| 1.1 | 2025-12-14 | 更新组织为 SinapseAI，用 Squads 系统替换 Squads [Story 6.10] | Dex (dev) |
...
| 3.1 | 2026-02-06 | 添加数据文件治理部分：记录7个缺失的数据文件及其所有者、填充规则和更新触发器。扩展 .sinapse-ai/data/ 和 product/data/ 树列表。[Story ACT-8] | Dex (dev) |

---

_这是官方 SINAPSE 框架标准。所有文件放置必须遵循此结构。_
