...
**故事开发循环**是 SINAPSE 中央工作流，用于故事开发。它自动化从创建到交付的完整流程，整合质量网关，遵循序列: **创建 -> 验证 -> 实现 -> QA 审查**。
...
1. **core-config.yaml** - 必需的 SINAPSE 配置文件
   - `devStoryLocation` - 故事位置
   - `prd.*` - PRD 配置
   - `architecture.*` - 架构配置
   - `qa.qaLocation` - QA 工件位置

2. **故事模板** - `.sinapse-ai/development/templates/` 中的 `story-tmpl.yaml`
...
| 故事模板 | YAML | `.sinapse-ai/development/templates/` | 标准模板 |
...
| 文件 | 路径 |
|------|------|
| 工作流定义 | `.sinapse-ai/development/workflows/story-development-cycle.yaml` |
| SM 代理 | `.sinapse-ai/development/agents/sm.md` |
| PO 代理 | `.sinapse-ai/development/agents/po.md` |
| 开发代理 | `.sinapse-ai/development/agents/dev.md` |
| QA 代理 | `.sinapse-ai/development/agents/qa.md` |
| 创建故事任务 | `.sinapse-ai/development/tasks/create-next-story.md` |
| 验证故事任务 | `.sinapse-ai/development/tasks/validate-next-story.md` |
| 开发故事任务 | `.sinapse-ai/development/tasks/dev-develop-story.md` |
| QA 网关任务 | `.sinapse-ai/development/tasks/qa-gate.md` |
| 故事模板 | `.sinapse-ai/development/templates/story-tmpl.yaml` |
