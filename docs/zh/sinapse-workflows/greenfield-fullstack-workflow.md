...
**类型:** 绿地项目
**最后更新:** 2026-02-04
**源文件:** `.sinapse-ai/development/workflows/greenfield-fullstack.yaml`

---

## 概述

**绿地全栈工作流** 是 SINAPSE 从概念到开发的主要工作流,用于构建全栈应用。本工作流既支持复杂项目的全面规划,也支持简单项目的快速原型设计。
...
flowchart TD
    subgraph FASE0["第 0 阶段: 环境引导"]
        ...
        CHECK -->|验证| ENV_REPORT[".sinapse/environment-report.json 存在?"]
...
| 步骤 | 代理 | 任务 | 输入 | 输出 | 必需 |
|------|------|------|------|------|------|
| 1 | @devops (Gage) | `environment-bootstrap.md` | `project_name`, `project_path`, `github_org` | `.sinapse/config.yaml`, `.sinapse/environment-report.json`, `.gitignore`, `README.md`, `package.json` | 是 |

### 创建的工件

| 文件 | 描述 |
|------|------|
| `.sinapse/config.yaml` | SINAPSE 项目配置 |
| `.sinapse/environment-report.json` | 完整的环境报告 |
| `.gitignore` | Git 忽略规则 |
| `README.md` | 初始项目文档 |
| `package.json` | NPM 配置 |
...
- 仅在项目已有 `.sinapse/environment-report.json` 时跳过
- 切换机器或新团队成员加入时重新执行
...
| 阶段 | 输入 | 输出 |
|------|------|------|
| 0 | 项目名、GitHub 组织 | SINAPSE 配置、Git 仓库、文件夹结构 |
...
| 阶段 | 决策点 | 选项 | 标准 |
|------|--------|------|------|
| 0 | 环境就绪? | 跳过/执行引导 | `.sinapse/environment-report.json` 存在 |
...
flowchart TD
    D1{环境就绪?}
    D1 -->|检查 .sinapse/environment-report.json| D1_CHECK
...
| 问题 | 原因 | 解决方案 |
|------|------|---------|
| 模板未找到 | 路径错误 | 检查 `.sinapse-ai/development/templates/` |
...
| 类型 | 文件 | 描述 |
|------|------|------|
| 工作流 | `.sinapse-ai/development/workflows/greenfield-fullstack.yaml` | 工作流定义 |
| 任务 | `.sinapse-ai/development/tasks/environment-bootstrap.md` | 环境引导 |
| 任务 | `.sinapse-ai/development/tasks/shard-doc.md` | 文档分片 |
| 任务 | `.sinapse-ai/development/tasks/sm-create-next-story.md` | 故事创建 |
| 代理 | `.sinapse-ai/development/agents/*.md` | 代理定义 |
| 模板 | `.sinapse-ai/development/templates/*.yaml` | 文档模板 |
| 检查列表 | `.sinapse-ai/development/checklists/*.md` | 验证检查列表 |
