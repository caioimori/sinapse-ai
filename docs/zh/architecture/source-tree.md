...
- [概述](#概述)
- [当前结构 (sinapse-ai Brownfield)](#当前结构-sinapse-ai-brownfield)
...
## 当前结构 (sinapse-ai Brownfield)

```
sinapse-ai/                             # 根目录 (brownfield 项目)
├── .sinapse-ai/                        # 框架核心 (可移植)
│   ├── core/                          # 框架必要组件 (v4)
│   │   ├── config/                    # 配置系统
│   │   ├── data/                      # 核心知识库
...
Squad 是添加专用功能到 SINAPSE 的模块化扩展。不同于已弃用的 Squad，Squad 遵循标准化模板结构。
...
```yaml
# 我在创建新代理:
位置: .sinapse-ai/agents/{agent-name}.md
示例: .sinapse-ai/agents/security-expert.md

# 我在创建新任务:
位置: .sinapse-ai/tasks/{task-name}.md
示例: .sinapse-ai/tasks/deploy-to-production.md

# 我在创建新工作流:
位置: .sinapse-ai/workflows/{workflow-name}.yaml
示例: .sinapse-ai/workflows/continuous-deployment.yaml

# 我在创建新模板:
位置: .sinapse-ai/product/templates/{template-name}-tmpl.{yaml|md}
示例: .sinapse-ai/product/templates/deployment-plan-tmpl.yaml

# 我在编写故事:
位置: docs/stories/{epic-context}/{story-file}.md
示例: docs/stories/sinapse migration/story-6.1.2.6.md
...
# 我在创建实用脚本:
位置: .sinapse-ai/utils/{utility-name}.js
示例: .sinapse-ai/utils/performance-monitor.js
...
_这是 SINAPSE 框架的官方模式。所有文件放置应遵循此结构。_
