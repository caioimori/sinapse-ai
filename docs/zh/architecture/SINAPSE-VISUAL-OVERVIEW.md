...
**SINAPSE** (AI Operating System) 是一个框架，它改变了团队构建软件的方式 — 将 **专业化 AI 代理** 与 **真实人类** 结合在一种独特的敏捷方法论中。
...
flowchart LR
    ...
    subgraph TASKFIRST["Task-First (SINAPSE)"]
        direction TB
        TK1["已验证任务"] --> DEC1{"执行器?"}
        DEC1 --> E1["代理"]
        DEC1 --> E2["工作器"]
...
```mermaid
flowchart TB
    subgraph EXECUTORS["SINAPSE 的 4 种执行器"]
        direction TB
...
这是决定 SINAPSE 中每个任务应分配哪个执行器的主要流程图:
...
SINAPSE 是模块化的 — 你像乐高积木一样组合小队、代理和配置。
...
flowchart TB
    subgraph WS["公司动态工作空间"]
        ...
        subgraph FRAMEWORK[".sinapse-ai/ — 框架 (不可变)"]
            AGENTS_CORE["11 个核心代理<br/>dev, qa, architect, pm,<br/>po, sm, analyst, devops,<br/>data-engineer, ux, sinapse-master"]
...
```mermaid
flowchart LR
    INSTALL(["npx sinapse-ai install"]) --> WIZARD["安装向导"]
...
SINAPSE 的差异化在于小队 **不会取代** 人类 — 它们 **一起** 工作:
...
flowchart LR
    ...
    subgraph SINAPSE_WAY["SINAPSE (Task-First)"]
        direction TB
    ...
    style SINAPSE_WAY fill:#27ae60,color:#fff
```

### 宪法原则

SINAPSE 在具有不可协商原则的 **正式宪法** 下运行:
...
| 文档 | 内容 |
|-----------|----------|
| [EXECUTOR-DECISION-TREE.md](../../.sinapse-ai/docs/standards/EXECUTOR-DECISION-TREE.md) | 带示例和成本效益分析的详细决策树 |
| [SYNAPSE-FLOWCHARTS.md](SYNAPSE/SYNAPSE-FLOWCHARTS.md) | SYNAPSE 上下文引擎的 12 个流程图 |
| [Constitution](../../.sinapse-ai/constitution.md) | 框架的不可协商原则 |
| [User Guide](../../.sinapse-ai/user-guide.md) | SINAPSE 使用完整指南 |
| [Squads Guide](../guides/squads-guide.md) | 如何创建和管理小队 |

---

*SINAPSE 可视化概览 v1.0.0*
