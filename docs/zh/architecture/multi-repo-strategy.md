...
- [概览](#概览)
- [仓库结构](#仓库结构)
- [主要仓库 (sinapse-ai)](#主要仓库-sinapse-ai)
- [Squad 仓库](#squad-仓库)
...
SINAPSE v4 采用**多仓库策略**以实现模块化开发、社区贡献和核心框架、扩展(squads)和专有组件之间的清晰分离。
...
主仓库包含所有项目依赖的基础 SINAPSE 框架。

### 内容

| 目录                       | 描述                                    |
| -------------------------- | --------------------------------------- |
| `.sinapse-ai/core/`         | 框架基础(配置、注册表、质量门)          |
| `.sinapse-ai/development/`  | 代理、任务、工作流定义                  |
| `.sinapse-ai/product/`      | 模板、检查表、PM 数据                   |
| `.sinapse-ai/infrastructure/` | 脚本、工具、集成                      |
| `docs/`                    | 框架文档                                |
...
| 包                   | 注册表 | 许可证     | 仓库        |
| -------------------- | ------ | ---------- | ----------- |
| `@sinapse/core`         | npm    | MIT        | sinapse-ai   |
| `@sinapse/squad-etl`    | npm    | MIT        | sinapse-squads |
| `@sinapse/squad-creator`| npm    | MIT        | sinapse-squads |
| `@sinapse/squad-mmos`   | npm    | MIT        | sinapse-squads |
| `@sinapse/mcp-presets`  | npm    | Apache 2.0 | mcp-ecosystem |
...
_最后更新: 2026-01-28 | SINAPSE 框架团队_
