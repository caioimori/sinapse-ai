<!--
  翻译：zh-CN（简体中文）
  原文：/docs/sinapse-agent-flows/squad-creator-system.md
  最后同步：2026-02-22
-->
...
**小队创建者** (Craft) 是 SINAPSE 用于创建、验证、发布和管理小队的专业代理。小队是代理、任务、工作流和资源的模块化包，可在项目之间重用。

该系统实现了 SINAPSE 的**任务优先架构**，其中任务是执行的主要入口点，代理编排这些任务。

### 系统目的

- **创建小队**遵循 SINAPSE 模式和结构
- **验证小队**针对 JSON Schema 和任务规范
- **列出小队**项目本地
- **分发小队**在 3 个级别（本地、sinapse-squads、SINAPSE API）
- **迁移小队**到带编排和技能的 v2 格式
- **分析和扩展**现有小队
...
4. **3 级分发**：本地、公共（sinapse-squads）、市场（SINAPSE API）
5. **与 sinapse-ai 集成**：小队与框架协同工作
...
| 文件 | 目的 |
|---------|-----------|
| `.sinapse-ai/development/agents/squad-creator.md` | 小队创建者代理核心定义 |
| `.claude/commands/SINAPSE/agents/squad-creator.md` | 用于激活 @squad-creator 的 Claude Code 命令 |

### @squad-creator 任务文件

| 文件 | 命令 | 目的 | 状态 |
|---------|---------|-----------|--------|
| `.sinapse-ai/development/tasks/squad-creator-create.md` | `*create-squad` | 创建带完整结构的新小队 | 活跃 |
| `.sinapse-ai/development/tasks/squad-creator-design.md` | `*design-squad` | 分析文档并生成蓝图 | 活跃 |
| `.sinapse-ai/development/tasks/squad-creator-validate.md` | `*validate-squad` | 验证小队针对 schema 和模式 | 活跃 |
| `.sinapse-ai/development/tasks/squad-creator-list.md` | `*list-squads` | 列出本地小队 | 活跃 |
| `.sinapse-ai/development/tasks/squad-creator-analyze.md` | `*analyze-squad` | 分析结构并建议改进 | 活跃 |
| `.sinapse-ai/development/tasks/squad-creator-extend.md` | `*extend-squad` | 使用新组件扩展小队 | 活跃 |
| `.sinapse-ai/development/tasks/squad-creator-migrate.md` | `*migrate-to-v2` | 迁移小队到 v2 格式 | 活跃 |
| `.sinapse-ai/development/tasks/squad-generate-skills.md` | `*generate-skills` | 生成小队知识技能 | 活跃 |
...
| 文件 | 命令 | 目的 |
|---------|---------|-----------|
| `.sinapse-ai/development/tasks/create-agent.md` | `*create-agent` | 创建单个代理定义 |
| `.sinapse-ai/development/tasks/create-task.md` | `*create-task` | 创建单个任务文件 |
| `.sinapse-ai/development/tasks/create-workflow.md` | `*create-workflow` | 创建编排工作流 |

### 支持脚本

| 文件 | 类/函数 | 目的 |
|---------|---------------|-----------|
| `.sinapse-ai/development/scripts/squad/squad-generator.js` | `SquadGenerator` | 生成完整小队结构 |
| `.sinapse-ai/development/scripts/squad/squad-validator.js` | `SquadValidator` | 验证小队针对 schema 和模式 |
| `.sinapse-ai/development/scripts/squad/squad-loader.js` | `SquadLoader` | 加载和解析小队 |
| `.sinapse-ai/development/scripts/squad/squad-designer.js` | `SquadDesigner` | 分析文档并生成蓝图 |
| `.sinapse-ai/development/scripts/squad/squad-analyzer.js` | `SquadAnalyzer` | 分析小队结构 |
| `.sinapse-ai/development/scripts/squad/squad-extender.js` | `SquadExtender` | 扩展现有小队 |
| `.sinapse-ai/development/scripts/squad/squad-migrator.js` | `SquadMigrator` | 迁移小队到 v2 |
| `.sinapse-ai/development/scripts/squad/squad-downloader.js` | `SquadDownloader` | 从仓库下载小队 |
| `.sinapse-ai/development/scripts/squad/squad-publisher.js` | `SquadPublisher` | 发布小队 |

### JSON Schema

| 文件 | 目的 |
|---------|-----------|
| `.sinapse-ai/schemas/squad-schema.json` | squad.yaml 验证 schema |
| `.sinapse-ai/schemas/squad-design-schema.json` | 蓝图验证 schema |
...
sinapse:
  minVersion: string  # SINAPSE 最低版本
  type: squad
...
- [任务：squad-creator-create.md](.sinapse-ai/development/tasks/squad-creator-create.md)
- [任务：squad-creator-validate.md](.sinapse-ai/development/tasks/squad-creator-validate.md)
- [任务：squad-creator-design.md](.sinapse-ai/development/tasks/squad-creator-design.md)
- [脚本：squad-generator.js](.sinapse-ai/development/scripts/squad/squad-generator.js)
- [脚本：squad-validator.js](.sinapse-ai/development/scripts/squad/squad-validator.js)
- [Schema：squad-schema.json](.sinapse-ai/schemas/squad-schema.json)
- [代理：squad-creator.md](.sinapse-ai/development/agents/squad-creator.md)
- [命令：squad-creator.md](.claude/commands/SINAPSE/agents/squad-creator.md)
...
| **分发级别** | 3 个（本地、sinapse-squads、SINAPSE API） |
