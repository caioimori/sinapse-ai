<!--
  翻译：zh-CN（简体中文）
  原文：/docs/sinapse-agent-flows/sm-system.md
  最后同步：2026-02-22
-->
...
**@sm (River)** 代理是 SINAPSE 的技术 Scrum Master，专门负责故事准备和敏捷流程促进。其主要角色是创建详细、可操作的故事，使开发代理能够以最少的额外研究进行实现。
...
| 文件 | 命令 | 目的 |
|---------|---------|-----------|
| `.sinapse-ai/development/tasks/sm-create-next-story.md` | `*draft` | 创建待办列表中下一个故事的主要任务 |
| `.sinapse-ai/development/tasks/create-next-story.md` | `*draft` | 故事创建任务的完整版本 |
| `.sinapse-ai/development/tasks/execute-checklist.md` | `*story-checklist` | 执行故事草稿验证检查清单 |
| `.sinapse-ai/development/tasks/correct-course.md` | `*correct-course` | 分析并纠正流程偏差 |
| `.sinapse-ai/development/tasks/collaborative-edit.md` | - | 文档协作编辑 |
| `.sinapse-ai/development/tasks/init-project-status.md` | - | 项目状态初始化 |

### 代理定义文件

| 文件 | 目的 |
|---------|-----------|
| `.sinapse-ai/development/agents/sm.md` | SM 代理核心定义 |
| `.claude/commands/SINAPSE/agents/sm.md` | 用于激活 @sm 的 Claude Code 命令 |
...
| 文件 | 目的 |
|---------|-----------|
| `.sinapse-ai/product/checklists/story-draft-checklist.md` | 验证故事草稿的质量和完整性 |
| `.sinapse-ai/product/checklists/story-dod-checklist.md` | 故事完成定义 |
| `.sinapse-ai/product/checklists/change-checklist.md` | 变更和课程纠正导航 |
| `.sinapse-ai/product/checklists/po-master-checklist.md` | 验证中使用的主检查清单 |

### 其他代理的相关文件

| 文件 | 代理 | 目的 |
|---------|--------|-----------|
| `.sinapse-ai/development/agents/po.md` | @po | 与 @sm 在待办和冲刺规划上协调 |
| `.sinapse-ai/development/agents/dev.md` | @dev | 从 @sm 接收故事进行实现 |
| `.sinapse-ai/development/agents/pm.md` | @pm | 创建 @sm 分解为故事的史诗 |
| `.sinapse-ai/development/agents/devops.md` | @github-devops | 接收已完成故事进行 push/PR |
| `.sinapse-ai/development/agents/qa.md` | @qa | 在风险分析上协调 |

### 使用 @sm 的工作流文件

| 文件 | 目的 |
|---------|-----------|
| `.sinapse-ai/development/workflows/story-development-cycle.yaml` | 完整的故事开发周期 |
| `.sinapse-ai/development/workflows/greenfield-fullstack.yaml` | 绿地全栈工作流 |
| `.sinapse-ai/development/workflows/greenfield-service.yaml` | 绿地服务工作流 |
| `.sinapse-ai/development/workflows/greenfield-ui.yaml` | 绿地 UI 工作流 |
| `.sinapse-ai/development/workflows/brownfield-fullstack.yaml` | 棕地全栈工作流 |
| `.sinapse-ai/development/workflows/brownfield-service.yaml` | 棕地服务工作流 |
| `.sinapse-ai/development/workflows/brownfield-ui.yaml` | 棕地 UI 工作流 |

### 配置文件

| 文件 | 目的 |
|---------|-----------|
| `.sinapse-ai/core-config.yaml` | 中央配置（devStoryLocation 等） |
| `.sinapse-ai/development/scripts/unified-activation-pipeline.js` | 智能问候管道 |
| `.sinapse-ai/development/scripts/agent-assignment-resolver.js` | 代理分配解析 |
...
**解决方案：**
1. 从 `GITHUB sinapse-ai/core-config.yaml` 复制
2. 或执行 SINAPSE 安装器：`npm run sinapse:install`
...
### 代理文件
- [代理：sm.md](.sinapse-ai/development/agents/sm.md)
- [任务：create-next-story.md](.sinapse-ai/development/tasks/create-next-story.md)
- [任务：execute-checklist.md](.sinapse-ai/development/tasks/execute-checklist.md)
- [任务：correct-course.md](.sinapse-ai/development/tasks/correct-course.md)

### 检查清单
- [检查清单：story-draft-checklist.md](.sinapse-ai/product/checklists/story-draft-checklist.md)
- [检查清单：story-dod-checklist.md](.sinapse-ai/product/checklists/story-dod-checklist.md)
- [检查清单：change-checklist.md](.sinapse-ai/product/checklists/change-checklist.md)

### 工作流
- [工作流：story-development-cycle.yaml](.sinapse-ai/development/workflows/story-development-cycle.yaml)
- [工作流：greenfield-fullstack.yaml](.sinapse-ai/development/workflows/greenfield-fullstack.yaml)
- [工作流：brownfield-fullstack.yaml](.sinapse-ai/development/workflows/brownfield-fullstack.yaml)

### 配置
- [核心配置](../.sinapse-ai/core-config.yaml)
