# 数据工程师代理系统 (@data-engineer) - SINAPSE
...
**@数据工程师(Dara)**是SINAPSE的数据库架构师和运营工程师，负责架构设计、迁移、安全性(RLS)、性能优化和DBA操作。该代理充当**Sage(智者)**，建模业务领域，使用快照和回滚实现安全迁移，并确保数据完整性和安全性。
...
| 文件 | 命令 | 目的 |
|------|------|------|
| `.sinapse-ai/development/tasks/db-domain-modeling.md` | `*model-domain` | 交互式域建模会话 |
| `.sinapse-ai/development/tasks/setup-database.md` | `*setup-database [type]` | 数据库项目设置(Supabase、PostgreSQL、MongoDB、MySQL、SQLite) |
| `.sinapse-ai/development/tasks/db-bootstrap.md` | `*bootstrap` | 创建标准Supabase项目结构 |
| `.sinapse-ai/development/tasks/db-env-check.md` | `*env-check` | 验证数据库环境变量 |
| `.sinapse-ai/development/tasks/db-apply-migration.md` | `*apply-migration {path}` | 应用迁移(含快照和锁定建议) |
| `.sinapse-ai/development/tasks/db-dry-run.md` | `*dry-run {path}` | 测试迁移无提交 |
| `.sinapse-ai/development/tasks/db-seed.md` | `*seed {path}` | 应用种子数据(幂等) |
| `.sinapse-ai/development/tasks/db-snapshot.md` | `*snapshot {label}` | 创建架构快照 |
| `.sinapse-ai/development/tasks/db-rollback.md` | `*rollback {snapshot_or_file}` | 恢复快照或执行回滚 |
| `.sinapse-ai/development/tasks/db-smoke-test.md` | `*smoke-test {version}` | 全面的数据库测试 |
| `.sinapse-ai/development/tasks/security-audit.md` | `*security-audit {scope}` | 安全审计(rls、架构、完整) |
| `.sinapse-ai/development/tasks/analyze-performance.md` | `*analyze-performance {type}` | 性能分析(查询、热路径、交互) |
| `.sinapse-ai/development/tasks/db-policy-apply.md` | `*policy-apply {table} {mode}` | 安装RLS策略(KISS或详细) |
| `.sinapse-ai/development/tasks/test-as-user.md` | `*test-as-user {user_id}` | 模拟用户以测试RLS |
| `.sinapse-ai/development/tasks/db-verify-order.md` | `*verify-order {path}` | 验证DDL顺序以处理依赖 |
| `.sinapse-ai/development/tasks/db-load-csv.md` | `*load-csv {table} {file}` | 安全的CSV加载程序(staging->merge) |
| `.sinapse-ai/development/tasks/db-run-sql.md` | `*run-sql {file_or_inline}` | 执行原始SQL(含事务) |
| `.sinapse-ai/development/tasks/create-deep-research-prompt.md` | `*research {topic}` | 生成深度研究提示 |
| `.sinapse-ai/development/tasks/execute-checklist.md` | `*execute-checklist {checklist}` | 执行DBA检查清单 |
| `.sinapse-ai/development/tasks/create-doc.md` | `*doc-out` | 完整文档输出 |

### 代理定义文件

| 文件 | 目的 |
|------|------|
| `.sinapse-ai/development/agents/data-engineer.md` | @data-engineer代理核心定义(角色、命令、工作流) |
| `.claude/commands/SINAPSE/agents/data-engineer.md` | Claude Code命令激活@data-engineer |
...
| 文件 | 目的 |
|------|------|
| `.sinapse-ai/development/workflows/brownfield-discovery.yaml` | Brownfield发现工作流(第2和第5阶段) |
