...
如何将旧版Squad迁移到SINAPSE 2.1格式。

## 概述

SINAPSE 2.1引入了新的Squad格式，具有以下特性：
- 任务优先的架构
- JSON Schema验证
- 三层级分发
- 标准化清单 (`squad.yaml`)
...
| SINAPSE类型字段 | 缺失 | `sinapse.type: squad` |
| 最低版本 | 缺失 | `sinapse.minVersion: "2.1.0"` |
| 结构 | Agent优先 | Task优先 |
...
- [ ] `squad.yaml` 存在且有效
- [ ] `sinapse.type` 是 `"squad"`
- [ ] `sinapse.minVersion` 是 `"2.1.0"` 或更高
