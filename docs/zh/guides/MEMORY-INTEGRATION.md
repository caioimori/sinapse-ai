...
集成遵循 SINAPSE Open Core 模型:
- **sinapse-ai:** UnifiedActivationPipeline 中的扩展点 (本指南)
- **sinapse-pro:** 记忆智能实现 (检索, 评分, 学习)
...
```javascript
isProAvailable() === true
// 且 .sinapse/session-digests/ 包含记忆摘要
```
...
**解决方案:**
1. 减少摘要数量 (将旧摘要归档到 `.sinapse/session-digests/archive/`)
