...
SINAPSE 工作流是一系列精心编排的代理活动序列，可以自动化复杂的开发流程。它为常见的开发场景提供了结构化、可重复的模式。
...
工作流根据上下文保存:
- **核心**: `.sinapse-ai/development/workflows/{name}.yaml`
...
状态持久化在`.sinapse/{instance-id}-state.yaml`:
...
| **状态** | 在会话间持久化 | 无状态 |
| **用例** | 复杂流程 | 原子操作 |
| **位置** | `.sinapse-ai/development/workflows/` | `.sinapse-ai/development/tasks/` |
...
- [SINAPSE工作流](../sinapse-workflows/README.md) - 每个工作流的完整文档
