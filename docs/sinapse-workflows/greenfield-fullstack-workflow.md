...
**Arquivo Fonte:** `.sinapse-ai/development/workflows/greenfield-fullstack.yaml`

---

## Visão Geral

O **Greenfield Full-Stack Workflow** e o fluxo de trabalho principal do SINAPSE para construcao de aplicacoes full-stack desde o conceito ate o desenvolvimento. Este workflow suporta tanto planejamento abrangente para projetos complexos quanto prototipagem rapida para projetos simples.
...
flowchart TD
    subgraph FASE0["FASE 0: Environment Bootstrap"]
        ...
        CHECK -->|Verificar| ENV_REPORT[".sinapse/environment-report.json existe?"]
...
|------|--------|------|---------|-------|-------------|
| 1 | @devops (Gage) | `environment-bootstrap.md` | `project_name`, `project_path`, `github_org` | `.sinapse/config.yaml`, `.sinapse/environment-report.json`, `.gitignore`, `README.md`, `package.json` | Sim |

### Artefatos Criados

| Arquivo | Descricao |
|---------|-----------|
| `.sinapse/config.yaml` | Configuracao do projeto SINAPSE |
| `.sinapse/environment-report.json` | Relatorio completo do ambiente |
...
- Pular apenas se o projeto ja tiver `.sinapse/environment-report.json`
...
| 5 | @developer (Dex) | `apply-qa-fixes.md` | Feedback de QA | Correcoes aplicadas | Condicional |
...
| 3 | Apply QA Fixes | @developer | `apply-qa-fixes.md` |
...
| Fase | Entrada | Saida |
|------|---------|-------|
| 0 | Nome do projeto, organizacao GitHub | Config SINAPSE, repo Git, estrutura de pastas |
...
|------|------------------|--------|----------|
| 0 | Ambiente pronto? | Pular / Executar bootstrap | Existencia de `.sinapse/environment-report.json` |
...
flowchart TD
    D1{Ambiente pronto?}
    D1 -->|Verificar .sinapse/environment-report.json| D1_CHECK
...
| Problema | Causa | Solucao |
|----------|-------|---------|
| Template nao encontrado | Caminho incorreto | Verificar `.sinapse-ai/development/templates/` |
...
```bash
# Verificar ambiente
cat .sinapse/environment-report.json
...
|------|---------|-----------|
| Workflow | `.sinapse-ai/development/workflows/greenfield-fullstack.yaml` | Definicao do workflow |
| Task | `.sinapse-ai/development/tasks/environment-bootstrap.md` | Bootstrap do ambiente |
| Task | `.sinapse-ai/development/tasks/shard-doc.md` | Fragmentacao de documentos |
| Task | `.sinapse-ai/development/tasks/sm-create-next-story.md` | Criacao de stories |
| Agente | `.sinapse-ai/development/agents/*.md` | Definicoes de agentes |
| Template | `.sinapse-ai/development/templates/*.yaml` | Templates de documentos |
| Checklist | `.sinapse-ai/development/checklists/*.md` | Checklists de validacao |
