# Integracao SINAPSE com Codex CLI (Estado Atual)

Este documento descreve o estado operacional atual da integracao SINAPSE + Codex CLI no SINAPSE `4.2.11`.
O foco aqui nao e historico: e operacao pratica, compatibilidade real e como manter sem regressao.

## Resumo Executivo

O Codex hoje e alvo de primeira classe no SINAPSE:
...
Em termos praticos: Codex esta no mesmo trilho arquitetural das integracoes principais do SINAPSE, sem caminho paralelo legado.

## Status de Compatibilidade (SINAPSE 4.2)

| O que você quer fazer | Funciona no Codex? | Como fazer |
| --- | --- | --- |
| Ativar agentes SINAPSE | Works | `/skills` depois escolha `sinapse-<agent-id>` |
| Sincronizar e validar arquivos SINAPSE | Works | `npm run sync:ide:codex` e `npm run validate:codex-sync` |
...
- Agentes canonicos: `.sinapse-ai/development/agents/*.md`
- Regras de projeto Codex: `AGENTS.md`

### Pipeline de ativacao

- Runtime: `.sinapse-ai/development/scripts/activation-runtime.js`
- Entrada de greeting: `.sinapse-ai/development/scripts/generate-greeting.js`
...
4. No Codex, usar `/skills` e escolher `sinapse-<agent-id>` como ativacao padrao.
...
- Integracao estrutural: `.sinapse-ai/infrastructure/scripts/validate-codex-integration.js`
- Skills: `.sinapse-ai/infrastructure/scripts/codex-skills-sync/validate.js`
- Paths/contratos: `.sinapse-ai/infrastructure/scripts/validate-paths.js`
...
No SINAPSE, o caminho recomendado continua: `AGENTS.md` + `/skills` + MCP + scripts de sync/validacao.
...
- menor automacao de eventos de ciclo de sessao (`SessionStart/SessionEnd`) no padrao SINAPSE
...
Mitigacao operacional no SINAPSE:
...
- garantir skill apontando para agente canonico em `.sinapse-ai/development/agents/`
