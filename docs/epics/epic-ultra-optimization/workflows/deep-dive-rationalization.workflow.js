// Workflow: deep-dive-rationalization — Ultra-Otimização SINAPSE-AI (sessão 3+)
// Frota multi-agente do deep-dive de gargalos + racionalização de features.
// Cópia canônica e versionada do script da sessão 11/06/2026.
//
// COMO RELANÇAR (nova sessão):
//   Workflow({ scriptPath: '<este arquivo>' })
// Pra aproveitar módulos já analisados (sem pagar de novo), passe via args:
//   Workflow({ scriptPath: '<este arquivo>', args: {
//     skipModules: ['docs','doctor', ...],            // nomes da lista MODULES a pular
//     precomputed: { modules: [<reports salvos>] },   // reports correspondentes (ver partial-results-*.json)
//   }})
// Resultados parciais da rodada 2 (5 módulos completos): partial-results-rodada2-2026-06-11.json (mesma pasta).
...
const SKIP_MODULES = new Set((args && args.skipModules) || [])
const SKIP_SPECIALS = new Set((args && args.skipSpecials) || [])
const PRE = (args && args.precomputed) || {}
const MODULES_TO_RUN = MODULES.filter((m) => !SKIP_MODULES.has(m))
...
log(`Mapa do core: ${MODULES_TO_RUN.length}/${MODULES.length} módulos a auditar (${SKIP_MODULES.size} pré-computados) + céticos por achado`)
...
const SPECIALS_TO_RUN = SPECIALS.filter((s) => !SKIP_SPECIALS.has(s.key))

log(`Frentes especiais: ${SPECIALS_TO_RUN.map((s) => s.key).join(', ')}`)
...
const moduleResults = moduleResultsRaw.filter(Boolean).concat(PRE.modules || [])
const specialResults = specialResultsRaw.filter(Boolean).concat(PRE.specials || [])

log(`Mapa: ${moduleResults.length}/${MODULES.length} módulos (incl. pré-computados). Especiais: ${specialResults.length}/${SPECIALS.length} frentes. Sintetizando...`)
