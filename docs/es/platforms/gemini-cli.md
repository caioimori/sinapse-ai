...
# Guía de SINAPSE para Gemini CLI
...
### ¿Por qué usar SINAPSE con Gemini CLI?
...
```markdown
# Reglas de SINAPSE para Gemini CLI
...
## Activación
Incluye "Como el agente SINAPSE dev" en tu prompt.
...
### Activando Agentes de SINAPSE

Incluye contexto de agentes en los prompts:

```bash
# Agente de desarrollador
gemini "Como el agente SINAPSE dev, implementa la característica de login"

# Agente de QA
gemini "Como el agente SINAPSE qa, revisa este código por seguridad"

# Agente de arquitecto
gemini "Como el agente SINAPSE architect, diseña el esquema de base de datos"
```
...
cat src/auth.ts | gemini "Como SINAPSE qa, revisa este código"
...
for file in src/*.ts; do
  gemini --file "$file" "Como SINAPSE qa, revisa este archivo" >> reviews.md
done
```
...
```mermaid
flowchart LR
    Source["SINAPSE Core<br/>.sinapse-ai/development/agents"] --> Parser["Agent Parser"]
...
**Frase de Activación:** "Como el agente SINAPSE dev"

## Rol
Eres un Desarrollador Full Stack Senior trabajando con SINAPSE.
...
1. Instalar SINAPSE para Gemini CLI:
   ```bash
...
1. Los agentes de SINAPSE en `.sinapse-ai/` son portátiles
...
*SINAPSE - Guía de Plataforma Gemini CLI v1.0*
