...
# Guía SINAPSE para Claude Code

> 🌐 [EN](../../platforms/claude-code.md) | [PT](../../pt/platforms/claude-code.md) | **ES**

---

> **CLI Oficial de Anthropic** - Plataforma Recomendada para SINAPSE
...
### ¿Por qué usar SINAPSE con Claude Code?

Claude Code ofrece la **mejor integración** con SINAPSE debido a:
...
# Verifica que los archivos SINAPSE fueron creados
ls -la .claude/
```
...
export SINAPSE_DEBUG="true"                        # Habilitar registro de depuración
```
...
```mermaid
flowchart LR
    Source["SINAPSE Core<br/>.sinapse-ai/development/agents"] --> Parser["Analizador de Agentes"]
...
1. Los cambios locales se respaldan en `.claude/commands/SINAPSE/agents/.backup/`
...
# Verificar registros de SINAPSE
cat .sinapse-ai/logs/sync.log
```
...
### Preguntas Específicas de SINAPSE
...
R: En `.claude/commands/SINAPSE/agents/` después de la sincronización.

**P: ¿Con qué frecuencia debo sincronizar agentes?**
R: Después de actualizar SINAPSE o cuando los agentes se modifiquen en el núcleo.
...
2. Inicializa SINAPSE con Claude Code:
   ```bash
...
1. Tus agentes SINAPSE se almacenan en `.sinapse-ai/development/agents/`
2. Inicializa SINAPSE para el IDE objetivo
...
*SINAPSE - Guía de Plataforma Claude Code v1.0*
