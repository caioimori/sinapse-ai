...
Este documento define los modos de ejecución para el Sistema de Verificación de Salud de SINAPSE (HCS), basado en investigación de mejores prácticas de la industria de Kubernetes, VS Code, Terraform, npm/yarn, y patrones CLI "doctor" (Flutter, Homebrew, WP-CLI).
...
| **Al instalar/bootstrap**    | `npx sinapse install` | 60-120s  | Esperado                 | Validación de setup         | ✅ **Requerido**  |
...
jobs:
  health-check:
    ...
    steps:
      ...
      - run: npx sinapse health-check --mode=full --report
      - uses: actions/upload-artifact@v4
        with:
          name: health-report
          path: .sinapse/reports/health-check-*.md
```
...
**Veredicto:** Requerido. Parte de `npx sinapse install` y `*bootstrap-setup`.
    ...
    steps:
      ...
      - name: Post to Slack (on failure)
        ...
        with:
          payload: |
            {
              "text": "⚠️ SINAPSE Health Check Falló",
              "blocks": [
                {
                  "type": "section",
                  "text": {
...
jobs:
  post-merge-health:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: npx sinapse health-check --mode=quick
```
...
{
  "sinapse.healthCheck.enabled": true,
  "sinapse.healthCheck.interval": 300,
  "sinapse.healthCheck.mode": "quick",
  "sinapse.healthCheck.showNotifications": true
}
```
