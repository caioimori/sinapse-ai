...
El **Motor de Desarrollo Autónomo (ADE)** es la infraestructura de SINAPSE para flujos de trabajo de desarrollo autónomo. Permite que los agentes de IA trabajen de forma independiente a través de pipelines inteligentes, bucles autorreparables y aprendizaje persistente.
...
│  │         ▼                ▼                ▼                ▼          │ │
│  │  ┌─────────────────────────────────────────────────────────────────┐  │ │
│  │  │                     Estado en Tiempo de Ejecución .sinapse/        │  │ │
│  │  └─────────────────────────────────────────────────────────────────┘  │ │
│  │         │                │                │                │          │ │
│  │         ▼                ▼                ▼                ▼          │ │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐   │ │
...
Todo el estado de ADE se persiste en `.sinapse/` para:
...
Ubicación: .sinapse-ai/infrastructure/scripts/
...
Archivo de Estado: .sinapse/project-status.yaml
```
...
| ----------------- | ------------------------------- | --------------------------------- |
| Patrones de Código | Patrones reutilizables de la base de código | .sinapse/patterns/code-patterns.json |
| Gotchas           | Trampas conocidas y soluciones  | .sinapse/patterns/gotchas.json       |
| Insights de Sesión | Descubrimientos durante sesiones | .sinapse/sessions/                   |
| Mapa de Base de Código | Análisis de estructura del proyecto | .sinapse/codebase-map.json           |
...
ADE se integra con agentes SINAPSE a través de:
...
Todos los componentes de ADE actualizan `.sinapse/status.json`:
...
Ubicado en `.sinapse-ai/core-config.yaml`:
...
ade:
  ...
  capaMemoria:
    habilitado: true
    almacenPatrones: .sinapse/patterns/
...
```
.sinapse-ai/workflow-intelligence/
├── engine/
...
_Este es un estándar oficial del framework SINAPSE que documenta el Motor de Desarrollo Autónomo._
