...
Este documento define límites claros de responsabilidad para todos los agentes SINAPSE, con enfoque particular en:
...
1. **Hook Git Pre-Push** (Aplicación Primaria)
   ...
   - Verifica: Variable de entorno `$SINAPSE_ACTIVE_AGENT`
   - Acción: Bloquea push si agente != "github-devops"

2. **Variables de Entorno** (Detección en Tiempo de Ejecución)
   ```bash
   export SINAPSE_ACTIVE_AGENT="github-devops"
   export SINAPSE_GIT_PUSH_ALLOWED="true"
   ```
...
- [ ] **Crear Hook Git Pre-Push**
  - Ubicación: `.git/hooks/pre-push`
  - Contenido: Verificar `$SINAPSE_ACTIVE_AGENT`, bloquear si != "github-devops"
...
- [ ] **Actualizar Scripts de Activación de Agentes**
  - Agregar configuración de variable de entorno: `SINAPSE_ACTIVE_AGENT={agent_id}`
  - Configurar `SINAPSE_GIT_PUSH_ALLOWED` apropiadamente
