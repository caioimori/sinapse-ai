...
Este documento especifica las capacidades de auto-reparación del Sistema de Verificación de Salud de SINAPSE (HCS). La auto-reparación permite que el sistema corrija automáticamente ciertos problemas mientras mantiene la seguridad y el control del usuario.
...
| ---------------------- | ----------------------------------------------------- | ------------ |
| `recreate_config`      | Recrear `.sinapse/config.yaml` faltante desde plantilla  | Sí           |
| `clear_cache`          | Limpiar archivos de caché obsoletos en `.sinapse/cache/` | Sí           |
...
- id: PC-001
  description: '.sinapse/config.yaml faltante'
  severity: CRITICAL
  tier: 1
...
✅ Auto-corregidos 3 problemas:
   • Recreado .sinapse/config.yaml (respaldo: .sinapse/backups/config.yaml.1735564800)
   • Reiniciado servidor MCP context7
   • Agregado .sinapse/cache/ a .gitignore
```
...
- id: PC-002
  ...
  guide:
    ...
    steps:
      - 'Abrir .sinapse-ai/development/tasks/deploy.md'
      - 'Encontrar línea: agent: legacy-dev'
      - 'Reemplazar con: agent: devops'
      - 'Verificar con: npx sinapse task validate deploy'
...
Pasos para resolver:
  1. Abrir .sinapse-ai/development/tasks/deploy.md
  2. Encontrar línea: agent: legacy-dev
  3. Reemplazar con: agent: devops
  4. Verificar con: npx sinapse task validate deploy
...
neverAutoFix:
  files:
    ...
    - '**/*.{json,yaml,yml}' # Config de usuario (fuera de .sinapse/)
...
safeToAutoFix:
  paths:
    - '.sinapse/**' # Archivos del workspace SINAPSE
    - '.sinapse-ai/**/*.yaml' # YAML del framework (con cuidado)
...
```json
{
  ...
  "rollbackCommand": "npx sinapse health-check --rollback health-check-2025-12-30T10-30-00"
}
```
...
async function createBackup(action) {
  ...
  const backupDir = path.join('.sinapse', 'backups', backupId);
...
npx sinapse health-check --rollback health-check-2025-12-30T10-30-00

# Listar respaldos disponibles
npx sinapse health-check --list-backups

# Revertir último respaldo
npx sinapse health-check --rollback-last
```
...
class RecreateConfigHealer extends BaseHealer {
  ...
  async canHeal(issue) {
    return issue.id === 'PC-001' && !(await fs.pathExists('.sinapse/config.yaml'));
  }

  async heal(issue, context) {
    const template = await fs.readFile('.sinapse-ai/templates/config-template.yaml');
    await fs.writeFile('.sinapse/config.yaml', template);
    return { success: true, message: 'Config recreado desde plantilla' };
  }

  async verify(issue) {
    return await fs.pathExists('.sinapse/config.yaml');
  }
}
```
...
```javascript
// .sinapse/logs/self-healing.log
const healingLog = {
  append: async (entry) => {
    const logPath = '.sinapse/logs/self-healing.log';
    const logEntry = {
