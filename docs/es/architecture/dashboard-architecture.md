# 🏛️ SINAPSE Dashboard - Arquitectura Completa
...
El SINAPSE Dashboard es una aplicación Next.js que proporciona una interfaz visual para monitorear y gestionar el sistema SINAPSE. Se comunica con el CLI/SINAPSE a través de archivos de estado en el sistema de archivos y Server-Sent Events (SSE).

### Principios Arquitectónicos

1. **CLI-First**: Dashboard es complementario al CLI, no sustituto
2. **File-Based Communication**: Estado a través de `.sinapse/dashboard/status.json`
...
│                              FILESYSTEM                                  │
│  ┌──────────────────────────┐  ┌──────────────────────────────────────┐ │
│  │ .sinapse/dashboard/         │  │ docs/stories/                        │ │
...
│   │   │   ├── status/route.ts       # Polling de estado SINAPSE
...
│   │   ├── use-sinapse-status.ts        # Estado con SWR
...
| ---------------- | ----------------------------- | -------------------------------- |
| `story-store`    | `sinapse-stories`            | `storyOrder` (orden de las columnas) |
| `ui-store`       | `sinapse-ui`                 | `sidebarCollapsed`, `activeView` |
| `projects-store` | `sinapse-projects`           | `projects`, `activeProjectId`    |
| `settings-store` | `sinapse-dashboard-settings` | Todo el objeto `settings`         |
...
```typescript
// Retorna estado actual de SINAPSE
// Lee de: .sinapse/dashboard/status.json

interface SinapseStatus {
  version: string;
  updatedAt: string;
  connected: boolean;
...
```
┌─────────────┐                              ┌──────────────────┐
│   CLI/SINAPSE  │                              │    Dashboard     │
│   (Claude)  │                              │    (Next.js)     │
└──────┬──────┘                              └────────┬─────────┘
       │                                              │
       │  1. Agente activado                         │
       │  ─────────────────────────────────────────▶  │
       │     Escribe .sinapse/dashboard/status.json      │
       │                                              │
       │                                              │ 2. Dashboard detecta
       │                                              │    (SSE poll 2s)
       │                                              │
...
const { status, isLoading, isConnected, statusError, mutate } = useSinapseStatus({
