...
# Guia de Usuario SINAPSE

> **ES** | [EN](../../guides/user-guide.md)

---

Guia completa para usar SINAPSE - el Sistema Orquestado por IA para Desarrollo Full Stack.
...
Antes de usar SINAPSE, asegurate de tener:
...
SINAPSE proporciona estructura orquestada mientras permite flexibilidad en la comunicacion. Esto significa:
...
### La Diferencia SINAPSE

| Desarrollo IA Tradicional       | SINAPSE                                       |
| ------------------------------- | ------------------------------------------ |
...
SINAPSE incluye 11 agentes especializados, cada uno con un rol y personalidad distintos:
...
| **Pax**   | `@sinapse-master`   | Orquestador  | Orquestacion del framework |
...
@sinapse-master        # Activar Pax (Orquestador)
...
Las Tasks son el punto de entrada principal en SINAPSE. Todo es una task.
...
| `brownfield-integration` | Agregar SINAPSE a existente       | dev, architect       |
...
Los Squads son equipos modulares de agentes IA que extienden la funcionalidad de SINAPSE.
...
```
my-project/
├── .sinapse-ai/                # Configuracion del framework
...
La configuracion principal esta en `.sinapse-ai/core/config/`:
...
SINAPSE soporta multiples IDEs. La configuracion se sincroniza a traves de:
...
SINAPSE implementa quality gates de 3 capas:
...
_Guia de Usuario SINAPSE v4.0_
