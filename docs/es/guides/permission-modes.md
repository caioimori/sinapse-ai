...
> Controla cuanta autonomia tienen los agentes SINAPSE sobre tu sistema.

---

## Vision General

Los Modos de Permisos te permiten controlar el nivel de autonomia que tienen los agentes SINAPSE. Ya sea que estes explorando un nuevo codebase o ejecutando builds completamente autonomos, hay un modo para tu flujo de trabajo.
...
El modo se persiste en `.sinapse/config.yaml`:
...
```yaml
# .github/workflows/sinapse.yml
- name: Run SINAPSE
  run: |
    echo "permissions:\n  mode: auto" > .sinapse/config.yaml
    sinapse run build
```
...
Verificar que `.sinapse/config.yaml` existe y es escribible:

```bash
ls -la .sinapse/config.yaml
```
