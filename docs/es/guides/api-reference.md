# Referencia de API SINAPSE

> [EN](../../guides/api-reference.md) | [PT](../../pt/guides/api-reference.md) | **ES**

---

Referencia completa de la API para SINAPSE - el Sistema Orquestado por IA para Desarrollo Full Stack.
...
SINAPSE proporciona una API unificada para interactuar con agentes de IA especializados a través de dos mecanismos principales:
...
| `@sinapse-master`   | Orion  | Orchestrator | Orquestación del framework, meta-operaciones   |

### Comportamiento de Activación

Cuando se activa un agente:

1. Se carga el archivo de definición del agente desde `.sinapse-ai/development/agents/{id}.md`
...
### @sinapse-master (Orquestador)

**Desarrollo del Framework:**

| Comando                | Argumentos      | Descripción                                    |
| ---------------------- | --------------- | ---------------------------------------------- |
| `*create`              | `{type} {name}` | Crear componente SINAPSE (agent/task/workflow)    |
...
| `*kb`   | -                          | Alternar modo KB (conocimiento SINAPSE Method)   |
...
| -------------------- | ---------------------------------- | -------------------------------------- |
| `AGENT_NOT_FOUND`    | Definición de agente faltante      | Verificar `.sinapse-ai/development/agents/` |
...
/sinapse-master  → Activa agente @sinapse-master
```
...
- [Guía de Usuario](./user-guide.md) - Comenzando con SINAPSE
...
- [Guía de Squads](./squads-guide.md) - Extendiendo SINAPSE con squads

---

_SINAPSE API Reference v4.2.11_
