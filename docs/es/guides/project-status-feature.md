...
La funcionalidad de Estado del Proyecto muestra automáticamente tu contexto de trabajo actual cuando cualquier agente SINAPSE se activa. Esto incluye:
...
3. Crea el archivo de caché `.sinapse/project-status.yaml`
...
1. Editar `.sinapse-ai/core-config.yaml`:
   ```yaml
   projectStatus:
     enabled: true
...
Ubicación: `.sinapse-ai/core-config.yaml`
...
projectStatus:
  ...
  statusFile: .sinapse/project-status.yaml  # Ubicación del archivo de caché
...
1. **Verifica caché** - Busca `.sinapse/project-status.yaml`
...
- **Ubicación del Caché:** `.sinapse/project-status.yaml`
...
Los 11 agentes SINAPSE muestran estado del proyecto:
...
3. ¿Existe `.sinapse-ai/infrastructure/scripts/project-status-loader.js`?
...
R: Sí, todos los agentes usan el mismo archivo de caché (`.sinapse/project-status.yaml`).
...
- **Config:** `.sinapse-ai/core-config.yaml` (sección projectStatus)
