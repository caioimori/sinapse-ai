...
Sincroniza agentes, tasks, workflows y checklists de SINAPSE a traves de multiples configuraciones de IDE.

## Vision General

La task `*command` automatiza la sincronizacion de componentes SINAPSE a todos los directorios de IDE configurados (`.claude/`, `.cursor/`, `.gemini/`, etc.), eliminando operaciones de copia manuales.
...
```bash
cp .sinapse-ai/infrastructure/templates/sinapse-sync.yaml.template .sinapse-sync.yaml
```

### 2. Configurar IDEs

Editar `.sinapse-sync.yaml` para habilitar tus IDEs:
...
│                 *command sync                        │
│                                                      │
│  1. Leer configuracion .sinapse-sync.yaml               │
...
```
tu-proyecto/
├── .sinapse-sync.yaml           # Configuracion de sincronizacion
...
**Solucion**: Agregar el alias a `.sinapse-sync.yaml`:
...
- [Arquitectura SINAPSE](../core-architecture.md)
