...
Guía para integrar SINAPSE con IDEs compatibles y plataformas de desarrollo con IA.
...
SINAPSE es compatible con 6 plataformas de desarrollo potenciadas por IA. Elige la que mejor se adapte a tu flujo de trabajo.
...
| IDE | Paridad de Hooks vs Claude | Qué se degrada sin hooks completos | Mitigación en SINAPSE |
| --- | --- | --- | --- |
| Claude Code | Completa | Ninguno (comportamiento de referencia) | Hooks nativos + pipeline completo de SINAPSE |
...
**Nivel de Recomendación:** Mejor integración con SINAPSE
...
1. SINAPSE crea automáticamente el directorio `.claude/` al inicializar
...
4. Usa `/skills` y selecciona `sinapse-<agent-id>`
...
1. SINAPSE crea el directorio `.cursor/` al inicializar
...
2. SINAPSE crea `.github/copilot-instructions.md`
...
1. SINAPSE crea el directorio `.antigravity/`
...
SINAPSE mantiene una única fuente de verdad para las definiciones de agentes y las sincroniza con todos los IDEs configurados:

```
┌─────────────────────────────────────────────────────┐
│                    SINAPSE Core                         │
│  .sinapse-ai/development/agents/  (Fuente de Verdad) │
│                        │                             │
│            ┌───────────┼───────────┐                │
│            ▼           ▼           ▼                │
│  .claude/     .codex/      .cursor/                  │
...
SINAPSE puede configurarse para sincronizar automáticamente cuando hay cambios en los agentes:
...
├── Sí --> Claude Code (Mejor integración con SINAPSE)
└── No
    └── ¿Prefieres VS Code?
...
_Guía de Integración con IDEs de SINAPSE v4.0_
