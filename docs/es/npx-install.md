...
SINAPSE puede instalarse via NPX para una configuración rápida sin instalación global. Esta guía cubre el uso correcto y la resolución de problemas para instalaciones basadas en NPX.

## Inicio Rápido

### Uso Correcto

Siempre ejecute `npx sinapse-ai install` **desde el directorio de su proyecto**:
...
NPX ejecuta paquetes en **directorios temporales** (por ejemplo, `/private/var/folders/.../npx-xxx/` en macOS). Cuando SINAPSE se ejecuta desde estas ubicaciones temporales, no puede:
...
A partir de la versión 4.31.1, SINAPSE detecta automáticamente cuando está ejecutándose desde un directorio temporal NPX y muestra un mensaje de error útil:
...
SINAPSE detecta estos patrones y previene instalaciones incorrectas.
...
SINAPSE v4.31.1+ implementa detección de dos capas:

1. **Capa PRIMARIA** (`tools/sinapse-npx-wrapper.js`):
...
2. **Capa SECUNDARIA** (`tools/installer/bin/sinapse.js`):
