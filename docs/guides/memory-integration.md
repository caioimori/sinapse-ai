...
The integration follows the SINAPSE Open Core model:
- **sinapse-ai:** Extension point in UnifiedActivationPipeline (this guide)
- **sinapse-pro:** Memory intelligence implementation (retrieval, scoring, learning)
...
```javascript
isProAvailable() === true
// but .sinapse/session-digests/ is empty
```
...
```javascript
isProAvailable() === true
// and .sinapse/session-digests/ contains memory digests
```
...
const fs = require('fs');
const digests = fs.readdirSync('.sinapse/session-digests');
...
**Solutions:**
1. Reduce digest count (archive old digests to `.sinapse/session-digests/archive/`)
