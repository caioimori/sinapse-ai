...
Complete reference guide for all SINAPSE meta-agent commands.
...
```bash
SINAPSE_AI_PROVIDER=openai          # AI provider
SINAPSE_AI_MODEL=gpt-4             # AI model
SINAPSE_LOG_LEVEL=debug            # Log level
SINAPSE_TELEMETRY=disabled         # Telemetry setting
SINAPSE_TIMEOUT=30000             # Command timeout (ms)
SINAPSE_MEMORY_CACHE=true         # Memory caching
```
