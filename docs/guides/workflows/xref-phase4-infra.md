...
**Scope:** `.sinapse-ai/infrastructure/` -- scripts, integrations, tools, templates, tests
...
These scripts are imported in `.sinapse-ai/infrastructure/index.js` and exposed as the module's public API.
...
| 7 | `scripts/status-mapper.js` | PM Integration | Maps story status between SINAPSE and PM tools | -- | clickup-helpers, tests/clickup, tasks | No |
...
| 9 | `scripts/template-engine.js` | Template/Generation | Template rendering engine (Handlebars-like) | `fs`, `yaml` | component-generator, template-validator, tasks, sinapse-ai index | No |
| 10 | `scripts/component-generator.js` | Template/Generation | SINAPSE component generation (agents, tasks, templates) | template-engine, yaml-validator, security-checker, transaction-manager | tasks (15+ tasks), sinapse-ai index | No |
| 11 | `scripts/component-metadata.js` | Template/Generation | Metadata management for SINAPSE components | `fs`, `yaml` | component-generator, transaction-manager, tasks/modify-agent | No |
| 12 | `scripts/component-search.js` | Template/Generation | Search across SINAPSE components | `fs` | sinapse-ai index, tasks (analyze-framework, qa-generate-tests) | No |
| 13 | `scripts/batch-creator.js` | Template/Generation | Batch creation of SINAPSE components | component-generator, dependency-analyzer, transaction-manager | sinapse-ai index, tasks/cleanup-utilities | No |
| 14 | `scripts/sinapse-validator.js` | Validation | Validates SINAPSE component structure and schema | `fs`, `yaml` | CI (ci.yml), installer tests, README, git-workflow-guide | No |
...
| 18 | `scripts/dependency-analyzer.js` | Analysis | Analyzes component dependencies | `fs` | batch-creator, modification-validator, sinapse-ai index, tasks | No |
...
| 42 | `scripts/usage-analytics.js` | Utilities | Analytics for SINAPSE feature usage | -- | tasks/analyze-framework | No |
...
| 48 | `scripts/repository-detector.js` | System | Detects repository type and structure | -- | sinapse-init.js, agent devops, tasks | No |
...
| 2 | `scripts/asset-inventory.js` | Analysis (Epic 2) | Creates comprehensive inventory of SINAPSE assets | `fs`, `yaml` | agent devops, many docs, install-manifest | No |
...
| # | File | Category | Purpose | Orphan? |
|---|------|----------|---------|---------|
| 1 | `templates/gitignore/gitignore-sinapse-base.tmpl` | Gitignore | Base SINAPSE gitignore template | No |
...
| 2 | `templates/sinapse-sync.yaml.template` | IDE Sync | SINAPSE sync configuration template | No |
...
| **Validation** | 6 | SINAPSE validator, template validator, output pattern, spot check, agent validator, yaml validator |
...
The infrastructure module is the **base layer** of the SINAPSE modular architecture:
...
- `infrastructure/` has **NO** dependencies on other SINAPSE modules
...
*Generated as part of SINAPSE cross-reference analysis, Phase 4.*
