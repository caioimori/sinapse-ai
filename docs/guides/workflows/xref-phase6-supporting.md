# SINAPSE Cross-Reference Phase 6: Supporting Systems Analysis

> **Generated:** 2026-02-05
> **Scope:** All supporting systems in `.sinapse-ai/`
...
**Location:** `.sinapse-ai/workflow-intelligence/`
**Total Files:** 19
**Purpose:** Provides intelligent workflow analysis, suggestions, pattern learning, and confidence scoring for the SINAPSE agent orchestration system.
...
| `registry/workflow-registry.js` | Central registry of all available workflows | `index.js`, `suggestion-engine.js` | Loads from `.sinapse-ai/development/workflows/` | No |
...
**Location:** `.sinapse-ai/core/quality-gates/` (10 files) and `.sinapse-ai/quality/` (4 files)
**Total Files:** 14
...
| File | Purpose | Consumers | Cross-refs | Orphan? |
|------|---------|-----------|------------|---------|
| `metrics-collector.js` | Collects and aggregates quality metrics across layers | `metrics-hook.js`, CLI metrics commands | Stores to `.sinapse/data/quality-metrics.json` | No |
...
| `seed-metrics.js` | Seeds initial demo metrics data | CLI `sinapse metrics seed` command | Uses `MetricsCollector` | No |
...
**Location:** `.sinapse-ai/cli/commands/`
**Total Files:** 37 (across 8 command groups)
**Purpose:** Commander.js-based CLI commands for the `sinapse` executable. Provides the CLI-first interface for all framework operations.

### Architecture

Commands are organized by domain into subdirectories, each with an `index.js` entry point. Registered in the main CLI (`cli/index.js`) and invoked via `bin/sinapse.js`.
...
| File | Purpose | Consumers | Cross-refs | Orphan? |
|------|---------|-----------|------------|---------|
| `config/index.js` | Config hierarchy management: show, diff, migrate, validate, init-local | `cli/index.js`, `bin/sinapse.js` | Uses `core/config/config-resolver.js`, `merge-utils.js`, `env-interpolator.js` | No |
...
| File | Purpose | Consumers | Cross-refs | Orphan? |
|------|---------|-----------|------------|---------|
| `validate/index.js` | Validate SINAPSE installation integrity | `cli/index.js` | Uses `install-manifest.yaml` | No |

### External References (20+ files reference CLI commands)

- **Main CLI:** `cli/index.js` registers all command groups
- **Entry point:** `bin/sinapse.js` creates the Commander program
...
**Location:** `.sinapse-ai/agent-teams/`
**Total Files:** 0 (directory does not exist or is empty)
**Purpose:** Was intended to hold agent team configurations.

### Status: DOES NOT EXIST

The `agent-teams/` directory was not found in the `.sinapse-ai/` directory. However, references to "agent-teams" exist in:
...
The `tools/` directory exists (referenced in `core-config.yaml` as `toolsLocation: .sinapse-ai/tools`) but contains no files. Tool configurations may have been:
...
- `core-config.yaml`: `toolsLocation: .sinapse-ai/tools`
- `framework-config.yaml`: `tools_dir: ".sinapse-ai/tools"`
...
**Location:** `.sinapse-ai/manifests/`
**Total Files:** 4
**Purpose:** CSV-based registry of all agents, tasks, and workers with JSON schema validation. Acts as the central inventory of all SINAPSE components.
...
**Location:** `.sinapse-ai/core/elicitation/` (5 files) and `.sinapse-ai/elicitation/` (3 files)
**Total Files:** 8
...
| `session-manager.js` | Manages elicitation session persistence (save/load/resume) | `elicitation-engine.js` | Stores sessions in `.sinapse-sessions/` | No |
...
- **Core config:** `core-config.yaml` has `elicitationLocation: .sinapse-ai/elicitation`
- **Framework config:** `framework-config.yaml` has `elicitation_dir: ".sinapse-ai/elicitation"`
...
**Location:** `.sinapse-ai/docs/standards/`
**Total Files:** 16
...
| `SINAPSE-LIVRO-DE-OURO-V2.1-COMPLETE.md` | **PRIMARY** -- Complete v4.0.4 framework guide (consolidated) | All agents, onboarding | Supersedes all legacy Livro docs | No |
...
| `SINAPSE-COLOR-PALETTE-V2.1.md` | Complete color system for SINAPSE branding | Dashboard, UI components | Referenced by UX agent | No |
| `SINAPSE-COLOR-PALETTE-QUICK-REFERENCE.md` | Quick reference for color palette | Developers, designers | Subset of full palette doc | No |
...
| `SINAPSE-LIVRO-DE-OURO.md` | v2.0 base document (DEPRECATED) | Historical reference | Superseded by V2.1-COMPLETE | Yes (Legacy) |
| `SINAPSE-LIVRO-DE-OURO-V2.1.md` | v4.0.4 delta document (DEPRECATED) | Historical reference | Superseded by V2.1-COMPLETE | Yes (Legacy) |
| `SINAPSE-LIVRO-DE-OURO-V2.1-SUMMARY.md` | v4.0.4 summary (DEPRECATED) | Historical reference | Superseded by V2.1-COMPLETE | Yes (Legacy) |
| `SINAPSE-LIVRO-DE-OURO-V2.2-SUMMARY.md` | Future v2.2 planning (DRAFT) | Planning only | Not yet active | No (Draft) |
| `SINAPSE-FRAMEWORK-MASTER.md` | v2.0 framework doc (DEPRECATED) | Historical reference | Superseded by V2.1-COMPLETE | Yes (Legacy) |
...
4 files are explicitly deprecated (superseded by `SINAPSE-LIVRO-DE-OURO-V2.1-COMPLETE.md`), 1 is an archive candidate. These should be cleaned up or moved to an `archive/` directory.

---

## 11. Processes System

**Location:** `.sinapse-ai/processes/`
**Total Files:** 0 (directory does not exist)
**Purpose:** Was intended to hold process definitions.

### Status: DOES NOT EXIST

No `processes/` directory was found under `.sinapse-ai/`. This may be:
...
No references to `.sinapse-ai/processes/` were found in the codebase.

---

## 12. Root Config System

**Location:** `.sinapse-ai/` (root-level files)
**Total Files:** 8
...
| `install-manifest.yaml` | Complete inventory of all files in the framework | Installer, validator, upgrader | Lists every file in `.sinapse-ai/` | No |
| `user-guide.md` | User guide for SINAPSE | Users, onboarding | Referenced by workflows, brownfield guide | No |
...
| `package.json` | npm package definition for `@sinapse-fullstack/core` (v4.31.0) | npm, build system | Defines dependencies, exports, scripts | No |
...
| 13 | Root Config | `.sinapse-ai/` root | 8 | Active | In migration (monolithic to layered) |
...
|------|--------|--------|----------------|
| `docs/standards/SINAPSE-LIVRO-DE-OURO.md` | Docs/Standards | Superseded by V2.1-COMPLETE | Move to archive/ |
| `docs/standards/SINAPSE-LIVRO-DE-OURO-V2.1.md` | Docs/Standards | Superseded by V2.1-COMPLETE | Move to archive/ |
| `docs/standards/SINAPSE-LIVRO-DE-OURO-V2.1-SUMMARY.md` | Docs/Standards | Superseded by V2.1-COMPLETE | Move to archive/ |
| `docs/standards/SINAPSE-FRAMEWORK-MASTER.md` | Docs/Standards | Superseded by V2.1-COMPLETE | Move to archive/ |
...
|-----------|---------------|--------|
| `.sinapse-ai/agent-teams/` | install-manifest, source-tree docs, core-architecture, data/sinapse-kb.md | Directory does not exist |
| `.sinapse-ai/tools/` (content) | core-config.yaml, framework-config.yaml | Directory exists but is empty |
| `.sinapse-ai/processes/` | Not referenced | Does not exist |
...
2. **Manifests are the Registry:** `manifests/workers.csv` contains 204 entries covering all SINAPSE components. The CLI `workers` commands are the primary consumer.
