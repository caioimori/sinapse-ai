...
**Scope:** `.sinapse-ai/core/` -- 130 JavaScript files across 14 subsystems
...
| File | Purpose | Internal Deps | External Consumers | Orphan? |
|------|---------|---------------|-------------------|---------|
| `elicitation-engine.js` | Core interactive elicitation engine (inquirer-based) | session-manager.js; optional: infrastructure/scripts/security-checker | core/index.js, .sinapse-ai/index.js, tests/core/, tests/security/ | No |
...
| `checks/project/` | 8 checks: package-json, node-version, dependencies, sinapse-directory, agent-config, framework-config, task-definitions, workflow-dependencies | project/index.js | base-check.js |
...
| `global-config-manager.js` | Manages ~/.sinapse/mcp/ global config | os-detector.js | config-migrator.js, CLI mcp/add, mcp/status, mcp/setup, mcp/link, tests/ | No |
...
| `skill-dispatcher.js` | Maps agent IDs to SINAPSE Skill invocations | None | workflow-orchestrator.js, orchestration/index.js | No |
...
| `build-registry.js` | Scans SINAPSE modules and builds registry JSON | None (glob) | **None found** (standalone script) | **Partial** -- used as CLI script |
...
| `context-loader.js` | Multi-agent session context loader | context-detector.js | core/index.js, .sinapse-ai/scripts/session-context-loader.js | No |
...
### 4.1 CLI Commands (`.sinapse-ai/cli/commands/`)

| CLI Command | Core Module(s) Used |
|-------------|-------------------|
| `sinapse workers list/info/search-keyword/search-semantic` | `registry/registry-loader` |
| `sinapse manifest validate` | `manifest/manifest-validator` |
| `sinapse manifest regenerate` | `manifest/manifest-generator` |
| `sinapse qa run/status` | `quality-gates/quality-gate-manager` |
| `sinapse mcp add` | `mcp/global-config-manager`, `mcp/os-detector` |
| `sinapse mcp status` | `mcp/global-config-manager`, `mcp/symlink-manager`, `mcp/os-detector`, `mcp/config-migrator` |
| `sinapse mcp setup` | `mcp/global-config-manager`, `mcp/os-detector` |
| `sinapse mcp link` | `mcp/symlink-manager`, `mcp/global-config-manager`, `mcp/os-detector`, `mcp/config-migrator` |
| `sinapse config show/diff/migrate/validate/init-local` | `config/config-resolver`, `config/merge-utils`, `config/env-interpolator` |

### 4.2 Development Scripts (`.sinapse-ai/development/scripts/`)
...
### 4.3 Package Entry Point (`.sinapse-ai/index.js`)

| Entry Point | Core Module(s) Used |
|-------------|-------------------|
| `.sinapse-ai/index.js` (npm package) | `elicitation/elicitation-engine` |
...
| `../../infrastructure/scripts/security-checker` | elicitation/elicitation-engine.js | Exists outside core/ at `.sinapse-ai/infrastructure/scripts/` |
...
| elicitation | (none internally) | core/index, .sinapse-ai/index, development/scripts |
