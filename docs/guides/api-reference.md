# SINAPSE API Reference

> **EN** | [PT](../pt/guides/api-reference.md) | [ES](../es/guides/api-reference.md)

---

Complete API reference for SINAPSE - the AI-Orchestrated System for Full Stack Development.
...
SINAPSE provides a unified API for interacting with specialized AI agents through two primary mechanisms:
...
| `@sinapse-master`   | Orion  | Orchestrator | Framework orchestration, meta-operations |

### Activation Behavior

When an agent is activated:

1. Agent definition file is loaded from `.sinapse-ai/development/agents/{id}.md`
...
| Command                | Arguments       | Description                                 |
| ---------------------- | --------------- | ------------------------------------------- |
| `*create`              | `{type} {name}` | Create SINAPSE component (agent/task/workflow) |
...
| `*kb`   | -                          | Toggle KB mode (SINAPSE Method knowledge) |
...
| -------------------- | ------------------------------- | -------------------------------------- |
| `AGENT_NOT_FOUND`    | Agent definition missing        | Check `.sinapse-ai/development/agents/` |
...
/sinapse-master  → Activates @sinapse-master agent
```
...
- [User Guide](./user-guide.md) - Getting started with SINAPSE
...
- [Squads Guide](./squads-guide.md) - Extending SINAPSE with squads

---

_SINAPSE API Reference v4.0.4_
