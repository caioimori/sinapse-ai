...
SINAPSE v3.12+ introduces a 4-level configuration hierarchy that replaces the single `core-config.yaml` file:
...
| **Pro** Extension | `pro/pro-config.yaml` | Submodule | Premium features (sinapse-pro) |
| **L3** App | `apps/<name>/sinapse-app.config.yaml` | Committed | Per-app overrides |
...
If `${...}` patterns are found in L1 or L2, `sinapse config validate` will warn — these files are committed to git and should not contain environment-specific values.
...
To suppress deprecation warnings: `SINAPSE_SUPPRESS_DEPRECATION=1`
...
sinapse config validate --level L4
```
...
| Command | Description |
|---------|-------------|
| `sinapse config show` | Show fully resolved configuration |
| `sinapse config show --level L2` | Show single level (raw, no merge) |
| `sinapse config show --debug` | Show with source annotations per value |
| `sinapse config diff --levels L1,L2` | Compare two levels |
| `sinapse config migrate` | Migrate monolithic to layered |
| `sinapse config migrate --dry-run` | Preview migration without changes |
| `sinapse config validate` | Validate YAML syntax and patterns |
| `sinapse config init-local` | Create local-config.yaml from template |
...
**Q: How does sinapse-pro config work?**
