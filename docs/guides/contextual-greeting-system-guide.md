...
O Sistema de Greeting Contextual é uma melhoria de UX que torna os greetings dos agentes SINAPSE inteligentes e adaptativos, mostrando informações e comandos relevantes baseados no contexto da sessão.
...
### Scripts Core
- `.sinapse-ai/core/session/context-detector.js` - Detecção de tipo de sessão
- `.sinapse-ai/infrastructure/scripts/git-config-detector.js` - Detecção de git config
- `.sinapse-ai/development/scripts/greeting-builder.js` - Montagem do greeting
- `.sinapse-ai/development/scripts/workflow-navigator.js` - Navegação de workflow
- `.sinapse-ai/development/scripts/agent-exit-hooks.js` - Hooks de saída (para persistência)

### Data Files
- `.sinapse-ai/data/workflow-patterns.yaml` - Definições de workflows
...
### Agents (Updated)
- `.sinapse-ai/agents/dev.md` - ✅ Command visibility metadata
- `.sinapse-ai/agents/po.md` - ✅ Command visibility metadata
...
A: Use o script de teste: `node .sinapse-ai/development/scripts/test-greeting-system.js`
