...
How to migrate legacy squads to SINAPSE 2.1 format.

## Overview

SINAPSE 2.1 introduced a new squad format with:
...
| SINAPSE type field | Missing | `sinapse.type: squad` |
| Min version | Missing | `sinapse.minVersion: "2.1.0"` |
...
**Result:**
- Adds missing `sinapse` fields to manifest
...
- [ ] `sinapse.type` is `"squad"`
- [ ] `sinapse.minVersion` is `"2.1.0"` or higher
