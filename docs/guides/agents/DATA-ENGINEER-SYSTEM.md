...
O agente **@data-engineer (Dara)** e o Database Architect & Operations Engineer do SINAPSE, responsavel pelo design de schemas, migracoes, seguranca (RLS), otimizacao de performance e operacoes de DBA. Este agente atua como um **Sage** que modela dominios de negocio, implementa migracoes seguras com snapshots e rollback, e garante integridade e seguranca dos dados.
...
|---------|---------|-----------|
| `.sinapse-ai/development/tasks/db-domain-modeling.md` | `*model-domain` | Sessao interativa de modelagem de dominio |
| `.sinapse-ai/development/tasks/setup-database.md` | `*setup-database [type]` | Setup de projeto de database (Supabase, PostgreSQL, MongoDB, MySQL, SQLite) |
| `.sinapse-ai/development/tasks/db-bootstrap.md` | `*bootstrap` | Cria estrutura padrao de projeto Supabase |
| `.sinapse-ai/development/tasks/db-env-check.md` | `*env-check` | Valida variaveis de ambiente do database |
| `.sinapse-ai/development/tasks/db-apply-migration.md` | `*apply-migration {path}` | Aplica migracao com snapshot e lock advisory |
| `.sinapse-ai/development/tasks/db-dry-run.md` | `*dry-run {path}` | Testa migracao sem commit |
| `.sinapse-ai/development/tasks/db-seed.md` | `*seed {path}` | Aplica seed data (idempotent) |
| `.sinapse-ai/development/tasks/db-snapshot.md` | `*snapshot {label}` | Cria snapshot do schema |
| `.sinapse-ai/development/tasks/db-rollback.md` | `*rollback {snapshot_or_file}` | Restaura snapshot ou executa rollback |
| `.sinapse-ai/development/tasks/db-smoke-test.md` | `*smoke-test {version}` | Testes abrangentes de database |
| `.sinapse-ai/development/tasks/security-audit.md` | `*security-audit {scope}` | Auditoria de seguranca (rls, schema, full) |
| `.sinapse-ai/development/tasks/analyze-performance.md` | `*analyze-performance {type}` | Analise de performance (query, hotpaths, interactive) |
| `.sinapse-ai/development/tasks/db-policy-apply.md` | `*policy-apply {table} {mode}` | Instala politica RLS (KISS ou granular) |
| `.sinapse-ai/development/tasks/test-as-user.md` | `*test-as-user {user_id}` | Emula usuario para testar RLS |
| `.sinapse-ai/development/tasks/db-verify-order.md` | `*verify-order {path}` | Valida ordenacao DDL para dependencias |
| `.sinapse-ai/development/tasks/db-load-csv.md` | `*load-csv {table} {file}` | Carregador seguro de CSV (staging->merge) |
| `.sinapse-ai/development/tasks/db-run-sql.md` | `*run-sql {file_or_inline}` | Executa SQL raw com transacao |
| `.sinapse-ai/development/tasks/create-deep-research-prompt.md` | `*research {topic}` | Gera prompt de pesquisa profunda |
| `.sinapse-ai/development/tasks/execute-checklist.md` | `*execute-checklist {checklist}` | Executa checklist DBA |
| `.sinapse-ai/development/tasks/create-doc.md` | `*doc-out` | Output de documento completo |
...
| Arquivo | Proposito |
|---------|-----------|
| `.sinapse-ai/development/agents/data-engineer.md` | Definicao core do agente @data-engineer (persona, comandos, workflows) |
...
| Arquivo | Proposito |
|---------|-----------|
| `.sinapse-ai/development/workflows/brownfield-discovery.yaml` | Workflow de discovery brownfield (Fase 2 e 5) |
