# @sinapse-master (Orion) - Execution Trace

> Traced from source code, not documentation.
> Agent definition: `.sinapse-ai/development/agents/sinapse-master.md`
...
|-------|------|--------|---------|
| 1 | `.sinapse-ai/development/agents/sinapse-master.md` | AgentConfigLoader.loadAgentDefinition() | Agent definition (YAML block) |
...
| 5 | `.sinapse/session-state.json` | ContextDetector._detectFromFile() | Session type detection (if no conversation history) |
| 6 | `.sinapse/project-status.yaml` | ProjectStatusLoader.loadCache() | Cached project status (60s TTL) |

**NOT loaded on activation:**

| File | Condition | Size |
|------|-----------|------|
| `.sinapse-ai/data/sinapse-kb.md` | ONLY when user types `*kb` | 35KB |
...
sequenceDiagram
    ...
    participant AMd as sinapse-master.md
    ...
    CC->>ACL: loadAgentDefinition('sinapse-master')
...
sinapse-master:
  config_sections:
    ...
    - registry
  files_loaded:
    - path: .sinapse-ai/data/sinapse-kb.md
      lazy: true
      condition: kb_command
      size: 35KB
  lazy_loading:
    registry: false      # Always load (15KB, frequently used)
    sinapse-kb: true        # Load only on *kb command
...
| Role | `persona.role` | Master Orchestrator, Framework Developer & SINAPSE Method Expert |
...
| `*list-components` | (built-in, scans .sinapse-ai/) | full | No |
...
**Task file:** `.sinapse-ai/development/tasks/kb-mode-interaction.md`

**Dependencies loaded:**
| File | Type | Status |
|------|------|--------|
| `kb-mode-interaction.md` | Task | EXISTS |
| `.sinapse-ai/data/sinapse-kb.md` | Data | EXISTS (lazy, 35KB) |
...
flowchart TD
    ...
    B --> C[Load sinapse-kb.md into context]
    C --> D[Toggle KB mode ON]
    D --> E[Agent now answers with full SINAPSE Method knowledge]
    E --> F[User types *kb again to toggle OFF]
```

**CRITICAL:** sinapse-kb.md is NEVER loaded unless user explicitly types `*kb`.

---

### `*create`

**Task files:** `.sinapse-ai/development/tasks/create-agent.md`, `create-task.md`, `create-workflow.md`
...
| `agent-template.yaml` | Template | EXISTS (in .sinapse-ai/product/templates/) |
| `task-template.md` | Template | EXISTS (in .sinapse-ai/product/templates/) |
| `workflow-template.yaml` | Template | EXISTS (in .sinapse-ai/product/templates/) |
| `security-checker.js` | Util | EXISTS (in .sinapse-ai/infrastructure/scripts/) |
| `yaml-validator.js` | Util | EXISTS (in .sinapse-ai/infrastructure/scripts/) |
...
flowchart TD
    ...
    M --> N{Validation pass?}
    N -->|yes| O[Write component to .sinapse-ai/development/{type}/]
    N -->|no| P[Report validation errors]
    O --> Q[Update manifest if applicable]
```

**Expected output:** New SINAPSE component file written to appropriate directory.

---

### `*modify`

**Task files:** `.sinapse-ai/development/tasks/modify-agent.md`, `modify-task.md`, `modify-workflow.md`
...
**Task file:** `.sinapse-ai/development/tasks/update-manifest.md`
...
**Task file:** `.sinapse-ai/development/tasks/deprecate-component.md`
...
**Task file:** `.sinapse-ai/development/tasks/propose-modification.md`
...
**Task file:** `.sinapse-ai/development/tasks/undo-last.md`
...
**Task file:** `.sinapse-ai/development/tasks/validate-workflow.md`
...
| `yaml-validator.js` | Util | EXISTS |
| `.sinapse-ai/development/workflows/{name}.yaml` | Workflow | Dynamically loaded |
...
**Task files:** `.sinapse-ai/development/tasks/run-workflow.md`, `run-workflow-engine.md`
**Template:** `.sinapse-ai/development/templates/subagent-step-prompt.md`
...
| `subagent-step-prompt.md` | Template | EXISTS (in .sinapse-ai/development/templates/) |
| `workflow-management.md` | Util | EXISTS (in .sinapse-ai/scripts/) |
| `.sinapse-ai/development/workflows/{name}.yaml` | Workflow | Dynamically loaded |
...
**Task file:** `.sinapse-ai/development/tasks/analyze-framework.md`
...
flowchart TD
    ...
    B --> C[Scan .sinapse-ai/ directory structure]
...
**Task file:** `.sinapse-ai/development/tasks/execute-checklist.md`

**Dependencies loaded:**
| File | Type | Status |
|------|------|--------|
| `execute-checklist.md` | Task | EXISTS |
| `.sinapse-ai/product/checklists/{checklist}.md` | Checklist | Dynamically loaded |
...
flowchart TD
    ...
    B --> C{Name provided?}
    C -->|yes| D[Resolve checklist from .sinapse-ai/product/checklists/]
...
**Task file:** `.sinapse-ai/development/tasks/create-doc.md`
**Templates:** All 15 templates from `.sinapse-ai/product/templates/`
...
| `.sinapse-ai/product/data/elicitation-methods.md` | Data | EXISTS |
...
flowchart TD
    ...
    C -->|yes| D[Load template from .sinapse-ai/product/templates/]
...
**Task file:** `.sinapse-ai/development/tasks/shard-doc.md`
...
**Task file:** `.sinapse-ai/development/tasks/document-project.md`
...
**Task file:** `.sinapse-ai/development/tasks/add-tech-doc.md` (MISSING)
...
**Task file:** `.sinapse-ai/development/tasks/create-next-story.md`
...
**Task file:** `.sinapse-ai/development/tasks/advanced-elicitation.md`
...
**Task file:** `.sinapse-ai/development/tasks/correct-course.md`
...
**Task file:** `.sinapse-ai/development/tasks/index-docs.md`
...
| `*guide` | Renders the `## SINAPSE Master Guide` section from agent .md |
...
| `*exit` | Exits sinapse-master mode, returns to base Claude Code |
...
|-------------|-----------|---------|
| @sinapse-master -> @pm | Delegate | Epic/story creation (brownfield-create-epic, brownfield-create-story) |
| @sinapse-master -> @analyst | Delegate | Brainstorming sessions (*brainstorm) |
| @sinapse-master -> @qa | Delegate | Test suite creation (*create-suite) |
| @sinapse-master -> @architect | Delegate | AI prompt generation (*generate-ai-prompt) |
| @sinapse-master -> @dev | Handoff | Story implementation after story creation |
| @sinapse-master -> @devops | Delegate | Git push operations, PR creation |
| @sinapse-master -> any agent | Execute | Can execute any task from any agent directly |
| Any agent -> @sinapse-master | Escalate | Cross-agent coordination, framework operations |
...
|------------|---------------|-------------|
| `security-checker.js` | `.sinapse-ai/development/utils/` | `.sinapse-ai/infrastructure/scripts/security-checker.js` |
| `yaml-validator.js` | `.sinapse-ai/development/utils/` | `.sinapse-ai/infrastructure/scripts/yaml-validator.js` |
| `workflow-management.md` | `.sinapse-ai/development/utils/` | `.sinapse-ai/scripts/workflow-management.md` |
| `subagent-step-prompt.md` | `.sinapse-ai/product/templates/` | `.sinapse-ai/development/templates/subagent-step-prompt.md` |
| `elicitation-methods.md` | `.sinapse-ai/data/` | `.sinapse-ai/product/data/elicitation-methods.md` |
| `brainstorming-techniques.md` | `.sinapse-ai/data/` | `.sinapse-ai/product/data/brainstorming-techniques.md` |
