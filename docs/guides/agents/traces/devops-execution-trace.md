...
> Agent definition: `.sinapse-ai/development/agents/devops.md`
...
|-------|------|--------|---------|
| 1 | `.sinapse-ai/development/agents/devops.md` | AgentConfigLoader.loadAgentDefinition() | Agent definition (YAML block) |
| 2 | `.sinapse-ai/core-config.yaml` | GreetingBuilder._loadConfig() | Core configuration |
| 3 | `.sinapse-ai/data/agent-config-requirements.yaml` | AgentConfigLoader.loadRequirements() | Config sections: dataLocation, cicdLocation |
| 4 | `.sinapse-ai/data/workflow-patterns.yaml` | WorkflowNavigator._loadPatterns() | Workflow state detection |
| 5 | `.sinapse-ai/data/technical-preferences.md` | AgentConfigLoader.loadFile() | Technical preferences (always loaded, 15KB) |
| 6 | `.sinapse/session-state.json` | ContextDetector._detectFromFile() | Session type detection (if no conversation history) |
| 7 | `.sinapse/project-status.yaml` | ProjectStatusLoader.loadCache() | Cached project status (60s TTL) |
...
devops:
  config_sections:
    ...
    - cicdLocation
  files_loaded:
    - path: .sinapse-ai/data/technical-preferences.md
      lazy: false
      size: 15KB
  lazy_loading: {}
...
**Task file:** `.sinapse-ai/development/tasks/github-devops-pre-push-quality-gate.md`

**Dependencies loaded:**
| File | Type | Status |
|------|------|--------|
| `github-devops-pre-push-quality-gate.md` | Task | EXISTS |
| `pre-push-checklist.md` | Checklist | EXISTS (at `.sinapse-ai/product/checklists/`) |
...
|------|------|--------|
| `github-devops-pre-push-quality-gate.md` | Task | EXISTS |
| `repository-detector.js` | Script | EXISTS (at `.sinapse-ai/infrastructure/scripts/`) |
...
**Task file:** `.sinapse-ai/development/tasks/github-devops-github-pr-automation.md`

**Dependencies loaded:**
| File | Type | Status |
|------|------|--------|
| `github-devops-github-pr-automation.md` | Task | EXISTS |
| `github-pr-template.md` | Template | EXISTS (at `.sinapse-ai/product/templates/`) |
...
**Task file:** `.sinapse-ai/development/tasks/github-devops-version-management.md`
...
**Task file:** `.sinapse-ai/development/tasks/release-management.md`

**Dependencies loaded:**
| File | Type | Status |
|------|------|--------|
| `release-management.md` | Task | EXISTS |
| `changelog-template.md` | Template | EXISTS (at `.sinapse-ai/product/templates/`) |
| `release-checklist.md` | Checklist | EXISTS (at `.sinapse-ai/product/checklists/`) |
...
**Task file:** `.sinapse-ai/development/tasks/ci-cd-configuration.md`

**Dependencies loaded:**
| File | Type | Status |
|------|------|--------|
| `ci-cd-configuration.md` | Task | EXISTS |
| `github-actions-ci.yml` | Template | EXISTS (at `.sinapse-ai/product/templates/`) |
| `github-actions-cd.yml` | Template | EXISTS (at `.sinapse-ai/product/templates/`) |
...
**Task file:** `.sinapse-ai/development/tasks/github-devops-repository-cleanup.md`

**Dependencies loaded:**
| File | Type | Status |
|------|------|--------|
| `github-devops-repository-cleanup.md` | Task | EXISTS |
| `branch-manager.js` | Script | EXISTS (at `.sinapse-ai/infrastructure/scripts/`) |
...
**Task file:** `.sinapse-ai/development/tasks/environment-bootstrap.md`
...
**Task file:** `.sinapse-ai/development/tasks/setup-github.md`
...
**Task file:** `.sinapse-ai/development/tasks/check-docs-links.md`
...
**Workflow file:** `.sinapse-ai/development/workflows/auto-worktree.yaml` (EXISTS)
...
**Scripts:** `.sinapse-ai/infrastructure/scripts/asset-inventory.js`, `path-analyzer.js`, `migrate-agent.js`
...
**Task file:** `.sinapse-ai/development/tasks/init-project-status.md`
...
flowchart TD
    ...
    D --> E["Create: .sinapse/project-status.yaml"]
...
|------|------|-------------------|-----------------|--------|
| `github-pr-template.md` | Template | `.sinapse-ai/development/templates/` | `.sinapse-ai/product/templates/` | Path mismatch -- file exists but at different path |
| `github-actions-ci.yml` | Template | `.sinapse-ai/development/templates/` | `.sinapse-ai/product/templates/` | Path mismatch -- file exists but at different path |
| `github-actions-cd.yml` | Template | `.sinapse-ai/development/templates/` | `.sinapse-ai/product/templates/` | Path mismatch -- file exists but at different path |
| `changelog-template.md` | Template | `.sinapse-ai/development/templates/` | `.sinapse-ai/product/templates/` | Path mismatch -- file exists but at different path |
| `pre-push-checklist.md` | Checklist | `.sinapse-ai/development/checklists/` | `.sinapse-ai/product/checklists/` | Path mismatch -- file exists but at different path |
| `release-checklist.md` | Checklist | `.sinapse-ai/development/checklists/` | `.sinapse-ai/product/checklists/` | Path mismatch -- file exists but at different path |
| `gitignore-manager` | Util | `.sinapse-ai/development/utils/` | (not found anywhere) | MISSING -- command may fall back to manual gitignore handling |
| `version-tracker` | Util | `.sinapse-ai/development/utils/` | (not found anywhere) | MISSING -- version tracking relies on git tags and package.json |

**Note on path resolution:** The `dependencies.templates` and `dependencies.checklists` in the agent YAML resolve via `IDE-FILE-RESOLUTION` to `.sinapse-ai/development/{type}/{name}`. However, all 4 templates and 2 checklists actually reside in `.sinapse-ai/product/{type}/`. The 3 infrastructure scripts (`asset-inventory.js`, `path-analyzer.js`, `migrate-agent.js`) and 3 utils (`branch-manager.js`, `repository-detector.js`, `git-wrapper.js`) exist at `.sinapse-ai/infrastructure/scripts/`, not at the declared path.
