...
> Configure global MCP (Model Context Protocol) servers for SINAPSE.
...
The MCP Global System allows you to configure MCP servers once and share them across all SINAPSE projects. This eliminates the need to configure the same servers in every project.
...
- `~/.sinapse/` - Global SINAPSE directory
- `~/.sinapse/mcp/` - MCP configuration directory
- `~/.sinapse/mcp/global-config.json` - Main config file
- `~/.sinapse/mcp/servers/` - Individual server configs
- `~/.sinapse/mcp/cache/` - Response cache
- `~/.sinapse/credentials/` - Secure credential storage
...
SINAPSE includes templates for popular MCP servers:
...
Credentials are stored in `~/.sinapse/credentials/` with a `.gitignore` to prevent accidental commits.
...
| Directory exists  | Use `sinapse mcp setup --force` to recreate                          |
