...
Complete guide for creating, validating, publishing, and managing Squads in SINAPSE.
...
Squads are modular teams of AI agents that extend SINAPSE functionality. Each squad is a self-contained package containing:
...
|-------|---------|-------------|
| [etl-squad](https://github.com/caioimori/sinapse-squads/tree/main/etl) | 2.0.0 | Data collection and transformation |
| [creator-squad](https://github.com/caioimori/sinapse-squads/tree/main/creator) | 1.0.0 | Content generation utilities |
...
- Node.js 18+
- SINAPSE project initialized (`.sinapse-ai/` exists)
...
Tasks must follow [TASK-FORMAT-SPECIFICATION-V1](../../.sinapse-ai/docs/standards/TASK-FORMAT-SPECIFICATION-V1.md).
...
| Mode | Behavior |
|------|----------|
| `extend` | Add squad rules to core SINAPSE rules |
...
```javascript
const { SquadAnalyzer } = require('./.sinapse-ai/development/scripts/squad/squad-analyzer');
const { SquadExtender } = require('./.sinapse-ai/development/scripts/squad/squad-extender');
...
### Level 2: sinapse-squads Repository (Public)
...
This creates a PR to [caioimori/sinapse-squads](https://github.com/caioimori/sinapse-squads).
...
3. **Add Fields** - `sinapse.type`, `sinapse.minVersion`
...
# - sinapse.type: must be "squad"
# - sinapse.minVersion: must be valid semver
```
...
### What's the difference between a Squad and legacy SINAPSE squad formats?

**Squads** are the standard format in SINAPSE 2.1+ with:
- Task-first architecture
...
3. Re-publish: `*publish-squad` or `*sync-squad-sinapse`
...
- **Level 3**: Sync with `--private` flag: `*sync-squad-sinapse my-squad --private`

### What's the minimum SINAPSE version for Squads?

Squads require SINAPSE 2.1.0+. Set in manifest:
