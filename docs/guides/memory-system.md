...
4. [Camada 2: SINAPSE Framework](#camada-2-sinapse-framework)
...
O sistema de memoria do SINAPSE opera em **duas camadas independentes** que coexistem mas **nao se comunicam entre si**:

| Camada | Gerenciado Por | Escopo |
|--------|---------------|--------|
| **Camada 1: Claude Code Nativo** | Claude Code CLI | Auto Memory, CLAUDE.md, Session Transcripts |
| **Camada 2: SINAPSE Framework** | Scripts JS em `.sinapse-ai/` | Gotchas, Session State, Context Snapshots, Timeline |
...
- **Cada camada persiste de forma independente** — `.claude/` vs `.sinapse/`
...
flowchart TB
    subgraph CLAUDE_NATIVE["Camada 1: Claude Code Nativo"]
        ...
        CLAUDE_MD_P["CLAUDE.md Projeto<br/>sinapse-ai/.claude/CLAUDE.md"]
    ...
    subgraph SINAPSE_FRAMEWORK["Camada 2: SINAPSE Framework"]
        direction TB
        GOTCHAS["Gotchas Memory<br/>.sinapse-ai/core/memory/gotchas-memory.js"]
        CTX_SNAP["Context Snapshot<br/>.sinapse-ai/core/memory/context-snapshot.js"]
        FILE_EVO["File Evolution Tracker<br/>.sinapse-ai/core/memory/file-evolution-tracker.js"]
        TIMELINE["Timeline Manager<br/>.sinapse-ai/core/memory/timeline-manager.js"]
        CTX_LOADER["Session Context Loader<br/>.sinapse-ai/core/session/context-loader.js"]
        CTX_DETECT["Context Detector<br/>.sinapse-ai/core/session/context-detector.js"]
        SESS_STATE["Session State<br/>.sinapse-ai/core/orchestration/session-state.js"]
        HOOKS_GEM["Gemini Hooks<br/>.sinapse-ai/hooks/gemini/"]
        HOOKS_UNI["Unified Hooks<br/>.sinapse-ai/hooks/unified/"]
    end
    ...
    subgraph STORAGE_SINAPSE["Storage SINAPSE (.sinapse/)"]
        S_GOTCHAS[".sinapse/gotchas.json + .md"]
        S_ERRORS[".sinapse/error-tracking.json"]
        S_SNAPSHOTS[".sinapse/snapshots/"]
        S_TIMELINE[".sinapse/timeline/"]
        S_FILE_EVO[".sinapse/file-evolution/"]
        S_SESSION[".sinapse/session-state.json"]
    ...
    style SINAPSE_FRAMEWORK fill:#FFF3E0,stroke:#E65100
    style STORAGE_CLAUDE fill:#E8F5E9,stroke:#2E7D32
    style STORAGE_SINAPSE fill:#FCE4EC,stroke:#C62828
```
...
```mermaid
flowchart LR
    subgraph MEMORY_LAYER["Memory Layer (.sinapse-ai/core/memory/)"]
    ...
    subgraph SESSION_LAYER["Session Layer (.sinapse-ai/core/session/)"]
        CL["context-loader.js"]
        CD["context-detector.js"]
    end

    subgraph ORCHESTRATION["Orchestration (.sinapse-ai/core/orchestration/)"]
        SS["session-state.js"]
    end

    subgraph ACTIVATION["Activation Pipeline (.sinapse-ai/development/scripts/)"]
        UAP["unified-activation-pipeline.js"]
        GB["greeting-builder.js"]
    end

    subgraph HOOKS["Hooks System (.sinapse-ai/hooks/)"]
...
flowchart TD
    ...
    subgraph LOAD["Carregamento Automatico — Ordem Fixa"]
        ...
        LOAD_PROJECT["3. sinapse-ai/.claude/CLAUDE.md<br/>Instrucoes nivel projeto"]
        LOAD_RULES["4. sinapse-ai/.claude/rules/*.md<br/>5 arquivos de regras detalhadas"]
...
flowchart LR
    ...
    NOTE["NOTA: Apenas agentes<br/>definidos em .claude/agents/<br/>usam esta memoria.<br/>Agentes SINAPSE (.sinapse-ai/<br/>development/agents/)<br/>NAO usam."]
...
flowchart TD
    subgraph GOTCHAS["gotchas-memory.js — Epic 9, Story 9.4"]
        ...
        G5[("Persiste em:<br/>.sinapse/gotchas.json<br/>.sinapse/gotchas.md<br/>.sinapse/error-tracking.json")]
    end
    ...
    subgraph SNAPSHOTS["context-snapshot.js — Story 12.6"]
        ...
        S5[("Persiste em:<br/>.sinapse/snapshots/{id}.json")]
    end
    ...
    subgraph FILE_EVO["file-evolution-tracker.js"]
        ...
        F4[("Persiste em:<br/>.sinapse/file-evolution/<br/>evolution-index.json")]
    end
    ...
    subgraph TIMELINE_MGR["timeline-manager.js — Facade Unificada"]
        ...
        T4[("Persiste em:<br/>.sinapse/timeline/<br/>unified-timeline.json")]
    end
...
flowchart TD
    subgraph CTX_DETECT["context-detector.js"]
        ...
        CD4["Le .sinapse/session-state.json"]
    ...
    subgraph CTX_LOADER["context-loader.js"]
        ...
        CL8["onTaskComplete()"]
        CL9[("Persiste em:<br/>.sinapse/session-state.json<br/>TTL: 1 hora")]
...
sequenceDiagram
    ...
    Note over CC: NENHUMA memoria SINAPSE<br/>e carregada automaticamente<br/>no system prompt.<br/>Apenas contexto de sessao<br/>aparece no greeting.
```
...
flowchart TD
    ...
    subgraph SAVES["O Que e Salvo"]
        S1[".sinapse/session-state.json<br/>Agente atual, comandos, workflow"]
        S2[".sinapse/error-tracking.json<br/>Ocorrencia do erro"]
        S3[".sinapse/gotchas.json<br/>Nova gotcha auto-capturada"]
        S4[".sinapse/gotchas.json<br/>Gotcha manual adicionada"]
        S5["docs/stories/.session-state.yaml<br/>Phase atualizada"]
        S6["docs/stories/.session-state.yaml<br/>Story marcada done"]
        S7[".sinapse/snapshots/{id}.json<br/>Contexto capturado"]
        S8[".sinapse/file-evolution/<br/>Evolucao registrada"]
    end
...
sequenceDiagram
    ...
    CL->>FS: Le .sinapse/session-state.json
    ...
    CL->>FS: Escreve .sinapse/session-state.json
    FS-->>CL: OK
...
flowchart TD
    subgraph ERROR_FLOW["Fluxo de Auto-Capture"]
        ...
        TRACK --> READ_ET["Le .sinapse/error-tracking.json"]
        ...
        CREATE --> SAVE_JSON["Salva .sinapse/gotchas.json"]
        SAVE_JSON --> SAVE_MD["Gera .sinapse/gotchas.md"]
...
classDiagram
    class GotchasStore {
        +string schema = "sinapse-gotchas-memory-v1"
...
flowchart TD
    subgraph SOURCES["Fontes de Dados"]
        SRC1["FileEvolutionTracker<br/>.sinapse/file-evolution/"]
        SRC2["ContextSnapshot<br/>.sinapse/snapshots/"]
    ...
    subgraph STORAGE["Storage"]
        ST1[(".sinapse/timeline/<br/>unified-timeline.json")]
    end
...
```mermaid
flowchart LR
    subgraph SINAPSE_EVENTS["Eventos SINAPSE"]
        E1["sessionStart"]
        E2["beforeAgent"]
        E3["beforeTool"]
        E4["afterTool"]
...
|---------|--------|------------|--------|
| `.sinapse-ai/core/memory/gotchas-memory.js` | Memory | Epic 9, Story 9.4 | Auto-capture de erros repetidos, gotchas manuais, injecao em tasks |
| `.sinapse-ai/core/memory/context-snapshot.js` | Memory | Story 12.6 | Captura e restaura contexto de desenvolvimento |
| `.sinapse-ai/core/memory/file-evolution-tracker.js` | Memory | Gap impl | Rastreia evolucao de arquivos, detecta drift |
| `.sinapse-ai/core/memory/timeline-manager.js` | Memory | Gap impl | Facade unificada para timeline cross-session |
| `.sinapse-ai/core/session/context-loader.js` | Session | Story 2.2, 6.1.2.5 | Continuidade inter-agente, handoff de contexto |
| `.sinapse-ai/core/session/context-detector.js` | Session | Story 2.2 | Deteccao hibrida de tipo de sessao (new/existing/workflow) |
| `.sinapse-ai/core/orchestration/session-state.js` | Orchestration | Story 11.5 | Estado persistente de epic/story, crash recovery |
| `.sinapse-ai/core/orchestration/context-manager.js` | Orchestration | Legacy | Estado de workflow entre fases (migrado para session-state) |
| `.sinapse-ai/core/elicitation/session-manager.js` | Elicitation | — | Sessions de elicitacao save/load |

### Scripts de Ativacao (consomem memoria)

| Arquivo | Funcao |
|---------|--------|
| `.sinapse-ai/development/scripts/unified-activation-pipeline.js` | Orchestrador principal — carrega sessao no Tier 3 |
| `.sinapse-ai/development/scripts/greeting-builder.js` | Monta greeting com contexto de sessao/memoria |

### Hooks (persistencia de sessao)

| Arquivo | CLI | Funcao |
|---------|-----|--------|
| `.sinapse-ai/hooks/gemini/session-start.js` | Gemini | Carrega contexto SINAPSE no inicio da sessao |
| `.sinapse-ai/hooks/gemini/session-end.js` | Gemini | Persiste sumario da sessao em `.sinapse/sessions/` |
| `.sinapse-ai/hooks/gemini/before-agent.js` | Gemini | Pre-processamento antes de agente |
| `.sinapse-ai/hooks/gemini/before-tool.js` | Ambos | Pre-processamento antes de tool |
| `.sinapse-ai/hooks/gemini/after-tool.js` | Ambos | Pos-processamento apos tool |
| `.sinapse-ai/hooks/unified/hook-interface.js` | Ambos | Classe base UnifiedHook + EVENT_MAPPING |
| `.sinapse-ai/hooks/unified/hook-registry.js` | Ambos | Registro central de hooks |
| `.sinapse-ai/hooks/unified/index.js` | Ambos | Entry point do sistema unificado |
...
| `sinapse-ai/.claude/CLAUDE.md` | Instrucoes projeto (carregado sempre) |
| `sinapse-ai/.claude/rules/*.md` | 5 arquivos de regras (carregados sempre) |
...
```
.sinapse/                                        # Runtime state (gitignored)
...
Os modulos SINAPSE (gotchas, snapshots, timeline) **nao fazem flush** automatico ao final da sessao. Dados em memoria podem se perder se o processo terminar abruptamente.
...
Claude auto-memory (`MEMORY.md`) e SINAPSE memory (`.sinapse/`) nunca se sincronizam. Gotchas capturadas pelo SINAPSE nao aparecem no MEMORY.md e vice-versa.
...
| Gotchas Memory Script | `.sinapse-ai/core/memory/gotchas-memory.js` |
| Context Snapshot Script | `.sinapse-ai/core/memory/context-snapshot.js` |
| File Evolution Tracker | `.sinapse-ai/core/memory/file-evolution-tracker.js` |
| Timeline Manager | `.sinapse-ai/core/memory/timeline-manager.js` |
| Session Context Loader | `.sinapse-ai/core/session/context-loader.js` |
| Context Detector | `.sinapse-ai/core/session/context-detector.js` |
| Session State | `.sinapse-ai/core/orchestration/session-state.js` |
| Unified Hook Interface | `.sinapse-ai/hooks/unified/hook-interface.js` |
| Hook Registry | `.sinapse-ai/hooks/unified/hook-registry.js` |
| Gemini Session Start | `.sinapse-ai/hooks/gemini/session-start.js` |
| Gemini Session End | `.sinapse-ai/hooks/gemini/session-end.js` |
...
*SINAPSE Memory System Architecture Guide v1.0*
