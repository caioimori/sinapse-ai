...
> Comprehensive guide to the testing strategy, tools, and best practices for SINAPSE.
...
SINAPSE follows a comprehensive testing strategy that ensures code quality across all layers of the framework. Our testing philosophy is built on:
 ...
 * Agent Backward Compatibility Tests
 *
 * Ensures agents from previous SINAPSE versions continue to work.
 */
...
SINAPSE supports Windows, macOS, and Linux. Cross-platform testing ensures consistent behavior.
...
│   ├── index.html        # Overview
│   └── .sinapse-ai/       # Per-module coverage
...
The SINAPSE Quality Gate System (see [Quality Gates Guide](./quality-gates.md)) integrates testing at multiple layers:
