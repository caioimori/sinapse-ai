...
Synchronize SINAPSE agents, tasks, workflows, and checklists across multiple IDE configurations.

## Overview

The `*command` task automates the synchronization of SINAPSE components to all configured IDE directories (`.claude/`, `.cursor/`, `.gemini/`, etc.), eliminating manual copy operations.
...
Edit `.sinapse-sync.yaml` to enable your IDEs:
...
│                 *command sync                        │
│                                                      │
│  1. Read .sinapse-sync.yaml configuration               │
...
```
your-project/
├── .sinapse-sync.yaml           # Sync configuration
...
**Solution**: Add the alias to `.sinapse-sync.yaml`:
