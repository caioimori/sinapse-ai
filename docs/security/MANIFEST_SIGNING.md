...
│  │ (sinapse-ai.key)  │    │         -s sinapse-ai.key          │  │
│  │ NEVER SHARE!     │    └───────────────────────────────────┘  │
│  └──────────────────┘                    │                       │
│                                          ▼                       │
│                          ┌───────────────────────────────────┐  │
...
minisign -G -p sinapse-ai.pub -s sinapse-ai.key

# You will be prompted for a password to protect the secret key
# CHOOSE A STRONG PASSWORD!

# Output:
#   sinapse-ai.pub  - PUBLIC key (safe to share, will be hardcoded)
#   sinapse-ai.key  - SECRET key (NEVER share, store securely)
```
...
cat sinapse-ai.pub
# Output example:
# untrusted comment: minisign public key SINAPSE0001
...
const PINNED_PUBLIC_KEY = {
  ...
  keyId: 'SINAPSE0001',
  // Base64 public key (the second line of sinapse-ai.pub)
...
- Store `sinapse-ai.key` in a secure location (password manager, HSM, etc.)
...
1. **Generate/Update Manifest**
   ...
   ```bash
   node bin/sinapse.js manifest:generate
   # Creates .sinapse-ai/install-manifest.yaml with all file hashes
   ```
...
2. **Sign the Manifest**
   ...
   ```bash
   cd .sinapse-ai
   minisign -Sm install-manifest.yaml -s /path/to/sinapse-ai.key
...
3. **Verify Signature (Optional but Recommended)**
   ...
   ```bash
   minisign -Vm install-manifest.yaml -p /path/to/sinapse-ai.pub
...
4. **Commit Both Files**
   ...
   ```bash
   git add .sinapse-ai/install-manifest.yaml
   git add .sinapse-ai/install-manifest.yaml.minisig
...
```bash
minisign -Sm .sinapse-ai/install-manifest.yaml -s /path/to/sinapse-ai.key
```
