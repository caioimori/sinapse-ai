...
1. Generate a key pair: `minisign -G -p sinapse-ai.pub -s sinapse-ai.key`
2. Update `PINNED_PUBLIC_KEY` in `manifest-signature.js` with the real public key
3. Sign the manifest: `minisign -Sm install-manifest.yaml -s sinapse-ai.key`
...
3. **Sign manifest** before each release:
   ...
   ```bash
   minisign -Sm .sinapse-ai/install-manifest.yaml -s sinapse-ai.key
   ```
