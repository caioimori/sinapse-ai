...
> Source: `.sinapse-ai/development/scripts/unified-activation-pipeline.js` (Story ACT-6)
> Previous source: `.sinapse-ai/development/scripts/greeting-builder.js` (949 lines)

## Overview

Every SINAPSE agent goes through a **single unified activation pipeline** before presenting its greeting. As of Story ACT-6, the previous two-path architecture (Path A: direct GreetingBuilder invocation, Path B: generate-greeting.js CLI wrapper) has been consolidated into one entry point.
...
Current source:
- `.sinapse-ai/development/scripts/activation-runtime.js`
- `.sinapse-ai/development/scripts/unified-activation-pipeline.js`
...
sequenceDiagram
    ...
    alt Cache hit
        ...
        ACL->>FS: readFile(.sinapse-ai/development/agents/{id}.md)
...
Reads from `.sinapse-ai/core-config.yaml` path `agentIdentity.greeting.preference`.
...
await mode.load();  // Reads .sinapse/config.yaml -> permissions.mode
...
1. Reads current mode from `.sinapse/config.yaml`
...
```javascript
const { enforcePermission } = require('./.sinapse-ai/core/permissions');
...
The `environment-bootstrap` task initializes `.sinapse/config.yaml` with `permissions.mode: ask` as the default. If the config file is missing or the field is absent, the system defaults to `ask` mode.
...
Each agent has specific config requirements defined in `.sinapse-ai/data/agent-config-requirements.yaml`:
...
| File | Loader | Purpose |
|------|--------|---------|
| `.sinapse-ai/development/agents/{agent-id}.md` | AgentConfigLoader | Agent definition |
| `.sinapse-ai/core-config.yaml` | GreetingBuilder._loadConfig() | Core configuration |
| `.sinapse-ai/data/agent-config-requirements.yaml` | AgentConfigLoader.loadRequirements() | Per-agent config needs |
| `.sinapse-ai/data/workflow-patterns.yaml` | WorkflowNavigator._loadPatterns() | Workflow state detection |

### Loaded conditionally

| File | Condition | Loader |
|------|-----------|--------|
| `.sinapse/session-state.json` | Path B (CLI wrapper) or file-based session detection | ContextDetector / SessionContextLoader |
| `.sinapse/project-status.yaml` | Cache check (60s TTL) | ProjectStatusLoader |
...
graph TD
    ...
    SCL --> SSF[.sinapse/session-state.json]
    PSL --> GIT[git CLI commands]
    PSL --> STORIES[docs/stories/**/*.md]
    PSL --> WTM[WorktreeManager]
    PSL --> PSC[.sinapse/project-status.yaml]
    GCD_R --> GIT
    WN --> WP[.sinapse-ai/data/workflow-patterns.yaml]
    GPM --> CC
```
...
The `user_profile` setting (`bob` or `advanced`) affects behavior across the entire SINAPSE pipeline. This section documents every file that references `user_profile`/`userProfile` and the behavioral difference between modes.
...
|---|------|----------|----------------|---------------------|
| 1 | `.sinapse-ai/core-config.yaml` | Config | `user_profile: bob` | `user_profile: advanced` |
| 2 | `.sinapse-ai/development/scripts/greeting-builder.js` | Greeting | Redirects non-PM agents to @pm; hides role/status sections; returns empty commands for non-PM | Full contextual greeting with all sections and commands |
| 3 | `.sinapse-ai/development/scripts/generate-greeting.js` | Greeting | Uses GreetingBuilder, same bob restrictions | Uses GreetingBuilder, full features |
| 4 | `.sinapse-ai/development/scripts/greeting-preference-manager.js` | Greeting | Forces preference to `minimal` or `named`; overrides `auto`/`archetypal` | All 4 preferences available (`auto`, `minimal`, `named`, `archetypal`) |
| 5 | `.sinapse-ai/infrastructure/scripts/validate-user-profile.js` | Validation | Validates `bob` as legal value; normalizes case | Validates `advanced` as legal value; normalizes case |
| 6 | `.sinapse-ai/core/config/config-resolver.js` | Config | `toggleUserProfile()` switches bob<->advanced; L5 user layer has priority | Same toggle; resolveConfig merges layers |
...
| 16 | `packages/sinapse-install/src/installer.js` | Install | Sets `config.user_profile = 'bob'` in YAML | Sets `config.user_profile = 'advanced'` |
| 17 | `.sinapse-ai/development/tasks/environment-bootstrap.md` | Task | Documents bob selection flow | Documents advanced selection flow |
| 18 | `docs/sinapse-workflows/bob-orchestrator-workflow.md` | Docs | Full bob orchestrator workflow documentation | N/A (bob-specific doc) |
