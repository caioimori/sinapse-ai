...
SINAPSE e um meta-framework open source que organiza **210 agentes de IA em 19 squads especializados**, operando direto no terminal via Claude Code ou Codex CLI. Cada agente tem um papel definido, cada squad domina uma disciplina, e o sistema inteiro e governado por uma **Constitution com enforcement real** — 13 hooks ativos que bloqueiam violacoes em tempo de execucao.

O conceito central e simples: em vez de um unico assistente de IA tentando fazer tudo, o SINAPSE estrutura o trabalho em equipes especializadas. Um squad de branding cuida da identidade visual. Um squad de cybersecurity cuida de compliance e pentest. Um squad de copywriting cuida de persuasao e conversao. Cada um com sua propria knowledge base, workflows e tasks — totalizando **1.450 tasks executaveis** prontas para uso (213 no core + 1.237 distribuidas pelos 19 squads).
...
SINAPSE resolve isso do jeito que times humanos resolvem: **especializacao coordenada**. Em vez de um generalista cansado, voce tem 210 agentes em 19 squads, cada um com papel definido, knowledge base propria e tasks executaveis. Um orquestrador roteia seu pedido para quem realmente sabe resolver — automaticamente, sem voce precisar decorar agent names ou comandos.
...
O wizard detecta seu ambiente, escolhe a IDE (Claude Code ou Codex), e instala os 19 squads automaticamente. Re-rodar o comando faz upsert idempotente — preserva suas customizacoes e so atualiza o que mudou.
...
Pronto. Voce tem 19 squads operando no seu terminal.
...
**Total: 19 squads, 210 agentes especializados, 1.450 tasks** (213 core + 1.237 squads)
...
Ambas as IDEs tem acesso a todos os 19 squads, 210 agentes, workflows e knowledge bases. O installer detecta e configura automaticamente.
...
Um comando. 19 squads. 210 agentes. Governanca constitucional. Tudo operando direto no terminal.
