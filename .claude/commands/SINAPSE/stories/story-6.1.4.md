...
- STEP 3: |
    ...
    1. Execute: node .sinapse-ai/scripts/generate-greeting.js {agent-id}
...
Add to `.sinapse-ai/scripts/agent-config-loader.js`:
...
async loadAgentDefinition(options = {}) {
  ...
  // Load from file
  const agentPath = path.join(process.cwd(), '.sinapse-ai', 'agents', `${this.agentId}.md`);
...
1. Add deprecation warning to `.sinapse-ai/scripts/config-loader.js`:
```javascript
/**
...
2. Check for usages:
```bash
grep -r "require.*config-loader" .sinapse-ai/
grep -r "from.*config-loader" .sinapse-ai/
```
...
1. `.sinapse-ai/scripts/batch-integrate-greeting-builder.js` ✅ (exists - verified)
2. `.sinapse-ai/scripts/apply-inline-greeting-all-agents.js` ❌ (not found - may already be deleted)
3. `.sinapse-ai/scripts/update-activation-instructions.js` ✅ (exists - verified)
4. `.sinapse-ai/scripts/batch-update-agents-session-context.js` ❌ (not found - may already be deleted)
...
1. **Verify script existence:**
   ```bash
   # Check each script
   test -f .sinapse-ai/scripts/batch-integrate-greeting-builder.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .sinapse-ai/scripts/apply-inline-greeting-all-agents.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .sinapse-ai/scripts/update-activation-instructions.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .sinapse-ai/scripts/batch-update-agents-session-context.js && echo "EXISTS" || echo "NOT FOUND"
   ```

2. **Delete existing scripts only:**
   ```bash
   # Delete only if file exists (graceful handling)
   [ -f .sinapse-ai/scripts/batch-integrate-greeting-builder.js ] && rm .sinapse-ai/scripts/batch-integrate-greeting-builder.js
   [ -f .sinapse-ai/scripts/update-activation-instructions.js ] && rm .sinapse-ai/scripts/update-activation-instructions.js
   ```
...
Modify `.sinapse-ai/scripts/greeting-builder.js`:
...
Create `.sinapse-ai/scripts/generate-greeting.js`:
...
async function generateGreeting(agentId) {
  ...
  try {
    // Load core config
    const coreConfigPath = path.join(process.cwd(), '.sinapse-ai', 'core-config.yaml');
...
- STEP 3: |
    ...
    1. Execute: node .sinapse-ai/scripts/generate-greeting.js qa
    2. Capture the complete output
    3. Display the greeting exactly as returned
...
4. Save file
5. Test activation: `/SINAPSE/agents/qa`
...
3. **Direct Script Test:**
   ...
   node .sinapse-ai/scripts/generate-greeting.js qa
...
4. **Error Handling Test:**
   ...
   node .sinapse-ai/scripts/generate-greeting.js invalid-agent
...
5. **Performance Test:**
   ...
   time node .sinapse-ai/scripts/generate-greeting.js qa
...
Create script `.sinapse/temp-update-remaining-agents.js`:
...
Create `.sinapse-ai/scripts/command-execution-hook.js`:
...
Create `.sinapse-ai/docs/session-update-pattern.md`:
...
const { generateGreeting } = require('../../.sinapse-ai/scripts/generate-greeting');
...
- `.sinapse-ai/scripts/generate-greeting.js` (Unified greeting generator)
- `.sinapse-ai/scripts/command-execution-hook.js` (Session update hook)
- `.sinapse-ai/docs/session-update-pattern.md` (Documentation)
...
- `.sinapse-ai/scripts/batch-integrate-greeting-builder.js` (Temporary migration script)
- `.sinapse-ai/scripts/apply-inline-greeting-all-agents.js` (Temporary migration script)
- `.sinapse-ai/scripts/update-activation-instructions.js` (Temporary migration script)
- `.sinapse-ai/scripts/batch-update-agents-session-context.js` (Temporary migration script)

### Files Referenced (No Changes)

- `.sinapse-ai/scripts/session-context-loader.js` (Used by generate-greeting.js)
- `.sinapse-ai/scripts/project-status-loader.js` (Used by generate-greeting.js)
- `.sinapse-ai/scripts/greeting-preference-manager.js` (Used by greeting-builder.js)
- `.sinapse-ai/scripts/context-detector.js` (Used by greeting-builder.js)
- `.sinapse-ai/scripts/workflow-navigator.js` (Used by greeting-builder.js)
