...
4. **Caio Imori approves** → finding copied to `audits/promoted/`
5. **If rejected or scope-bound** → finding moved to `audits/archived/` (or left in project)

## What belongs here

- **`promoted/`** — AuditFindings where the FrameworkProposal was APPROVED by Caio Imori and shipped as a sinapse-ai PR. These are the institutional memory of what real production usage taught the framework.
...
- **Any agent or Caio Imori** can write an AuditFinding (in the project's `docs/audits/`)
- **@sinapse-orqx** decides if it's a framework-candidate and drafts the proposal
- **Caio Imori** is the sole approver for promotion to `promoted/`
