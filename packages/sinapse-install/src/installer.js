/**
 * SINAPSE Installer - Main installation logic
 *
 * Orchestrates the complete SINAPSE installation flow:
 * 1. OS detection
...
/**
 * Check if a config-resolver is available (sinapse-ai installed)
 * @param {string} projectRoot - Project root
 * @returns {Object|null} Config resolver module or null
 */
function tryLoadConfigResolver(projectRoot) {
  const possiblePaths = [
    path.join(projectRoot, '.sinapse-ai/core/config/config-resolver.js'),
    path.join(projectRoot, 'node_modules/sinapse-ai/.sinapse-ai/core/config/config-resolver.js'),
  ];
...
async function createUserConfigDirect(profile, logger, dryRun) {
  const userConfigDir = path.join(os.homedir(), '.sinapse');
...
/**
 * Check if this is a brownfield installation (existing SINAPSE)
 * @param {string} projectRoot - Project root
...
function detectBrownfield(projectRoot) {
  ...
  const legacyConfigPath = path.join(projectRoot, '.sinapse-ai/core-config.yaml');
  ...
  const frameworkConfigPath = path.join(projectRoot, '.sinapse-ai/framework-config.yaml');
  if (fs.existsSync(frameworkConfigPath)) {
    result.isBrownfield = true;
    result.hasLayeredConfig = true;
  }

  // Check for .sinapse-ai directory
...
/**
 * Run the SINAPSE doctor command
 * @param {string} projectRoot - Project root
 * @param {InstallLogger} logger - Logger instance
 * @param {boolean} dryRun - Whether this is a dry run
 */
async function runDoctor(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npx sinapse-ai doctor');
    return;
  }

  const spinner = ora('Running SINAPSE doctor...').start();

  try {
    const { stdout } = await execa('npx', ['sinapse-ai', 'doctor'], {
      cwd: projectRoot,
      timeout: 60000,
    });

    spinner.succeed('SINAPSE doctor completed');

    if (logger.verbose) {
      console.log(chalk.dim(stdout));
    }
  } catch (error) {
    spinner.warn('SINAPSE doctor had warnings');
    if (logger.verbose) {
...
/**
 * Install sinapse-ai package
 * @param {string} projectRoot - Project root
 * @param {InstallLogger} logger - Logger instance
 * @param {boolean} dryRun - Whether this is a dry run
 */
async function installSinapseCore(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npm install sinapse-ai --save-dev');
    return;
  }

  const spinner = ora('Installing sinapse-ai...').start();

  try {
    await execa('npm', ['install', 'sinapse-ai', '--save-dev'], {
      cwd: projectRoot,
      timeout: 300000, // 5 minutes
    });

    spinner.succeed('sinapse-ai installed');
  } catch (error) {
    spinner.fail('Failed to install sinapse-ai');
    throw error;
  }
}

/**
 * Initialize SINAPSE in the project
 * @param {string} projectRoot - Project root
 * @param {InstallLogger} logger - Logger instance
 * @param {boolean} dryRun - Whether this is a dry run
 */
async function initializeSinapse(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npx sinapse-ai install');
    return;
  }

  const spinner = ora('Initializing SINAPSE...').start();

  try {
    await execa('npx', ['sinapse-ai', 'install'], {
      cwd: projectRoot,
      timeout: 120000, // 2 minutes
    });

    spinner.succeed('SINAPSE initialized');
  } catch (error) {
    spinner.fail('Failed to initialize SINAPSE');
    throw error;
  }
}
...
async function runInstaller(options = {}) {
  ...
  // Introduction
  intro(chalk.bgCyan(' SINAPSE Installer '));
  ...
  if (!profile) {
    ...
    const profileSelection = await select({
      message: 'Select your SINAPSE profile:',
      options: [
        {
          value: 'bob',
  ...
  if (brownfield.isBrownfield) {
    logger.info('Existing SINAPSE installation detected');
    ...
    if (brownfield.hasLegacyConfig && !brownfield.hasLayeredConfig) {
      ...
      if (shouldMigrate && !options.dryRun) {
        ...
        try {
          await execa('npx', ['sinapse-ai', 'config', 'migrate'], {
            cwd: projectRoot,
            timeout: 60000,
          });
        ...
        logger.action('Run: npx sinapse-ai config migrate');
      }
    } else {
    ...
    // Install sinapse-ai
    await installSinapseCore(projectRoot, logger, options.dryRun);
  ...
  if (options.dryRun) {
    ...
    outro(chalk.green(`SINAPSE installed successfully in ${elapsed}!`));
    console.log('');
