...
class PostInstallValidator {
   ...
   *
   * @param {string} targetDir - Directory where SINAPSE was installed (project root)
  ...
  constructor(targetDir, sourceDir = null, options = {}) {
    ...
    this.sinapseCoreTarget = path.join(this.targetDir, '.sinapse-ai');
    this.sinapseCoreSource = this.sourceDir ? path.join(this.sourceDir, '.sinapse-ai') : null;
  ...
  _getRealTargetDir() {
    if (this._realTargetDirCache === null) {
      try {
        this._realTargetDirCache = fs.realpathSync(this.sinapseCoreTarget);
      } catch {
  ...
  async loadManifest() {
    ...
    const sourceManifestPath = this.sinapseCoreSource
      ? path.join(this.sinapseCoreSource, 'install-manifest.yaml')
      : null;
    const targetManifestPath = path.join(this.sinapseCoreTarget, 'install-manifest.yaml');
  ...
  async validateFile(entry) {
    ...
    const absolutePath = path.resolve(this.sinapseCoreTarget, relativePath);
    ...
    if (!isPathContained(absolutePath, this.sinapseCoreTarget)) {
      ...
      if (error.code === 'ENOENT') {
        result.issue = {
          ...
          remediation: this.sourceDir
            ? "Run 'sinapse validate --repair' to restore"
            : 'Re-run installation',
          relativePath,
        };
    ...
    // allowing path traversal attacks (e.g., /install/.sinapse-ai/symlinked-dir/../../../etc/passwd)
    //
    ...
    try {
      ...
      const expectedRelative = path.relative(this.sinapseCoreTarget, absolutePath);
  ...
  async detectExtraFiles() {
    ...
    await scanDir(this.sinapseCoreTarget, this.sinapseCoreTarget);
    return extraFiles;
  }
  ...
  async validate() {
    ...
    if (!fs.existsSync(this.sinapseCoreTarget)) {
      this.issues.push({
        type: IssueType.MISSING_FILE,
        severity: Severity.CRITICAL,
  ...
  generateRecommendations() {
    ...
    if (this.stats.missingFiles > 0) {
      if (this.stats.missingFiles > 50) {
        ...
        recommendations.push(
          `${this.stats.missingFiles} file(s) missing. Run 'sinapse validate --repair'.`,
        );
      }
    }
    ...
    if (this.stats.corruptedFiles > 0) {
      recommendations.push(
        `${this.stats.corruptedFiles} file(s) corrupted. Run 'sinapse validate --repair'.`,
      );
    }
  ...
  async repair(options = {}) {
    ...
    if (!this.sourceDir || !fs.existsSync(this.sinapseCoreSource)) {
      return {
        success: false,
    ...
    for (let i = 0; i < repairableIssues.length; i++) {
      ...
      const sourcePath = path.resolve(this.sinapseCoreSource, relativePath);
      const targetPath = path.resolve(this.sinapseCoreTarget, relativePath);

      onProgress(i + 1, repairableIssues.length, relativePath);

      // SECURITY: Path containment for source
      if (!isPathContained(sourcePath, this.sinapseCoreSource)) {
        result.skipped.push({ path: relativePath, reason: 'Source path traversal blocked' });
        continue;
      }

      // SECURITY: Path containment for target
      if (!isPathContained(targetPath, this.sinapseCoreTarget)) {
      ...
      try {
        ...
        // Walk each path component from sinapseCoreTarget to targetDir
        // and verify none are symlinks
        let currentPath = this.sinapseCoreTarget;
        const relativeParts = path.relative(this.sinapseCoreTarget, targetDir).split(path.sep);
        ...
        for (const part of relativeParts) {
          ...
          try {
            ...
            if (componentStat.isSymbolicLink()) {
              result.skipped.push({
                path: relativePath,
                reason: `Symlink detected in path component: ${path.relative(this.sinapseCoreTarget, currentPath)}`,
              });
              continue;
            }
          } catch (_statError) {
        ...
        // Final realpath verification: ensure resolved target stays within resolved sinapseCoreTarget
        ...
        if (fs.existsSync(targetDir)) {
          ...
          const realSinapseCoreTarget = fs.realpathSync(this.sinapseCoreTarget);

          if (!isPathContained(realTargetDir, realSinapseCoreTarget)) {
            result.skipped.push({
              path: relativePath,
              reason: `Realpath escapes target directory: ${realTargetDir} is outside ${realSinapseCoreTarget}`,
            });
            continue;
          }
        }
