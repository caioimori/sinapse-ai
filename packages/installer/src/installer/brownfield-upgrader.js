...
function loadInstalledManifest(targetDir) {
  return loadManifest(
    path.join(targetDir, '.sinapse-ai'),
 ...
 * @param {string} sourceDir - Source package directory (.sinapse-ai from npm)
...
function generateUpgradeReport(sourceManifest, installedManifest, targetDir) {
  ...
  for (const [filePath, sourceEntry] of sourceMap) {
    ...
    const absolutePath = path.join(sinapseCoreDir, filePath);
 ...
 * @param {string} sourceDir - Source directory (npm package .sinapse-ai)
 * @param {string} targetDir - Target project directory
...
async function applyUpgrade(report, sourceDir, targetDir, options = {}) {
  ...
  // Ensure .sinapse-ai directory exists
  if (!dryRun) {
  ...
  for (const file of report.newFiles) {
    ...
    const targetPath = path.join(sinapseCoreDir, file.path);
...
function updateInstalledManifest(targetDir, sourceManifest, sourcePackage) {
  const installedManifestPath = path.join(targetDir, '.sinapse-ai', '.installed-manifest.yaml');
