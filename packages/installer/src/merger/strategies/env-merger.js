...
class EnvMerger extends BaseMerger {
  ...
  async merge(existingContent, newContent, _options = {}) {
    ...
    // Add SINAPSE section if there are new vars or suggestions
    if (newVars.length > 0 || suggestions.length > 0) {
      const date = new Date().toISOString().split('T')[0];
      merged += `\n# === SINAPSE Variables (added ${date}) ===\n`;

      // Add new variables
      for (const { key, value, comment } of newVars) {
        if (comment && !comment.includes('SINAPSE')) {
          // Add original comment if it's not an SINAPSE header
          merged += `${comment}\n`;
        }
        merged += `${key}=${value}\n`;
      }
      ...
      if (suggestions.length > 0) {
        merged += '\n# SINAPSE_NOTE: The following variables already exist with different values:\n';
        for (const { key, suggestedValue } of suggestions) {
          merged += `# SINAPSE_SUGGESTED: ${key}=${suggestedValue}\n`;
        }
      }
    }
