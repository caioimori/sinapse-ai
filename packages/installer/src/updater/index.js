...
/**
 * SINAPSE Updater Class
 * Handles intelligent updates while preserving user customizations
 */
class SINAPSEUpdater {
  /**
   * Create a new SINAPSEUpdater instance
   *
  ...
  constructor(projectRoot, options = {}) {
    ...
    this.sinapseCoreDir = path.join(this.projectRoot, '.sinapse-ai');
    this.sinapseConfigDir = path.join(this.projectRoot, '.sinapse');
  ...
  async checkForUpdates() {
    ...
    try {
      ...
      if (!result.installed) {
        result.error = 'SINAPSE not installed or version info not found';
        return result;
      }
      ...
      if (!result.latest) {
        ...
        if (!isOnline) {
          ...
          result.error = 'Package sinapse-ai not found on npm registry. This may be a local development installation.';
        }
        return result;
      }
  ...
  async getInstalledVersion() {
    ...
    const versionJsonPath = path.join(this.sinapseCoreDir, 'version.json');
    ...
    const packageJsonPath = path.join(this.projectRoot, 'node_modules', 'sinapse-ai', 'package.json');
  ...
  async getLatestVersion() {
    return new Promise((resolve) => {
      const request = https.get(
        'https://registry.npmjs.org/sinapse-ai/latest',
  ...
  async detectCustomizations() {
    ...
    for (const [relativePath, originalHash] of Object.entries(this.versionInfo.fileHashes)) {
      const absolutePath = path.join(this.sinapseCoreDir, relativePath);
   ...
   */
  async createBackup() {
    this.backupDir = path.join(this.sinapseConfigDir, 'backup', `pre-update-${Date.now()}`);
    ...
    for (const file of filesToBackup) {
      const src = path.join(this.sinapseCoreDir, file);
  ...
  async rollback() {
    ...
    for (const file of backupFiles) {
      ...
      const dest = path.join(this.sinapseCoreDir, file);
...
function formatCheckResult(result, options = {}) {
  ...
  lines.push('');
  lines.push(`${c.bold}🔍 SINAPSE Update Check${c.reset}`);
  lines.push('');
  ...
  switch (result.status) {
    ...
    case UpdateStatus.UPDATE_AVAILABLE:
      ...
      lines.push(`  Run ${c.cyan}npx sinapse-ai update${c.reset} to update.`);
      break;
...
function formatUpdateResult(result, options = {}) {
  ...
  if (result.success) {
    ...
    lines.push('');
    lines.push(`Run ${c.cyan}npx sinapse-ai validate${c.reset} to verify installation.`);
  } else {
