...
describe('artifact-copy-pipeline (Story INS-4.3)', () => {
  ...
  describe('copyExtraCommandFiles', () => {
    test('copies .md files recursively excluding SINAPSE/agents/ via real function', async () => {
      const sourceRoot = createTempDir();
      const targetRoot = createTempDir();
      ...
      try {
        ...
        // Create SINAPSE/stories/story.md (should be EXCLUDED — project-specific)
        const storiesDir = path.join(cmdDir, 'SINAPSE', 'stories');
        ...
        // Verify SINAPSE/agents/ was excluded (handled by copyAgentFiles)
        expect(fs.existsSync(path.join(targetCmd, 'SINAPSE', 'agents', 'dev.md'))).toBe(false);

        // Verify SINAPSE/stories/ was excluded (project-specific)
        expect(fs.existsSync(path.join(targetCmd, 'SINAPSE', 'stories', 'story.md'))).toBe(false);
  ...
  describe('generator failure graceful degradation', () => {
    test('generate-settings-json throws on missing config — install should catch', () => {
      const { generate } = require('../../../../../.sinapse-ai/infrastructure/scripts/generate-settings-json');
