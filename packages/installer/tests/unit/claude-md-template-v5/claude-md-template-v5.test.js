...
describe('CLAUDE.md Template v5 (Story INS-4.4)', () => {
  ...
  describe('template has 4 new sections with SINAPSE-MANAGED markers', () => {
    test('template file exists and has SINAPSE markers', () => {
      expect(templateContent).toBeTruthy();
      expect(hasSinapseMarkers(templateContent)).toBe(true);
    });

    test('contains framework-boundary section', () => {
      expect(templateContent).toContain('<!-- SINAPSE-MANAGED-START: framework-boundary -->');
      expect(templateContent).toContain('<!-- SINAPSE-MANAGED-END: framework-boundary -->');
      expect(templateContent).toContain('## Framework vs Project Boundary');
    });

    test('contains rules-system section', () => {
      expect(templateContent).toContain('<!-- SINAPSE-MANAGED-START: rules-system -->');
      expect(templateContent).toContain('<!-- SINAPSE-MANAGED-END: rules-system -->');
      expect(templateContent).toContain('## Rules System');
    });

    test('contains code-intelligence section', () => {
      expect(templateContent).toContain('<!-- SINAPSE-MANAGED-START: code-intelligence -->');
      expect(templateContent).toContain('<!-- SINAPSE-MANAGED-END: code-intelligence -->');
      expect(templateContent).toContain('## Code Intelligence');
    });

    test('contains graph-dashboard section', () => {
      expect(templateContent).toContain('<!-- SINAPSE-MANAGED-START: graph-dashboard -->');
      expect(templateContent).toContain('<!-- SINAPSE-MANAGED-END: graph-dashboard -->');
      expect(templateContent).toContain('## Graph Dashboard');
    });

    test('template has exactly 11 SINAPSE-MANAGED sections total', () => {
      const startMatches = templateContent.match(/<!-- SINAPSE-MANAGED-START:/g);
      const endMatches = templateContent.match(/<!-- SINAPSE-MANAGED-END:/g);
  ...
  describe('section content quality', () => {
    ...
    test('graph-dashboard has CLI commands', () => {
      expect(templateContent).toContain('sinapse graph --deps');
      ...
      expect(templateContent).toContain('sinapse graph --stats');
    });
  });

  describe('existing sections preserved', () => {
    test('original 5 SINAPSE-MANAGED sections still present', () => {
      expect(templateContent).toContain('<!-- SINAPSE-MANAGED-START: core-framework -->');
      expect(templateContent).toContain('<!-- SINAPSE-MANAGED-START: agent-system -->');
      expect(templateContent).toContain('<!-- SINAPSE-MANAGED-START: framework-structure -->');
      expect(templateContent).toContain('<!-- SINAPSE-MANAGED-START: sinapse-patterns -->');
      expect(templateContent).toContain('<!-- SINAPSE-MANAGED-START: common-commands -->');
    });
  ...
  describe('upgrade: new sections added to existing CLAUDE.md', () => {
    ...
    test('upgrade adds 4 new sections to CLAUDE.md without them', async () => {
      const existingClaudeMd = `# SINAPSE
...
My custom testing rules`;
      ...
      expect(result.content).toContain('<!-- SINAPSE-MANAGED-START: framework-boundary -->');
    });
  });

  describe('section order in template', () => {
    test('4 new sections come after framework-structure and before workflow-execution', () => {
      const frameworkStructureEnd = templateContent.indexOf('<!-- SINAPSE-MANAGED-END: framework-structure -->');
      const frameworkBoundaryStart = templateContent.indexOf('<!-- SINAPSE-MANAGED-START: framework-boundary -->');
      const graphDashboardEnd = templateContent.indexOf('<!-- SINAPSE-MANAGED-END: graph-dashboard -->');
