#!/usr/bin/env node
/**
 * SINAPSE Agents Command - List available agents
 */
...
async function main() {
  ...
  const agentsPath = path.join(projectDir, '.sinapse-ai', 'development', 'agents');

  console.log('🤖 SINAPSE Agents\n');
  console.log('━'.repeat(50));

  if (!fs.existsSync(agentsPath)) {
    console.log('No agents found. Run: npx sinapse-ai install');
    return;
  }
  ...
  console.log('\n' + '━'.repeat(50));
  console.log('Quick launch with: /sinapse-menu or /sinapse-<agent-id>');
  console.log('Alternative: /sinapse-agent <agent-id>');
}
