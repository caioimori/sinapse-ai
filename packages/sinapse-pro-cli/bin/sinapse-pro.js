 ...
 * sinapse-pro CLI
 *
 * Thin CLI wrapper for @sinapse-fullstack/pro.
 * Provides a clean npx interface: npx sinapse-pro install
 *
 * Commands:
 *   install             Install @sinapse-fullstack/pro in the current project
...
const PRO_PACKAGE = '@sinapse-fullstack/pro';
...
function isProInstalled() {
  try {
    const pkgPath = path.join(process.cwd(), 'node_modules', '@sinapse-fullstack', 'pro', 'package.json');
...
function findSinapseCli() {
  // Check local node_modules first
  const localBin = path.join(process.cwd(), 'node_modules', '.bin', 'sinapse');
  ...
  // Check global
  try {
    execSync('sinapse --version', { stdio: 'pipe' });
    return 'sinapse';
  } catch {
    return null;
  }
...
function delegateToSinapse(subcommand) {
  const sinapse = findSinapseCli();
  if (!sinapse) {
    console.error('sinapse-ai CLI not found.');
    console.error('Install it first: npm install sinapse-ai');
    process.exit(1);
  }

  const spawnArgs = ['pro', subcommand, ...args.slice(1)];
  const result = spawnSync(sinapse, spawnArgs, { stdio: 'inherit' });
...
function runProWizard(key) {
  ...
  try {
    ...
    console.error('Ensure sinapse-ai installer is available.\n');
    process.exit(1);
  }
...
function showHelp() {
  console.log(`
sinapse-pro v${VERSION} — SINAPSE Pro CLI

Usage:
  npx sinapse-pro <command> [options]
...
Examples:
  ...
  npx sinapse-pro setup
  npx sinapse-pro wizard --key PRO-XXXX-XXXX-XXXX-XXXX
...
function installPro() {
  ...
  console.log('  npx sinapse-pro activate --key PRO-XXXX-XXXX-XXXX-XXXX');
  console.log('  npx sinapse-pro status');
  console.log('');
}
...
if (command === '--version' || command === '-v') {
  console.log(`sinapse-pro v${VERSION}`);
  process.exit(0);
}
...
switch (command) {
  ...
  case 'validate':
    if (!isProInstalled()) {
      ...
      console.error('Run first: npx sinapse-pro install\n');
      process.exit(1);
    }
    delegateToSinapse(command);
    break;
