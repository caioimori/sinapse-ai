...
/**
 * Dependency Graph Validator — .sinapse-ai/package.json completeness
 * Story INS-4.12 (AC1, AC4, AC6)
 *
 * Scans all .js files in .sinapse-ai/development/scripts/ for require() calls,
 * then verifies each non-builtin, non-relative package is declared in
 * .sinapse-ai/package.json dependencies.
 *
 * Exit codes: 0 = PASS, 1 = FAIL (missing deps found)
 * Usage: node scripts/validate-sinapse-ai-deps.js
 */
...
const SINAPSE_CORE_DIR = path.join(PROJECT_ROOT, '.sinapse-ai');
const SCRIPTS_DIR = path.join(SINAPSE_CORE_DIR, 'development', 'scripts');
const PACKAGE_JSON = path.join(SINAPSE_CORE_DIR, 'package.json');
...
function main() {
  console.log('--- .sinapse-ai Dependency Validation (INS-4.12) ---\n');

  // Read .sinapse-ai/package.json
  if (!fs.existsSync(PACKAGE_JSON)) {
    console.error('FAIL: .sinapse-ai/package.json not found');
    process.exit(1);
  }
  ...
  if (scripts.length === 0) {
    console.log('WARN: No .js files found in .sinapse-ai/development/scripts/');
    process.exit(0);
  }

  console.log(`Scanning ${scripts.length} scripts in .sinapse-ai/development/scripts/\n`);
  ...
  console.log(`Declared in .sinapse-ai/package.json: ${declared.size}`);
  ...
  if (errors.length > 0) {
    ...
    console.error('\nFix: Add missing packages to .sinapse-ai/package.json dependencies');
    console.error('Or add to ALLOWLIST if they are optional dev-time tools (wrap in try-catch)\n');
    process.exit(1);
  }

  console.log('PASS: All script dependencies are declared in .sinapse-ai/package.json\n');
  process.exit(0);
}
