...
async function generateManifest() {
  ...
  if (!fs.existsSync(sinapseCoreDir)) {
    throw new Error(`.sinapse-ai directory not found at ${sinapseCoreDir}`);
  }
  ...
  console.log(`Generating manifest for sinapse-ai v${version}...`);

  const allFiles = [];

  // Scan folders
  for (const folder of FOLDERS_TO_COPY) {
    const folderPath = path.join(sinapseCoreDir, folder);
    if (fs.existsSync(folderPath)) {
      scanDirectory(folderPath, sinapseCoreDir, allFiles);
    }
  }
  ...
  for (const fullPath of allFiles) {
    try {
      const relativePath = path.relative(sinapseCoreDir, fullPath).replace(/\\/g, '/');
...
async function writeManifest(manifest) {
  ...
  // Add header comment
  const header = `# SINAPSE-Core Install Manifest
...
async function main() {
  try {
    ...
    console.log('SINAPSE-Core Install Manifest Generator');
    console.log('='.repeat(60));
    console.log('');
