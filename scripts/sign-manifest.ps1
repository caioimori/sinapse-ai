...
#
# Usage:
#   .\scripts\sign-manifest.ps1 -SecretKey C:\path\to\sinapse-ai.key
#
# Prerequisites:
...
$ManifestPath = Join-Path $ProjectRoot ".sinapse-ai\install-manifest.yaml"
$SignaturePath = "$ManifestPath.minisig"

Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Yellow
Write-Host "  SINAPSE-Core Manifest Signing Tool (Windows)" -ForegroundColor Yellow
Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Yellow
Write-Host ""
...
if (-not (Test-Path $ManifestPath)) {
    ...
    Write-Host "  node bin\sinapse.js manifest:generate"
    exit 1
}
...
Push-Location (Join-Path $ProjectRoot ".sinapse-ai")
try {
...
Write-Host "  1. git add .sinapse-ai\install-manifest.yaml.minisig"
