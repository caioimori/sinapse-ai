...
function shouldCheckManifest(stagedFiles) {
  ...
  return stagedFiles.some((file) => {
    if (file === '.sinapse-ai/install-manifest.yaml') return false;
    return file.startsWith('.sinapse-ai/');
  });
}
...
async function main() {
  ...
  if (!shouldCheckManifest(stagedFiles)) {
    console.log('ℹ️ manifest: no .sinapse-ai changes staged, skipping check');
    return;
  }
  ...
  execSync('git add .sinapse-ai/install-manifest.yaml', { stdio: 'inherit' });
