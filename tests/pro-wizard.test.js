...
beforeEach(() => {
  ...
  delete process.env.SINAPSE_PRO_KEY;
...
describe('showProHeader', () => {
  test('outputs branded header', () => {
    ...
    expect(output).toContain('SINAPSE Pro');
...
describe('stepLicenseGate', () => {
  test('CI mode: fails when SINAPSE_PRO_KEY not set', async () => {
    process.env.CI = 'true';
    delete process.env.SINAPSE_PRO_KEY;
    ...
    expect(result.error).toContain('SINAPSE_PRO_KEY');
  });
  ...
  test('CI mode with valid format key: attempts API validation', async () => {
    process.env.CI = 'true';
    process.env.SINAPSE_PRO_KEY = 'PRO-ABCD-EFGH-IJKL-MNOP';
...
describe('stepInstallScaffold', () => {
  test('resolves pro source from bundled dir or fails gracefully', async () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sinapse-pro-test-'));
...
describe('runProWizard', () => {
  test('fails in CI mode without SINAPSE_PRO_KEY', async () => {
    process.env.CI = 'true';
    delete process.env.SINAPSE_PRO_KEY;
  ...
  test('fails with invalid key format in CI', async () => {
    process.env.CI = 'true';
    process.env.SINAPSE_PRO_KEY = 'INVALID';
  ...
  test('does not show branding in CI mode', async () => {
    process.env.CI = 'true';
    process.env.SINAPSE_PRO_KEY = 'PRO-AAAA-BBBB-CCCC-DDDD';
...
describe('Key Masking Security', () => {
  test('full key never appears in console output during wizard', async () => {
    process.env.CI = 'true';
    const fullKey = 'PRO-ABCD-EFGH-IJKL-MNOP';
    process.env.SINAPSE_PRO_KEY = fullKey;
