...
describe('license-cache', () => {
  ...
  beforeEach(() => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sinapse-license-test-'));
  });
  ...
  describe('writeLicenseCache', () => {
    ...
    it('should create .sinapse directory if not exists', () => {
      const data = createTestCacheData();
      const sinapseDir = getSinapseDir(testDir);

      expect(fs.existsSync(sinapseDir)).toBe(false);

      writeLicenseCache(data, testDir);

      expect(fs.existsSync(sinapseDir)).toBe(true);
    });
  ...
  describe('readLicenseCache', () => {
    ...
    it('should return null for corrupted JSON', () => {
      ...
      fs.mkdirSync(getSinapseDir(testDir), { recursive: true });
  ...
  describe('Pending Deactivation (AC-7b)', () => {
    ...
    describe('markPendingDeactivationSynced', () => {
      ...
      it('should add syncedAt timestamp', () => {
        ...
        const filePath = path.join(getSinapseDir(testDir), 'pending-deactivation.json');
