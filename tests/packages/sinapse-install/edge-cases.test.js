...
} = require('../../../packages/sinapse-install/src/installer');

const {
  checkAllDependencies,
  checkDockerRunning,
} = require('../../../packages/sinapse-install/src/dep-checker');

const {
  checkDocker,
  ensureDocker,
} = require('../../../packages/sinapse-install/src/edmcp');
...
describe('Edge Cases - Task 7.7', () => {
  ...
  describe('No Internet Connectivity', () => {
    ...
    it('should handle npm install failure due to network error', async () => {
      ...
      // When/Then
      await expect(async () => {
        await execa('npm', ['install', 'sinapse-ai']);
  ...
  describe('Insufficient Permissions', () => {
    ...
    it('should handle permission denied during npm install', async () => {
      ...
      // When/Then
      await expect(async () => {
        await execa('npm', ['install', '-g', 'sinapse-ai']);
      }).rejects.toThrow('EACCES');
    });
