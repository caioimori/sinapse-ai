...
describe('v2.1 Module Structure Tests', () => {
  describe('INS-01: Source Path Configuration', () => {
    it('should point getSinapseCorePath to .sinapse-ai directory', () => {
      const sinapseCorePath = resourceLocator.getSinapseCorePath();
      expect(sinapseCorePath).toContain('.sinapse-ai');
      expect(sinapseCorePath).not.toMatch(/[^.]sinapse-ai$/);
    });

    it('should have .sinapse-ai as source for config-loader', () => {
      const sinapseCorePath = configLoader.getSinapseCorePath();
      expect(sinapseCorePath).toContain('.sinapse-ai');
    });
  });
  ...
  describe('INS-02: Agent Path Resolution', () => {
    ...
    it('should list agents from development/agents/', async () => {
      ...
      for (const agent of agents) {
        expect(agent.file).toContain('.sinapse-ai/development/agents/');
      }
    });
  });
  ...
  describe('INS-04: Resource Locator v2.1 Paths', () => {
    it('should resolve agent dependencies with v2.1 module mapping', async () => {
      ...
      for (const depPath of deps.all) {
        // Should not have flat structure paths like .sinapse-ai/tasks/
        expect(depPath).not.toMatch(/\.sinapse-ai\/(tasks|templates|checklists|workflows|utils|data)\//);
  ...
  describe('INS-05: Manifest Files Location', () => {
    it('should have manifests directory in .sinapse-ai', async () => {
      const manifestsPath = path.join(resourceLocator.getSinapseCorePath(), 'manifests');
      const exists = await fs.pathExists(manifestsPath);
      expect(exists).toBe(true);
    });

    it('should have agents.csv manifest', async () => {
      const agentsCsvPath = path.join(resourceLocator.getSinapseCorePath(), 'manifests', 'agents.csv');
      const exists = await fs.pathExists(agentsCsvPath);
      expect(exists).toBe(true);
    });

    it('should have tasks.csv manifest', async () => {
      const tasksCsvPath = path.join(resourceLocator.getSinapseCorePath(), 'manifests', 'tasks.csv');
      const exists = await fs.pathExists(tasksCsvPath);
      expect(exists).toBe(true);
    });

    it('should have workers.csv manifest', async () => {
      const workersCsvPath = path.join(resourceLocator.getSinapseCorePath(), 'manifests', 'workers.csv');
