...
describe('ACT-12: Language delegated to Claude Code settings.json', () => {
  ...
  beforeEach(async () => {
    tempDir = path.join(os.tmpdir(), `sinapse-test-lang-${Date.now()}`);
    await fse.ensureDir(tempDir);
    await fse.ensureDir(path.join(tempDir, '.sinapse-ai'));
  });
  ...
  test('should NOT include language in generated core-config.yaml', async () => {
    ...
    const configPath = path.join(tempDir, '.sinapse-ai', 'core-config.yaml');
