...
describe('brownfield-upgrader', () => {
  ...
  describe('generateUpgradeReport', () => {
    ...
    it('should identify modified files', () => {
      const sinapseCoreDir = path.join(targetDir, '.sinapse-ai');
      fs.ensureDirSync(sinapseCoreDir);
      fs.writeFileSync(path.join(sinapseCoreDir, 'changed.md'), 'original');
      const originalHash = `sha256:${hashFile(path.join(sinapseCoreDir, 'changed.md'))}`;
    ...
    it('should identify user-modified files', () => {
      const sinapseCoreDir = path.join(targetDir, '.sinapse-ai');
      fs.ensureDirSync(sinapseCoreDir);
      fs.writeFileSync(path.join(sinapseCoreDir, 'user-changed.md'), 'user modified content');
  ...
  describe('applyUpgrade', () => {
    ...
    it('should install new files', async () => {
      ...
      expect(fs.existsSync(path.join(targetDir, '.sinapse-ai', 'new-file.md'))).toBe(true);
    });
    ...
    it('should perform dry run without modifying files', async () => {
      ...
      expect(result.filesInstalled).toHaveLength(1);
      expect(fs.existsSync(path.join(targetDir, '.sinapse-ai', 'new-file.md'))).toBe(false);
    });
  });
  ...
  describe('updateInstalledManifest', () => {
    it('should create installed manifest file', () => {
      ...
      fs.ensureDirSync(path.join(targetDir, '.sinapse-ai'));
      updateInstalledManifest(targetDir, sourceManifest, 'sinapse-ai@2.1.0');

      const installedPath = path.join(targetDir, '.sinapse-ai', '.installed-manifest.yaml');
      expect(fs.existsSync(installedPath)).toBe(true);

      const content = yaml.load(fs.readFileSync(installedPath, 'utf8'));
      expect(content.installed_version).toBe('2.1.0');
      expect(content.installed_from).toBe('sinapse-ai@2.1.0');
    });
  });
