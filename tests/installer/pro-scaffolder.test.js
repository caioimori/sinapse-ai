...
beforeEach(async () => {
  ...
  await fs.writeJson(
    ...
    { name: '@sinapse-fullstack/pro', version: '2.0.0' }
  );
});
...
describe('scaffoldProContent', () => {
  ...
  it('should copy all pro content to project (AC1, AC2, AC3)', async () => {
    ...
    // AC2: pro-config.yaml exists in .sinapse-ai/
    expect(await fs.pathExists(
      path.join(targetDir, '.sinapse-ai', 'pro-config.yaml')
    )).toBe(true);

    // AC3: feature-registry.yaml exists in .sinapse-ai/
    expect(await fs.pathExists(
      path.join(targetDir, '.sinapse-ai', 'feature-registry.yaml')
    )).toBe(true);
  });
  ...
  it('should be idempotent: 2nd run skips identical files (AC5)', async () => {
    ...
    const configContent = yaml.load(
      await fs.readFile(path.join(targetDir, '.sinapse-ai', 'pro-config.yaml'), 'utf8')
    );
