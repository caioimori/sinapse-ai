...
const {
  ...
  formatMigrationPlan,
  analyzeImports,
} = require('../../.sinapse-ai/cli/commands/migrate/analyze');

describe('Migration Analyze Module', () => {
  let testDir;

  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `sinapse-analyze-test-${Date.now()}`);
  ...
  describe('detectV2Structure', () => {
    ...
    it('should detect v2.1 modular structure', async () => {
      ...
      await fs.promises.mkdir(path.join(sinapseCoreDir, 'product'), { recursive: true });
      await fs.promises.mkdir(path.join(sinapseCoreDir, 'infrastructure'), { recursive: true });
    ...
    it('should return error if no .sinapse-ai exists', async () => {
      const result = await detectV2Structure(testDir);
  ...
  describe('analyzeMigrationPlan', () => {
    it('should generate migration plan for v2.0 project', async () => {
      ...
      await fs.promises.writeFile(path.join(sinapseCoreDir, 'tasks', 'build.md'), 'Task');
    ...
    it('should detect conflicts', async () => {
      ...
      const sinapseCoreDir = path.join(testDir, '.sinapse-ai');
      await fs.promises.mkdir(path.join(sinapseCoreDir, 'agents'), { recursive: true });
      await fs.promises.mkdir(path.join(sinapseCoreDir, 'core'), { recursive: true }); // Conflict
  ...
  describe('analyzeImports', () => {
    it('should analyze importable files', async () => {
      const sinapseCoreDir = path.join(testDir, '.sinapse-ai');
      await fs.promises.mkdir(path.join(sinapseCoreDir, 'cli'), { recursive: true });
      await fs.promises.writeFile(path.join(sinapseCoreDir, 'cli', 'index.js'), 'module.exports = {}');
