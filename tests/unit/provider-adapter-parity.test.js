...
const { assertProviderAdapterParity, REQUIRED_GLOBAL_PROVIDER_SKILL_IDS } = require('../../bin/lib/provider-parity');
...
describe('provider adapter parity', () => {
  ...
  test('validates exact canonical identities and alias skills for both providers', () => {
    expect(assertProviderAdapterParity('both', {
      claude: claude(),
      claudeSkills: REQUIRED_GLOBAL_PROVIDER_SKILL_IDS,
      codex: codex(),
      skills: REQUIRED_GLOBAL_PROVIDER_SKILL_IDS,
    }, canonical())).toBe(172);
  });
  ...
  test('rejects a missing alias skill separately from agent parity', () => {
    expect(() => assertProviderAdapterParity('claude-code', {
      claude: claude(),
      claudeSkills: REQUIRED_GLOBAL_PROVIDER_SKILL_IDS.filter((id) => id !== 'snps-orqx'),
      codex: [],
    }, canonical())).toThrow('Claude Code global skills missing: snps-orqx');
  });
