...
describe('IDE Config Generator', () => {
  ...
  describe('generateTemplateVariables', () => {
    it('should generate default variables', () => {
      ...
      expect(variables).toHaveProperty('sinapseVersion');
    });
    ...
    it('should include SINAPSE version', () => {
      const wizardState = {};
      const variables = generateTemplateVariables(wizardState);

      expect(variables.sinapseVersion).toBeDefined();
      expect(typeof variables.sinapseVersion).toBe('string');
    });
  });
  ...
  describe.skip('generateIDEConfigs', () => {
    ...
    it('should configure Gemini hooks and settings with active SINAPSE hooks', async () => {
      ...
      const beforeAgentWrapper = settings.hooks.BeforeAgent.find(
        (w) => Array.isArray(w.hooks) && w.hooks.some((h) => h.name === 'sinapse-context-inject'),
      );
      expect(beforeAgentWrapper).toBeDefined();
      const hook = beforeAgentWrapper.hooks.find((h) => h.name === 'sinapse-context-inject');
