...
`;
  ...
  describe('Cache Mechanism', () => {
    test('should cache tool after first resolution', async () => {
      // Mock file system
      fg.sync.mockReturnValue(['sinapse-ai/tools/test-simple.yaml']);
  ...
  describe('Search Path Priority', () => {
    ...
    test('should fall back to core when squad tool not found', async () => {
      ...
      fg.sync.mockImplementation((pattern) => {
        ...
        if (pattern.includes('sinapse-ai/tools')) {
          return ['sinapse-ai/tools/test-simple.yaml'];
        }
        return [];
      });
  ...
  describe('Schema Validation', () => {
    test('should validate required fields', async () => {
      fg.sync.mockReturnValue(['sinapse-ai/tools/invalid.yaml']);
    ...
    test('should validate v1.0 tool schema', async () => {
      fg.sync.mockReturnValue(['sinapse-ai/tools/simple.yaml']);
      fs.readFile.mockResolvedValue(simpleToolYaml);
    ...
    test('should validate v2.0 tool schema with executable knowledge', async () => {
      fg.sync.mockReturnValue(['sinapse-ai/tools/complex.yaml']);
    ...
    test('should reject invalid YAML syntax', async () => {
      fg.sync.mockReturnValue(['sinapse-ai/tools/broken.yaml']);
...
`;
      ...
      fg.sync.mockReturnValue(['sinapse-ai/tools/auto.yaml']);
...
`;
      ...
      fg.sync.mockReturnValue(['sinapse-ai/tools/api.yaml']);
...
`;
      ...
      fg.sync.mockReturnValue(['sinapse-ai/tools/anti.yaml']);
...
`;
      ...
      fg.sync.mockReturnValue(['sinapse-ai/tools/health.yaml']);
...
`;
      ...
      fg.sync.mockReturnValue(['sinapse-ai/tools/http-health.yaml']);
...
`;
      ...
      fg.sync.mockReturnValue(['sinapse-ai/tools/function-health.yaml']);
  ...
  describe('Additional Methods', () => {
    test('should list all available tools', async () => {
      fg.sync.mockReturnValue([
        'sinapse-ai/tools/tool1.yaml',
        'sinapse-ai/tools/tool2.yaml',
    ...
    test('should check if tool exists without loading', async () => {
      ...
      fg.sync.mockImplementation((pattern) => {
        if (pattern.includes('/exists.yaml')) {
          return ['sinapse-ai/tools/exists.yaml'];
        }
        return [];
      });
