...
describe('Mode Detector', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sinapse-test-'));
  });
  ...
  describe('detectInstallationMode', () => {
    ...
    it('should detect FRAMEWORK_DEV mode for sinapse-ai repository', () => {
      // Create .sinapse-ai directory
      fs.mkdirSync(path.join(tempDir, '.sinapse-ai'));

      // Create package.json with sinapse-ai name
      fs.writeFileSync(
      ...
      expect(result.legacyType).toBe(LegacyProjectType.EXISTING_SINAPSE);
      expect(result.confidence).toBe(100);
      expect(result.markers.isSinapseCoreRepo).toBe(true);
    });

    it('should detect BROWNFIELD mode for user project with existing SINAPSE', () => {
      // Create .sinapse-ai directory
      fs.mkdirSync(path.join(tempDir, '.sinapse-ai'));
  ...
  describe('isSinapseCoreRepository', () => {
    it('should return true for @sinapse/core package', () => {
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
        JSON.stringify({ name: '@sinapse/core' }),
      );

      expect(isSinapseCoreRepository(tempDir)).toBe(true);
    });

    it('should return true for sinapse-ai package', () => {
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
        JSON.stringify({ name: 'sinapse-ai' }),
      );
    ...
    it('should return false for generic monorepo with workspaces pattern only', () => {
      // Generic monorepos should NOT be detected as sinapse-ai
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
        JSON.stringify({ name: 'something', workspaces: ['packages/*'] }),
      );

      expect(isSinapseCoreRepository(tempDir)).toBe(false);
    });

    it('should return true for workspaces pattern with sinapse marker', () => {
      // Workspaces + .sinapse-ai/infrastructure marker = sinapse-ai repo
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
        JSON.stringify({ name: 'something', workspaces: ['packages/*'] }),
      );
      fs.mkdirSync(path.join(tempDir, '.sinapse-ai', 'infrastructure'), { recursive: true });
  ...
  describe('mapLegacyTypeToMode', () => {
    it('should map EXISTING_SINAPSE to BROWNFIELD by default (safer)', () => {
      // Without context, EXISTING_SINAPSE defaults to BROWNFIELD (won't skip project setup)
      expect(mapLegacyTypeToMode(LegacyProjectType.EXISTING_SINAPSE)).toBe(
        InstallationMode.BROWNFIELD,
      );
    });

    it('should map EXISTING_SINAPSE to FRAMEWORK_DEV with isSinapseCoreRepo context', () => {
      expect(mapLegacyTypeToMode(LegacyProjectType.EXISTING_SINAPSE, { isSinapseCoreRepo: true })).toBe(
        InstallationMode.FRAMEWORK_DEV,
      );
    });

    it('should map EXISTING_SINAPSE to BROWNFIELD with non-sinapse-ai context', () => {
      expect(mapLegacyTypeToMode(LegacyProjectType.EXISTING_SINAPSE, { isSinapseCoreRepo: false })).toBe(
        InstallationMode.BROWNFIELD,
      );
    });
  ...
  describe('validateModeSelection', () => {
    ...
    it('should warn when selecting framework-dev for non-sinapse-ai repo', () => {
      const detected = {
        mode: InstallationMode.BROWNFIELD,
        markers: { isSinapseCoreRepo: false },
      };

      const result = validateModeSelection(InstallationMode.FRAMEWORK_DEV, detected);

      expect(result.warnings).toHaveLength(1);
      expect(result.warnings[0]).toContain('sinapse-ai repository');
    });
  ...
  describe('LegacyProjectType enum', () => {
    it('should have all legacy types', () => {
      expect(LegacyProjectType.EXISTING_SINAPSE).toBe('EXISTING_SINAPSE');
