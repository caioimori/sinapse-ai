...
const {
  ...
  DeploymentWorkflow,
  DeploymentPlatform,
} = require('../../../.sinapse-ai/infrastructure/scripts/documentation-integrity/config-generator');

describe('Config Generator', () => {
  let tempDir;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sinapse-configgen-test-'));
  });
  ...
  describe('generateConfig', () => {
    ...
    it('should create .sinapse-ai directory', () => {
      const context = buildConfigContext('test', 'greenfield', {});

      generateConfig(tempDir, 'greenfield', context);

      expect(fs.existsSync(path.join(tempDir, '.sinapse-ai'))).toBe(true);
      expect(fs.existsSync(path.join(tempDir, '.sinapse-ai', 'core-config.yaml'))).toBe(true);
    });
    ...
    it('should support dry run mode', () => {
      ...
      expect(result.success).toBe(true);
      expect(result.content).toBeTruthy();
      expect(fs.existsSync(path.join(tempDir, '.sinapse-ai'))).toBe(false);
    });
