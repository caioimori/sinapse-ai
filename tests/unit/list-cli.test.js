...
// Test modules
const { formatTree, formatTreeCollapsed, groupWorkers } = require('../../.sinapse-ai/cli/commands/workers/formatters/list-tree');
const { formatTable, formatJSON, formatYAML, formatList, formatCount, truncate } = require('../../.sinapse-ai/cli/commands/workers/formatters/list-table');
const { paginate, formatPaginationInfo, formatPaginationHint } = require('../../.sinapse-ai/cli/commands/workers/utils/pagination');
...
const mockWorkers = [
  ...
  },
  {
    ...
    subcategory: 'transformation',
    tags: ['etl', 'json', 'csv'],
    path: '.sinapse-ai/tasks/csv-json-transformer.md',
  },
  {
    id: 'json-validator',
    ...
    path: '.sinapse-ai/tasks/json-validator.md',
  },
  {
    id: 'unit-test-runner',
    ...
    path: '.sinapse-ai/tasks/unit-test-runner.md',
  },
  {
    id: 'api-generator',
    ...
    path: '.sinapse-ai/tasks/api-generator.md',
  },
];
...
describe('Tree Formatter', () => {
  ...
  test('formatTree includes usage hints', () => {
    const output = formatTree(mockWorkers, {});
    expect(output).toContain('sinapse workers info <id>');
    expect(output).toContain('sinapse workers search <query>');
  });
...
describe('Performance Requirements', () => {
  ...
  const largeMockWorkers = Array.from({ length: 250 }, (_, i) => ({
    ...
    tags: ['tag1', 'tag2', 'tag3'],
    path: `.sinapse-ai/tasks/worker-${i}.md`,
  }));
