...
const { RegistryProvider, LAYER_PRIORITY } = require('../../../.sinapse-ai/core/code-intel/providers/registry-provider');
const { CodeIntelProvider } = require('../../../.sinapse-ai/core/code-intel/providers/provider-interface');
const { CodeGraphProvider } = require('../../../.sinapse-ai/core/code-intel/providers/code-graph-provider');
const { CodeIntelClient, CB_CLOSED } = require('../../../.sinapse-ai/core/code-intel/code-intel-client');
...
entities:
  tasks:
    create-story:
      path: .sinapse-ai/development/tasks/create-story.md
      layer: L2
      type: task
      ...
      dependencies:
        - story-template
    validate-story:
      path: .sinapse-ai/development/tasks/validate-story.md
      layer: L2
      type: task
  ...
  templates:
    story-template:
      path: .sinapse-ai/development/templates/story-tmpl.yaml
      layer: L2
      type: template
  ...
  core:
    code-intel-client:
      path: .sinapse-ai/core/code-intel/code-intel-client.js
      layer: L1
      type: module
...
entities:
  tasks:
    yaml-validator:
      path: .sinapse-ai/development/tasks/yaml-validator.md
      layer: L2
      type: task
      ...
      dependencies: []
    index:
      path: .sinapse-ai/development/tasks/index.md
      layer: L2
      type: task
  ...
  scripts:
    yaml-validator:
      path: .sinapse-ai/development/scripts/yaml-validator.js
      layer: L2
      type: script
      ...
      dependencies: []
    backup-manager:
      path: .sinapse-ai/development/scripts/backup-manager.js
      layer: L2
      type: script
  ...
  core:
    yaml-validator:
      path: .sinapse-ai/core/yaml-validator.js
      layer: L1
      type: module
    ...
    index:
      ...
      dependencies: []
    backup-manager:
      path: .sinapse-ai/core/backup-manager.js
      layer: L1
      type: module
  ...
  templates:
    yaml-validator:
      path: .sinapse-ai/development/templates/yaml-validator.yaml
      layer: L3
      type: template
      ...
      dependencies: []
    index:
      path: .sinapse-ai/development/templates/index.yaml
      layer: L3
      type: template
      ...
      dependencies: []
    backup-manager:
      path: .sinapse-ai/development/templates/backup-manager.yaml
      layer: L3
      type: template
...
describe('RegistryProvider — 5 Implemented Primitives', () => {
  ...
  describe('findDefinition()', () => {
    test('finds entity by exact name', async () => {
      ...
      expect(result.file).toBe('.sinapse-ai/development/tasks/create-story.md');
  ...
  describe('findReferences()', () => {
    ...
    test('finds references via dependencies field', async () => {
      ...
      expect(paths).toContain('.sinapse-ai/development/tasks/create-story.md');
    });
...
describe('RegistryProvider — Entity Name Disambiguation', () => {
  ...
  test('yaml-validator has 4 entries, all preserved in index', async () => {
    ...
    expect(result.file).toBe('.sinapse-ai/core/yaml-validator.js');
  });
  ...
  test('index has 3 entries, all preserved', async () => {
    ...
    expect(result).not.toBeNull();
    // L1 core should win
    expect(result.file).toBe('.sinapse-ai/core/index.js');
  });

  test('backup-manager has 3 entries, L1 wins', async () => {
    const result = await provider.findDefinition('backup-manager');
    expect(result).not.toBeNull();
    expect(result.file).toBe('.sinapse-ai/core/backup-manager.js');
  });
  ...
  test('disambiguation scoring: L1 > L2 > L3', async () => {
    ...
    const result = await provider.findDefinition('yaml-validator');
    expect(result.file).toContain('.sinapse-ai/core/');
  });
});
