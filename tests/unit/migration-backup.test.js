...
const {
  ...
  findLatestBackup,
  listBackups,
} = require('../../.sinapse-ai/cli/commands/migrate/backup');
...
describe('Migration Backup Module', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `sinapse-backup-test-${testId}`);
  ...
  describe('createBackupDirName', () => {
    it('should create backup directory name with date format', () => {
      ...
      expect(name).toMatch(/^\.sinapse-backup-\d{4}-\d{2}-\d{2}$/);
    });

    it('should use current date', () => {
      const name = createBackupDirName();
      const today = new Date().toISOString().split('T')[0];
      expect(name).toBe(`.sinapse-backup-${today}`);
    });
  });
  ...
  describe('createBackup', () => {
    it('should create backup of .sinapse-ai directory', async () => {
      // Create mock .sinapse-ai structure
      const sinapseCoreDir = path.join(testDir, '.sinapse-ai');
      await fs.promises.mkdir(path.join(sinapseCoreDir, 'agents'), { recursive: true });
      await fs.promises.writeFile(path.join(sinapseCoreDir, 'agents', 'test.md'), 'Agent content');
      await fs.promises.writeFile(path.join(sinapseCoreDir, 'index.js'), 'module.exports = {}');
    ...
    it('should fail if .sinapse-ai does not exist', async () => {
      await expect(createBackup(testDir)).rejects.toThrow(/No .sinapse-ai directory/);
    });

    it('should include backup manifest', async () => {
      const sinapseCoreDir = path.join(testDir, '.sinapse-ai');
      await fs.promises.mkdir(sinapseCoreDir, { recursive: true });
      await fs.promises.writeFile(path.join(sinapseCoreDir, 'test.js'), 'test');
  ...
  describe('verifyBackup', () => {
    it('should verify valid backup', async () => {
      ...
      const sinapseCoreDir = path.join(testDir, '.sinapse-ai');
      await fs.promises.mkdir(sinapseCoreDir, { recursive: true });
      await fs.promises.writeFile(path.join(sinapseCoreDir, 'test.js'), 'test content');
    ...
    it('should detect corrupted files', async () => {
      const sinapseCoreDir = path.join(testDir, '.sinapse-ai');
      await fs.promises.mkdir(sinapseCoreDir, { recursive: true });
      await fs.promises.writeFile(path.join(sinapseCoreDir, 'test.js'), 'original');

      const backupResult = await createBackup(testDir);

      // Corrupt a file in backup
      const backedUpFile = path.join(backupResult.backupDir, '.sinapse-ai', 'test.js');
  ...
  describe('findLatestBackup', () => {
    it('should find the most recent backup', async () => {
      ...
      expect(latest.name).toMatch(/^\.sinapse-backup-/);
    });
