...
const {
  ...
  commandNameForAgent,
} = require('../../packages/gemini-sinapse-extension/commands/lib/agent-launcher');
...
describe('gemini agent launcher', () => {
  ...
  it('lists canonical agents from source-of-truth directory', () => {
    ...
    write(path.join(tmpRoot, '.sinapse-ai', 'development', 'agents', 'architect.md'), '# architect');
    write(path.join(tmpRoot, '.sinapse-ai', 'development', 'agents', '_README.md'), '# ignore');
  ...
  it('detects agent presence in canonical or gemini mirrored folders', () => {
    write(path.join(tmpRoot, '.gemini', 'rules', 'SINAPSE', 'agents', 'qa.md'), '# qa');
  ...
  it('builds deterministic activation prompt', () => {
    ...
    expect(prompt).toContain('.sinapse-ai/development/agents/devops.md');
