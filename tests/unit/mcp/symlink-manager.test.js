...
jest.mock('../../../.sinapse-ai/core/mcp/os-detector', () => ({
  isWindows: jest.fn(),
...
const {
  ...
  createLink,
  removeLink,
} = require('../../../.sinapse-ai/core/mcp/symlink-manager');

const osDetector = require('../../../.sinapse-ai/core/mcp/os-detector');

describe('Symlink Manager', () => {
  const mockProjectRoot = '/mock/project';
  const mockGlobalMcpDir = '/mock/home/.sinapse/mcp';
  const mockLinkPath = path.join(mockProjectRoot, '.sinapse-ai', 'tools', 'mcp');
  ...
  describe('getProjectMcpPath', () => {
    it('should return correct path for project root', () => {
      ...
      expect(result).toBe(path.join(mockProjectRoot, '.sinapse-ai', 'tools', 'mcp'));
    });

    it('should use current directory when no root provided', () => {
      const cwd = process.cwd();
      const result = getProjectMcpPath();
      expect(result).toBe(path.join(cwd, '.sinapse-ai', 'tools', 'mcp'));
    });
  });
