...
const { filterContent, stripHtml, truncateAtBoundary, extractFields, CHARS_PER_TOKEN } = require('../../.sinapse-ai/utils/filters/content-filter');
const { filterSchema, projectFields } = require('../../.sinapse-ai/utils/filters/schema-filter');
const { filterFields } = require('../../.sinapse-ai/utils/filters/field-filter');
const { applyFilter, loadFilterConfig } = require('../../.sinapse-ai/utils/filters/index');
...
describe('constants.js', () => {
  it('exports CHARS_PER_TOKEN as 4', () => {
    const { CHARS_PER_TOKEN } = require('../../.sinapse-ai/utils/filters/constants');
    expect(CHARS_PER_TOKEN).toBe(4);
  });

  it('is the same value used by content-filter and schema-filter', () => {
    const { CHARS_PER_TOKEN: contentCPT } = require('../../.sinapse-ai/utils/filters/content-filter');
    const { CHARS_PER_TOKEN: schemaCPT } = require('../../.sinapse-ai/utils/filters/schema-filter');
    const { CHARS_PER_TOKEN: sharedCPT } = require('../../.sinapse-ai/utils/filters/constants');
...
describe('index.js (filter dispatcher)', () => {
  const registryPath = path.join(__dirname, '..', '..', '.sinapse-ai', 'data', 'tool-registry.yaml');
