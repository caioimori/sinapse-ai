...
describe('validate-codex-delegation', () => {
  ...
  beforeEach(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'sinapse-validate-codex-delegation-'));
  });
  ...
  it('fails when delegation route aliases collide', () => {
    writeBaseFixtures();
    write(
      '.codex/delegation-matrix.json',
      JSON.stringify({
        ...
        orchestrators: {
          'sinapse-orqx': {
            sourceOfTruth: '.codex/agents/sinapse-orqx.md',
            approvedRoutes: ['route-a', 'route-b'],
          },
        },
        frameworkAgents: {},
        routes: {
    ...
    expect(result.ok).toBe(false);
    expect(result.errors.some((error) => error.includes('duplicate delegation route alias'))).toBe(true);
  });
});
