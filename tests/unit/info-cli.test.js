...
const {
  ...
  formatNotFoundError,
  wrapText,
} = require('../../.sinapse-ai/cli/commands/workers/formatters/info-formatter');
const { findSuggestions, findRelatedWorkers } = require('../../.sinapse-ai/cli/commands/workers/info');
const { levenshteinDistance } = require('../../.sinapse-ai/cli/commands/workers/search-keyword');
...
describe('Info Formatter - Pretty Output', () => {
  ...
  test('formatInfoPretty includes usage example', () => {
    ...
    expect(output).toContain('sinapse task run json-csv-transformer');
  });
...
describe('Not Found Error Formatter', () => {
  ...
  test('formatNotFoundError includes search hint', () => {
    ...
    expect(output).toContain('sinapse workers search invalid');
  });
});
