...
const VALIDATOR_PATH = path.join(PROJECT_ROOT, 'scripts', 'validate-sinapse-ai-deps.js');

describe('validate-sinapse-ai-deps.js (INS-4.12)', () => {
  ...
  test('.sinapse-ai/package.json includes fast-glob', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.sinapse-ai', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('fast-glob');
  });

  test('.sinapse-ai/package.json includes fs-extra', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.sinapse-ai', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('fs-extra');
  });

  test('.sinapse-ai/package.json includes semver', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.sinapse-ai', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('semver');
  });

  test('.sinapse-ai/package.json includes ajv', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.sinapse-ai', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('ajv');
  });

  test('.sinapse-ai/package.json includes tar', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.sinapse-ai', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
...
describe('validate-publish.js dependency check (INS-4.12)', () => {
  test('validate-publish includes dependency completeness check', () => {
    ...
    expect(content).toContain('validate-sinapse-ai-deps');
  });
});
...
describe('wizard NODE_PATH fix (INS-4.12)', () => {
  test('wizard/index.js sets NODE_PATH for entity registry bootstrap', () => {
    ...
    expect(content).toContain('sinapseCoreNodeModules');
  });
...
describe('doctor npm-packages check (INS-4.12)', () => {
  test('doctor check validates .sinapse-ai/node_modules/', () => {
    const checkPath = path.join(PROJECT_ROOT, '.sinapse-ai', 'core', 'doctor', 'checks', 'npm-packages.js');
    const content = fs.readFileSync(checkPath, 'utf8');
    expect(content).toContain('sinapseCoreNodeModules');
    expect(content).toContain('.sinapse-ai');
    expect(content).toContain('INS-4.12');
  });
});
