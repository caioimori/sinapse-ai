...
jest.mock('../../.sinapse-ai/core/execution/build-state-manager', () => ({
  BuildStateManager: jest.fn().mockImplementation(() => ({ ...mockStateInstance })),
}));

// Mock optional dependencies to not load
jest.mock('../../.sinapse-ai/infrastructure/scripts/recovery-tracker', () => {
...
const {
  ...
  SubtaskResult,
  DEFAULT_CONFIG,
} = require('../../.sinapse-ai/core/execution/autonomous-build-loop');

const { BuildStateManager } = require('../../.sinapse-ai/core/execution/build-state-manager');
