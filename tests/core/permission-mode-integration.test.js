...
const { PermissionMode } = require('../../.sinapse-ai/core/permissions/permission-mode');
const { OperationGuard } = require('../../.sinapse-ai/core/permissions/operation-guard');
const {
  createGuard,
  checkOperation,
  getModeBadge,
  ...
  cycleMode,
  enforcePermission,
} = require('../../.sinapse-ai/core/permissions');
...
describe('Permission Mode Integration (Story ACT-4)', () => {
  ...
  beforeEach(() => {
    ...
    const sinapseDir = path.join(tempDir, '.sinapse');
    fs.mkdirSync(sinapseDir, { recursive: true });
    configPath = path.join(sinapseDir, 'config.yaml');
  });
