...
const {
  ...
  ConflictSeverity,
  MergeDecision,
} = require('../../.sinapse-ai/core/execution/semantic-merge-engine');
...
describe('SemanticMergeEngine Module', () => {
  ...
  describe('CustomRulesLoader', () => {
    ...
    test('constructor sets paths', () => {
      ...
      expect(loader.rulesPath).toContain('.sinapse');
    });
  ...
  describe('SemanticMergeEngine', () => {
    ...
    test('saveReport writes JSON and MD files', async () => {
      const engine = new SemanticMergeEngine({
        rootPath: tmpDir,
        storageDir: path.join(tmpDir, '.sinapse', 'merge'),
      });
      ...
      const mergeDir = path.join(tmpDir, '.sinapse', 'merge');
