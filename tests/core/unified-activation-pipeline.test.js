...
jest.mock('../../.sinapse-ai/development/scripts/agent-config-loader', () => ({
  AgentConfigLoader: jest.fn().mockImplementation(() => ({
    loadComplete: jest.fn().mockResolvedValue({
      config: { dataLocation: '.sinapse-ai/data' },
...
jest.mock('../../.sinapse-ai/core/session/context-loader', () => {
...
jest.mock('../../.sinapse-ai/infrastructure/scripts/project-status-loader', () => ({
  loadProjectStatus: jest.fn().mockResolvedValue(mockProjectStatus),
}));

// Mock git config detector
jest.mock('../../.sinapse-ai/infrastructure/scripts/git-config-detector', () => {
  return jest.fn().mockImplementation(() => ({
...
jest.mock('../../.sinapse-ai/core/permissions', () => ({
...
jest.mock('../../.sinapse-ai/core/config/config-resolver', () => ({
...
jest.mock('../../.sinapse-ai/infrastructure/scripts/validate-user-profile', () => ({
...
jest.mock('../../.sinapse-ai/development/scripts/greeting-preference-manager', () => {
  return jest.fn().mockImplementation(() => ({
...
jest.mock('../../.sinapse-ai/core/session/context-detector', () => {
  return jest.fn().mockImplementation(() => ({
...
jest.mock('../../.sinapse-ai/development/scripts/workflow-navigator', () => {
  return jest.fn().mockImplementation(() => ({
...
// Mock config cache
jest.mock('../../.sinapse-ai/core/config/config-cache', () => ({
  globalConfigCache: {
...
jest.mock('../../.sinapse-ai/infrastructure/scripts/performance-tracker', () => ({
  trackConfigLoad: jest.fn(),
}));

// --- Require modules AFTER mocks ---
const { UnifiedActivationPipeline, ALL_AGENT_IDS, LOADER_TIERS, DEFAULT_PIPELINE_TIMEOUT_MS, FALLBACK_PHRASE } = require('../../.sinapse-ai/development/scripts/unified-activation-pipeline');
const { AgentConfigLoader } = require('../../.sinapse-ai/development/scripts/agent-config-loader');
const SessionContextLoader = require('../../.sinapse-ai/core/session/context-loader');
...
describe('UnifiedActivationPipeline', () => {
  ...
  describe('ACT-11: configurable timeout', () => {
    ...
    it('should use env var over config value', () => {
      const originalEnv = process.env.SINAPSE_PIPELINE_TIMEOUT;
      process.env.SINAPSE_PIPELINE_TIMEOUT = '800';
      try {
        const timeout = pipeline._resolvePipelineTimeout({ pipeline: { timeout_ms: 300 } });
        expect(timeout).toBe(800);
      } finally {
        if (originalEnv !== undefined) {
          process.env.SINAPSE_PIPELINE_TIMEOUT = originalEnv;
        } else {
          delete process.env.SINAPSE_PIPELINE_TIMEOUT;
        }
      }
    });
    ...
    it('should ignore invalid env var values', () => {
      const originalEnv = process.env.SINAPSE_PIPELINE_TIMEOUT;
      process.env.SINAPSE_PIPELINE_TIMEOUT = 'not-a-number';
      try {
        const timeout = pipeline._resolvePipelineTimeout({});
        expect(timeout).toBe(DEFAULT_PIPELINE_TIMEOUT_MS);
      } finally {
