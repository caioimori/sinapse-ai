...
const {
  ...
  USER_FIELDS,
  PROJECT_FIELDS,
} = require('../../../.sinapse-ai/core/config/migrate-config');
...
describe('extractProjectFields', () => {
  ...
  it('should extract project.type from nested project object', () => {
    ...
    // Then
    expect(projectFields.project_type).toBe('EXISTING_SINAPSE');
  });
...
describe('migrateConfig', () => {
  ...
  it('should be idempotent: re-running does not duplicate', () => {
    ...
    const userConfigPath = path.join(FAKE_HOME, '.sinapse', 'user-config.yaml');
    setupFileSystem({
      'core-config.yaml': LEGACY_CONFIG_YAML,
...
describe('ensureUserConfigDir', () => {
  it('should create ~/.sinapse/ directory if it does not exist', () => {
    // Given
    fs.existsSync.mockReturnValue(false);
    ...
    expect(fs.mkdirSync).toHaveBeenCalledWith(
      expect.stringContaining('.sinapse'),
