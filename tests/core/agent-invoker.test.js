...
const {
  ...
  SUPPORTED_AGENTS,
  InvocationStatus,
} = require('../../.sinapse-ai/core/orchestration/agent-invoker');
...
describe('Agent Invoker (Story 0.7)', () => {
  ...
  beforeEach(async () => {
    ...
    const agentsDir = path.join(tempDir, '.sinapse-ai', 'development', 'agents');
    ...
    const tasksDir = path.join(tempDir, '.sinapse-ai', 'development', 'tasks');
...
describe('Integration with MasterOrchestrator', () => {
  ...
  it('should integrate AgentInvoker with MasterOrchestrator', async () => {
    const { MasterOrchestrator } = require('../../.sinapse-ai/core/orchestration');
