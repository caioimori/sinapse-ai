...
const {
  ...
  EXTERNAL_TOOLS,
  DEPRECATED_PATTERNS,
} = require('../../../.sinapse-ai/development/scripts/populate-entity-registry');
...
describe('populate-entity-registry (AC: 3, 4, 12)', () => {
  ...
  describe('resolveUsedBy enhanced (NOG-15 AC4)', () => {
    it('creates correct reverse references via name index', () => {
      const entities = {
        tasks: {
          'task-a': {
            path: '.sinapse-ai/development/tasks/task-a.md',
            type: 'task',
            dependencies: ['util-x'],
            usedBy: [],
          },
        ...
        utils: {
          'util-x': {
            path: '.sinapse-ai/core/utils/util-x.js',
            type: 'util',
            dependencies: [],
            usedBy: [],
          },
    ...
    it('resolves by filename when ID does not match', () => {
      const entities = {
        tasks: {
          'my-task': {
            path: '.sinapse-ai/development/tasks/my-task.md',
            type: 'task',
        ...
        scripts: {
          helper: {
            path: '.sinapse-ai/development/scripts/helper.js',
            type: 'script',
            dependencies: [],
            usedBy: [],
          },
  ...
  describe('buildNameIndex() (NOG-16A AC5)', () => {
    it('builds index from entity IDs, filenames, and basenames', () => {
      const entities = {
        tasks: {
          'task-a': { path: '.sinapse-ai/development/tasks/task-a.md', dependencies: [], usedBy: [] },
        },
        scripts: {
          helper: { path: '.sinapse-ai/development/scripts/helper.js', dependencies: [], usedBy: [] },
        },
      };
