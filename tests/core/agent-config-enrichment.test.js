...
describe('Story ACT-8: Agent Config Enrichment', () => {
  ...
  beforeAll(() => {
    agentConfig = loadYaml('.sinapse-ai/data/agent-config-requirements.yaml');
  });
  ...
  describe('7 Required Data Files Exist on Disk', () => {
    const requiredFiles = [
      ...
      { path: '.sinapse-ai/data/technical-preferences.md', owner: '@architect' },
      { path: '.sinapse-ai/product/data/test-levels-framework.md', owner: '@qa' },
      { path: '.sinapse-ai/product/data/test-priorities-matrix.md', owner: '@qa' },
      { path: '.sinapse-ai/product/data/brainstorming-techniques.md', owner: '@analyst' },
      { path: '.sinapse-ai/product/data/elicitation-methods.md', owner: '@po' },
    ];
  ...
  describe('Enriched Agent: @analyst', () => {
    test('analyst now loads brainstorming-techniques, tech-stack, and source-tree', () => {
      ...
      expect(paths).toContain('.sinapse-ai/product/data/brainstorming-techniques.md');
      expect(paths).toContain('docs/framework/tech-stack.md');
  ...
  describe('Enriched Agent: @sm', () => {
    test('sm now loads mode-selection, workflow-patterns, and coding-standards', () => {
      ...
      expect(paths).toContain('.sinapse-ai/product/data/mode-selection-best-practices.md');
      expect(paths).toContain('.sinapse-ai/data/workflow-patterns.yaml');
      expect(paths).toContain('docs/framework/coding-standards.md');
    });
...
describe('Story ACT-8: Document Governance', () => {
  ...
  describe('Governance Task', () => {
    test('update-source-tree.md task file exists', () => {
      expect(fileExists('.sinapse-ai/development/tasks/update-source-tree.md')).toBe(true);
    });

    test('task file contains expected sections', () => {
      const content = readFile('.sinapse-ai/development/tasks/update-source-tree.md');
