...
describe('TerminalSpawner', () => {
  ...
  describe('createContextFile', () => {
    ...
    test('should create valid JSON file', async () => {
      ...
      try {
        expect(contextPath).toContain('sinapse-context-');
  ...
  describe('pollForOutput', () => {
    test('should return output when lock file is missing', async () => {
      ...
      const outputPath = path.join(tmpDir, `sinapse-output-${timestamp}.md`);
    ...
    test('should return "No output captured" if file does not exist and no lock', async () => {
      const fakePath = path.join(tmpDir, `sinapse-output-nonexistent-${Date.now()}.md`);
    ...
    test('should timeout when lock file persists', async () => {
      ...
      const lockPath = path.join(tmpDir, `sinapse-lock-${timestamp}.md`);
    ...
    test('should return output after lock is removed', async () => {
      const timestamp = Date.now();
      const outputPath = path.join(tmpDir, `sinapse-output-${timestamp}.md`);
      const lockPath = path.join(tmpDir, `sinapse-lock-${timestamp}.lock`);
  ...
  describe('cleanupOldFiles', () => {
    test('should clean old SINAPSE temp files', async () => {
      // Create old files (mock old timestamp by naming)
      const oldTimestamp = Date.now() - 7200000; // 2 hours ago
      const oldOutputPath = path.join(tmpDir, `sinapse-output-${oldTimestamp}.md`);
      const oldLockPath = path.join(tmpDir, `sinapse-lock-${oldTimestamp}.lock`);
      const oldContextPath = path.join(tmpDir, `sinapse-context-${oldTimestamp}.json`);
    ...
    test('should not clean recent files', async () => {
      ...
      const recentPath = path.join(tmpDir, `sinapse-output-${recentTimestamp}.md`);
