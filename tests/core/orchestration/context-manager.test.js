...
const ContextManager = require('../../../.sinapse-ai/core/orchestration/context-manager');
...
describe('ContextManager', () => {
  ...
  describe('constructor', () => {
    ...
    test('calcula stateDir como .sinapse/workflow-state dentro do projectRoot', () => {
      expect(manager.stateDir).toBe(path.join(PROJECT_ROOT, '.sinapse', 'workflow-state'));
    });

    test('calcula statePath com o workflowId no nome do arquivo', () => {
      expect(manager.statePath).toBe(
        path.join(PROJECT_ROOT, '.sinapse', 'workflow-state', `${WORKFLOW_ID}.json`),
      );
    });

    test('calcula handoffDir dentro do stateDir', () => {
      expect(manager.handoffDir).toBe(
        path.join(PROJECT_ROOT, '.sinapse', 'workflow-state', 'handoffs'),
      );
    });

    test('calcula confidenceDir dentro do stateDir', () => {
      expect(manager.confidenceDir).toBe(
        path.join(PROJECT_ROOT, '.sinapse', 'workflow-state', 'confidence'),
      );
    });
  ...
  describe('_resolveConfidenceThreshold', () => {
    const originalEnv = process.env.SINAPSE_DELIVERY_CONFIDENCE_THRESHOLD;

    afterEach(() => {
      if (originalEnv !== undefined) {
        process.env.SINAPSE_DELIVERY_CONFIDENCE_THRESHOLD = originalEnv;
      } else {
        delete process.env.SINAPSE_DELIVERY_CONFIDENCE_THRESHOLD;
      }
    });
    ...
    test('retorna valor numérico da env quando é um número válido', () => {
      process.env.SINAPSE_DELIVERY_CONFIDENCE_THRESHOLD = '85';

      expect(manager._resolveConfidenceThreshold()).toBe(85);
    });

    test('retorna 70 quando env contém valor não numérico', () => {
      process.env.SINAPSE_DELIVERY_CONFIDENCE_THRESHOLD = 'invalid';

      expect(manager._resolveConfidenceThreshold()).toBe(70);
    });

    test('retorna 0 quando env é string vazia (Number("") === 0)', () => {
      process.env.SINAPSE_DELIVERY_CONFIDENCE_THRESHOLD = '';
    ...
    test('aceita valores decimais', () => {
      process.env.SINAPSE_DELIVERY_CONFIDENCE_THRESHOLD = '75.5';
