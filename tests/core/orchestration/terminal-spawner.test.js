...
const {
  ...
  POLL_INTERVAL_MS,
  MAX_RETRIES,
} = require('../../../.sinapse-ai/core/orchestration/terminal-spawner');

// Test fixtures
const TEST_OUTPUT_DIR = path.join(os.tmpdir(), 'sinapse-terminal-spawner-test');
...
describe('Terminal Spawner (Story 12.10)', () => {
  ...
  describe('Timeout handling (Task 3.2)', () => {
    it('should timeout if lock file persists', async () => {
      // Given
      const outputFile = path.join(TEST_OUTPUT_DIR, 'sinapse-output-timeout-test.md');
    ...
    it('should return output when lock is removed', async () => {
      // Given
      const outputFile = path.join(TEST_OUTPUT_DIR, 'sinapse-output-success-test.md');
      const lockFile = outputFile.replace('output', 'lock');
  ...
  describe('Lock cleanup (Task 3.3, 3.4)', () => {
    ...
    it('should cleanup old files', async () => {
      ...
      const oldFile = path.join(TEST_OUTPUT_DIR, 'sinapse-output-old.md');
