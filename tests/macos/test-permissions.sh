...
LOG_FILE="/tmp/sinapse-test-perms-$(date +%Y%m%d-%H%M%S).log"
...
test_script_permissions() {
    ...
    # Find SINAPSE scripts
    if [[ -d "$HOME/.sinapse/bin" ]]; then
        SCRIPTS=$(find "$HOME/.sinapse/bin" -type f -name "*.sh" 2>/dev/null || true)
...
test_config_permissions() {
    ...
    CONFIG_FILES=(
        "$HOME/.sinapse/config.json"
        "$HOME/.sinapse/.sinapserc"
    )
...
test_directory_permissions() {
    ...
    if [[ -d "$HOME/.sinapse" ]]; then
        DIR_PERMS=$(stat -f "%Lp" "$HOME/.sinapse" 2>/dev/null || stat -c "%a" "$HOME/.sinapse" 2>/dev/null)
...
test_no_sudo_required() {
    log_info "Test 4: Verifying no sudo required for SINAPSE operations..."

    # Check if current user owns .sinapse directory
    if [[ -d "$HOME/.sinapse" ]]; then
        OWNER=$(stat -f "%Su" "$HOME/.sinapse" 2>/dev/null || stat -c "%U" "$HOME/.sinapse" 2>/dev/null)

        if [[ "$OWNER" == "$(whoami)" ]]; then
            pass_test "User owns .sinapse directory (no sudo needed)"
        else
            fail_test ".sinapse owned by: $OWNER (expected: $(whoami))"
        fi
    fi
}
