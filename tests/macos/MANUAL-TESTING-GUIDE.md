...
This guide provides step-by-step instructions for manually testing SINAPSE on macOS (Intel and Apple Silicon).
...
```bash
# Backup existing SINAPSE installation (if any)
if [ -d "$HOME/.sinapse" ]; then
    mv "$HOME/.sinapse" "$HOME/.sinapse.backup.$(date +%Y%m%d-%H%M%S)"
fi

# Verify backup
ls -la "$HOME/.sinapse.backup"*
```
...
```bash
npx @sinapse/sinapse@latest init
```
...
```bash
# Check SINAPSE command availability
which sinapse
# Expected: /usr/local/bin/sinapse or similar
...
```bash
# Check SINAPSE installation path
which sinapse
# Should use forward slashes: /usr/local/bin/sinapse
# NOT backslashes: \usr\local\bin\sinapse
```
...
cd ~/.sinapse
pwd
# Should show full path: /Users/username/.sinapse
```

#### 3. Test Symlinks

```bash
# Check if sinapse command is a symlink
ls -la $(which sinapse)
...
```bash
# Check line endings of SINAPSE config
file ~/.sinapse/config.json
# Should show: ASCII text (LF line endings)
# NOT: ASCII text, with CRLF line endings

# Check for carriage returns
grep -r $'\r' ~/.sinapse/ || echo "No CRLF found - PASS"
```
...
ls -la ~/.sinapse/bin/*.sh 2>/dev/null
# Should show: -rwxr-xr-x (executable)

# Try running a script without chmod
~/.sinapse/bin/some-script.sh
...
time npx @sinapse/sinapse@latest init
...
- [ ] No "unidentified developer" warnings for SINAPSE
...
# Simulate corruption
echo "corrupted" > ~/.sinapse/config.json

# Try to recover
mv ~/.sinapse ~/.sinapse.broken
mv ~/.sinapse.backup ~/.sinapse
...
- Permission denied: `chmod 000 ~/.sinapse`
...
npx @sinapse/sinapse@latest init --verbose
```

#### Command Not Found

**Symptom:** `bash: sinapse: command not found`
...
# Check PATH
echo $PATH | grep -o '/[^:]*sinapse[^:]*'

# Manually add to PATH if needed
export PATH="$HOME/.sinapse/bin:$PATH"
```
...
chmod +x ~/.sinapse/bin/*.sh

# Fix ownership
sudo chown -R $(whoami) ~/.sinapse
...
**Symptom:** `sinapse health` shows unhealthy MCPs

**Solution:**

```bash
# Reinstall specific MCP
sinapse mcp reinstall <mcp-name>
...
2. **Test Logs:**
   - All files from `/tmp/sinapse-test-*.log`
