...
describe('code-intel hook-runtime', () => {
  ...
  describe('resolveCodeIntel', () => {
    ...
    it('handles absolute paths by normalizing to relative', async () => {
      const absPath = path.join(PROJECT_ROOT, '.sinapse-ai', 'development', 'tasks', 'create-next-story.md');
  ...
  describe('formatAsXml', () => {
    ...
    it('generates valid XML with entity data', () => {
      const intel = {
        entity: {
          file: '.sinapse-ai/core/some-module.js',
      ...
      expect(xml).toContain('<path>.sinapse-ai/core/some-module.js</path>');
