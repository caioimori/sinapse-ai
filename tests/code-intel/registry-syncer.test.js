...
const { RegistrySyncer, inferRole, ROLE_MAP } = require('../../.sinapse-ai/core/code-intel/registry-syncer');
    ...
    RegistryLoader: jest.fn().mockImplementation(() => ({
      load: jest.fn().mockReturnValue({
        ...
        entities: {
          ...
          scripts: {
            'greeting-builder': {
              path: '.sinapse-ai/development/scripts/greeting-builder.js',
              type: 'script',
...
function createMockClient(overrides = {}) {
  return {
    findReferences: jest.fn().mockResolvedValue([
      { file: '.sinapse-ai/development/tasks/create-next-story.md' },
    ]),
...
describe('RegistrySyncer', () => {
  ...
  describe('usedBy population (T2 — AC2)', () => {
    ...
    it('should deduplicate usedBy entries', async () => {
      const mockClient = createMockClient({
        findReferences: jest.fn().mockResolvedValue([
          { file: '.sinapse-ai/development/tasks/create-next-story.md' },
          { file: '.sinapse-ai/development/tasks/create-next-story.md' }, // Duplicate
        ]),
      });
      const syncer = createSyncer({ client: mockClient });
      ...
      const entities = {
        tasks: {
          'test-entity': {
            path: '.sinapse-ai/development/tasks/test.md',
            usedBy: [],
            dependencies: [],
          },
        },
  ...
  describe('codeIntelMetadata schema (T4 — AC4)', () => {
    ...
    it('should set callerCount based on usedBy length', async () => {
      const mockClient = createMockClient({
        findReferences: jest.fn().mockResolvedValue([
          { file: '.sinapse-ai/development/tasks/create-next-story.md' },
          { file: '.sinapse-ai/development/scripts/greeting-builder.js' },
        ]),
      });
      const syncer = createSyncer({ client: mockClient });
    ...
    it('should set provider from active provider name', async () => {
      ...
      const entity = {
        id: 'test',
        category: 'tasks',
        data: { path: '.sinapse-ai/development/tasks/test.md', usedBy: [], dependencies: [] },
      };
  ...
  describe('inferRole', () => {
    it('should infer role from path patterns', () => {
      ...
      expect(inferRole('.sinapse-ai/data/entity-registry.yaml')).toBe('config');
      expect(inferRole('.sinapse-ai/product/templates/prd.yaml')).toBe('template');
    });
