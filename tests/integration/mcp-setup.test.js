...
describe('MCP Setup Integration', () => {
  const cliIndexPath = path.join(__dirname, '../../.sinapse-ai/cli/index.js');
  let tempDir;
  let originalHome;
  ...
  describe('CLI Module Loading', () => {
    it('should load mcp command module without errors', () => {
      expect(() => {
        require('../../.sinapse-ai/cli/commands/mcp');
      }).not.toThrow();
    });

    it('should export createMcpCommand function', () => {
      const mcpModule = require('../../.sinapse-ai/cli/commands/mcp');
      expect(typeof mcpModule.createMcpCommand).toBe('function');
    });

    it('should load all mcp subcommand modules', () => {
      expect(() => {
        require('../../.sinapse-ai/cli/commands/mcp/setup');
        require('../../.sinapse-ai/cli/commands/mcp/link');
        require('../../.sinapse-ai/cli/commands/mcp/status');
        require('../../.sinapse-ai/cli/commands/mcp/add');
      }).not.toThrow();
    });
  });
  ...
  describe('Core Module Loading', () => {
    it('should load os-detector module', () => {
      const osDetector = require('../../.sinapse-ai/core/mcp/os-detector');

      expect(osDetector.detectOS).toBeDefined();
      expect(osDetector.isWindows).toBeDefined();
      expect(osDetector.getGlobalSinapseDir).toBeDefined();
      expect(osDetector.getGlobalMcpDir).toBeDefined();
    });

    it('should load global-config-manager module', () => {
      const configManager = require('../../.sinapse-ai/core/mcp/global-config-manager');
    ...
    it('should load symlink-manager module', () => {
      const symlinkManager = require('../../.sinapse-ai/core/mcp/symlink-manager');
    ...
    it('should load config-migrator module', () => {
      const configMigrator = require('../../.sinapse-ai/core/mcp/config-migrator');
    ...
    it('should load mcp index module with all exports', () => {
      const mcp = require('../../.sinapse-ai/core/mcp');
  ...
  describe('OS Detection', () => {
    const { detectOS, isWindows, isMacOS, isLinux, getOSInfo } = require('../../.sinapse-ai/core/mcp/os-detector');
  ...
  describe('Server Templates', () => {
    const { getAvailableTemplates, getServerTemplate, SERVER_TEMPLATES } = require('../../.sinapse-ai/core/mcp/global-config-manager');
  ...
  describe('Path Generation', () => {
    ...
    it('should generate correct global SINAPSE directory path', () => {
      const sinapseDir = getGlobalSinapseDir();

      expect(sinapseDir).toContain('.sinapse');
      expect(path.isAbsolute(sinapseDir)).toBe(true);
    });

    it('should generate correct MCP directory path', () => {
      const mcpDir = getGlobalMcpDir();

      expect(mcpDir).toContain('.sinapse');
  ...
  describe('Link Status Constants', () => {
    const { LINK_STATUS } = require('../../.sinapse-ai/core/mcp/symlink-manager');
  ...
  describe('Migration Options Constants', () => {
    const { MIGRATION_OPTION } = require('../../.sinapse-ai/core/mcp/config-migrator');
  ...
  describe('Default Config Structure', () => {
    const { DEFAULT_CONFIG } = require('../../.sinapse-ai/core/mcp/global-config-manager');
  ...
  describe('CLI Help Output', () => {
    const { createMcpCommand } = require('../../.sinapse-ai/cli/commands/mcp');
...
describe('MCP CLI Command Registration', () => {
  it('should have mcp command in main CLI', () => {
    const { createProgram } = require('../../.sinapse-ai/cli/index');
