...
describeIntegration('Unified Greeting System Integration', () => {
      ...
      it(`should generate greeting for ${agentId} agent`, async function() {
        ...
        try {
          const { stdout, stderr } = await execPromise(
            `node .sinapse-ai/development/scripts/generate-greeting.js ${agentId}`,
          );
  ...
  describeIntegration('Performance Validation', () => {
    it('should complete within target time', async function() {
      ...
      try {
        await execPromise('node .sinapse-ai/development/scripts/generate-greeting.js qa');
  ...
  describeIntegration('Agent Configuration Loading', () => {
    it('should load complete agent definition', async () => {
      ...
      const coreConfig = yaml.load(
        fs.readFileSync('.sinapse-ai/core-config.yaml', 'utf8'),
      );
    ...
    it('should provide fallback greeting on failure', async function() {
      ...
      try {
        const { stdout } = await execPromise(
          'node .sinapse-ai/development/scripts/generate-greeting.js nonexistent-agent 2>&1',
        );
...
if (require.main === module) {
  ...
  console.log('This requires the full SINAPSE environment.\n');
  
  const tests = [
    {
      name: 'Generate greeting for QA agent',
      fn: async () => {
        const { stdout } = await execPromise('node .sinapse-ai/development/scripts/generate-greeting.js qa 2>&1');
    ...
    },
    {
      ...
      fn: async () => {
        const { stdout } = await execPromise('node .sinapse-ai/development/scripts/generate-greeting.js dev 2>&1');
    ...
    },
    {
      ...
      fn: async () => {
        ...
        await execPromise('node .sinapse-ai/development/scripts/generate-greeting.js qa 2>&1');
