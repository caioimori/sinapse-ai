...
describe('Pattern Learning Integration', () => {
  ...
  beforeAll(() => {
    learningModule = require('../../../.sinapse-ai/workflow-intelligence/learning');
  });
  ...
  describe('Capture Hook Integration', () => {
    ...
    beforeAll(() => {
      captureHook = require('../../../.sinapse-ai/workflow-intelligence/learning/capture-hook');
    });
    ...
    it('should handle disabled state gracefully', async () => {
      // Save original env
      const originalEnv = process.env.SINAPSE_PATTERN_CAPTURE;

      try {
        process.env.SINAPSE_PATTERN_CAPTURE = 'false';
        captureHook.reset();
        ...
        if (originalEnv !== undefined) {
          process.env.SINAPSE_PATTERN_CAPTURE = originalEnv;
        } else {
          delete process.env.SINAPSE_PATTERN_CAPTURE;
        }
        captureHook.reset();
      }
    });
