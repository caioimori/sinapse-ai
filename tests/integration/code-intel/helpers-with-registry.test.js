...
} = require('../../../.sinapse-ai/core/code-intel');

const {
  checkBeforeWriting,
  suggestReuse,
} = require('../../../.sinapse-ai/core/code-intel/helpers/dev-helper');

const {
  getBlastRadius,
  getReferenceImpact,
} = require('../../../.sinapse-ai/core/code-intel/helpers/qa-helper');

const {
  getDependencyGraph,
  getCodebaseOverview,
} = require('../../../.sinapse-ai/core/code-intel/helpers/planning-helper');

const {
  suggestRelevantFiles,
} = require('../../../.sinapse-ai/core/code-intel/helpers/story-helper');
...
describe('AC8: Helper Functions Return Real Data with RegistryProvider', () => {
  ...
  describe('qa-helper.js', () => {
    test('getBlastRadius() returns real data for known files', async () => {
      const files = ['.sinapse-ai/development/tasks/create-story.md'];
  ...
  describe('AC13: Helper null-rate < 30%', () => {
    test('at least 5/7 helpers return non-null data', async () => {
      const results = await Promise.all([
        ...
        getBlastRadius(['.sinapse-ai/development/tasks/create-next-story.md']).then(r => ({ name: 'getBlastRadius', result: r })),
