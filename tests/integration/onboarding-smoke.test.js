/**
 * Story SINAPSE-DIFF-4.0.5: Onboarding smoke tests in clean environment.
 *
 * Objective:
...
describe('Onboarding smoke flow (SINAPSE-DIFF-4.0.5)', () => {
  const repoRoot = path.resolve(__dirname, '..', '..');
  const cliBin = path.join(repoRoot, 'bin', 'sinapse.js');
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'sinapse-onboarding-smoke-'));
  });
  ...
  it('validates Start Here commands are discoverable and executable from clean env', async () => {
    expect(await fs.pathExists(path.join(tempDir, '.sinapse-ai'))).toBe(false);
