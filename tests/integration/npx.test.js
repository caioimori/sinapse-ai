 ...
 * Tests for npx sinapse-ai@latest execution
 */
...
describeIntegration('npx Execution', () => {
  ...
  describeIntegration('Package Configuration', () => {
    it('should have correct bin entries in package.json', () => {
      ...
      expect(packageJson.bin['sinapse']).toBe('./bin/sinapse.js');
      expect(packageJson.bin['sinapse-ai']).toBe('./bin/sinapse.js');
    });
    ...
    it('should include required files in package', () => {
      ...
      // Note: .sinapse-ai/ should be included if it exists
      const hasSinapseCore = packageJson.files.some(f => 
        f === '.sinapse-ai/' || f === 'sinapse-ai/',
      );
      expect(hasSinapseCore).toBe(true);
    });
  ...
  describeIntegration('CLI Execution', () => {
    it('should execute sinapse command with --version', (done) => {
      const cliPath = path.join(__dirname, '../../bin/sinapse.js');
    ...
    it('should execute sinapse command with --help', (done) => {
      const cliPath = path.join(__dirname, '../../bin/sinapse.js');
      ...
      child.on('close', (code) => {
        ...
        expect(output).toContain('sinapse-ai');
        done();
      });
    }, timeout);

    it('should execute sinapse info command', (done) => {
      const cliPath = path.join(__dirname, '../../bin/sinapse.js');
  ...
  describeIntegration('Index.js init Export', () => {
    ...
    it('should maintain SINAPSE class export in source', () => {
      const indexPath = path.join(__dirname, '../../index.js');
      const indexContent = fs.readFileSync(indexPath, 'utf8');

      // Verify SINAPSE class exists in exports
      expect(indexContent).toContain('SINAPSE');
      expect(indexContent).toContain('class SINAPSE');
    });
