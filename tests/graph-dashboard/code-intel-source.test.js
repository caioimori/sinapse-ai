...
jest.mock('../../.sinapse-ai/core/code-intel', () => ({
  getClient: () => mockClient,
  isCodeIntelAvailable: () => mockIsAvailable,
}));

jest.mock('../../.sinapse-ai/core/ids/registry-loader', () => ({
  RegistryLoader: jest.fn().mockImplementation(() => ({
    load: () => mockRegistryData,
  })),
}));

const { CodeIntelSource, _classifyScript, _detectCategory } = require('../../.sinapse-ai/core/graph-dashboard/data-sources/code-intel-source');
...
describe('CodeIntelSource', () => {
  ...
  describe('_classifyScript', () => {
    it('should classify development scripts as scripts/task', () => {
      expect(_classifyScript('.sinapse-ai/development/scripts/build.js')).toBe('scripts/task');
    });

    it('should classify core scripts as scripts/engine', () => {
      expect(_classifyScript('.sinapse-ai/core/graph-dashboard/cli.js')).toBe('scripts/engine');
    });

    it('should classify infrastructure scripts as scripts/infra', () => {
      expect(_classifyScript('.sinapse-ai/infrastructure/ci/deploy.js')).toBe('scripts/infra');
    });
  ...
  describe('_detectCategory', () => {
    it('should detect checklists from path', () => {
      expect(_detectCategory('other', '.sinapse-ai/development/checklists/pre-push.md')).toBe('checklists');
    });

    it('should detect workflows from path', () => {
      expect(_detectCategory('other', '.sinapse-ai/development/workflows/deploy.yaml')).toBe('workflows');
    });

    it('should detect utils from path', () => {
      expect(_detectCategory('other', '.sinapse-ai/utils/helper.js')).toBe('utils');
    });

    it('should detect data from path', () => {
      expect(_detectCategory('other', '.sinapse-ai/data/entity-registry.yaml')).toBe('data');
    });

    it('should detect tools from path', () => {
      expect(_detectCategory('other', '.sinapse-ai/development/tools/coderabbit.md')).toBe('tools');
    });

    it('should subcategorize scripts by directory', () => {
      expect(_detectCategory('scripts', '.sinapse-ai/development/scripts/build.js')).toBe('scripts/task');
      expect(_detectCategory('scripts', '.sinapse-ai/core/code-intel/index.js')).toBe('scripts/engine');
      expect(_detectCategory('scripts', '.sinapse-ai/infrastructure/ci/lint.js')).toBe('scripts/infra');
    });
  ...
  describe('getData - category detection in registry', () => {
    it('should apply _detectCategory to registry entities', async () => {
      mockRegistryData = {
        ...
        entities: {
          scripts: {
            'build-script': { path: '.sinapse-ai/development/scripts/build.js', type: 'script', dependencies: [] },
            'engine-script': { path: '.sinapse-ai/core/graph-dashboard/cli.js', type: 'script', dependencies: [] },
          },
          other: {
            'my-checklist': { path: '.sinapse-ai/development/checklists/pre-push.md', type: 'checklist', dependencies: [] },
          },
        },
      };
  ...
  describe('getData - lifecycle field in registry nodes', () => {
    it('should include lifecycle from registry entity data', async () => {
      mockRegistryData = {
        ...
        entities: {
          ...
          tasks: {
            'orphan-task': { path: '.sinapse-ai/tasks/orphan.md', type: 'task', lifecycle: 'orphan', dependencies: [], usedBy: [] },
          },
        },
      };
    ...
    it('should default lifecycle to production when missing', async () => {
      mockRegistryData = {
        ...
        entities: {
          agents: {
            'no-lifecycle': { path: '.sinapse-ai/agents/test.md', type: 'agent', dependencies: [], usedBy: [] },
          },
        },
      };
