...
const { parseArgs, handleWatch, WATCH_FORMAT_MAP } = require('../../.sinapse-ai/core/graph-dashboard/cli');

describe('watch-mode', () => {
  let consoleLogSpy;
  let consoleErrorSpy;
  let watchState;
  const testOutputDir = path.resolve(process.cwd(), '.sinapse');
  ...
  describe('handleWatch lifecycle', () => {
    ...
    it('should create .sinapse directory if it does not exist', async () => {
      // .sinapse should be created by handleWatch
      watchState = await handleWatch({ format: 'dot', interval: 5 });
