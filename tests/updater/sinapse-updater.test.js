...
describe('SINAPSEUpdater', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'sinapse-updater-test-'));

    // Create minimal .sinapse-ai structure
    await fs.ensureDir(path.join(tempDir, '.sinapse-ai'));
    await fs.ensureDir(path.join(tempDir, '.sinapse'));

    // Create version.json
    await fs.writeJson(path.join(tempDir, '.sinapse-ai', 'version.json'), {
      version: '1.0.0',
    ...
    updater = new SINAPSEUpdater(tempDir, { verbose: false });
  });
  ...
  describe('constructor', () => {
    it('should initialize with default options', () => {
      const u = new SINAPSEUpdater(tempDir);
  ...
  describe('getInstalledVersion', () => {
    ...
    it('should return null if version.json not found', async () => {
      await fs.remove(path.join(tempDir, '.sinapse-ai', 'version.json'));
      const version = await updater.getInstalledVersion();
      expect(version).toBeNull();
    });

    it('should handle corrupted version.json', async () => {
      await fs.writeFile(path.join(tempDir, '.sinapse-ai', 'version.json'), 'invalid json');
      const version = await updater.getInstalledVersion();
      expect(version).toBeNull();
    });
  });
  ...
  describe('detectCustomizations', () => {
    it('should detect unchanged files', async () => {
      ...
      const testFile = path.join(tempDir, '.sinapse-ai', 'test-file.md');
  ...
  describe('rollback', () => {
    ...
    it('should restore files from backup', async () => {
      ...
      // Verify restored
      const restored = await fs.readJson(path.join(tempDir, '.sinapse-ai', 'version.json'));
  ...
  describe('updateVersionInfo', () => {
    it('should update version.json with new version', async () => {
      ...
      const versionJson = await fs.readJson(path.join(tempDir, '.sinapse-ai', 'version.json'));
