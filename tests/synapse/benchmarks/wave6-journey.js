...
const { UnifiedActivationPipeline } = require(
  path.join(PROJECT_ROOT, '.sinapse-ai', 'development', 'scripts', 'unified-activation-pipeline.js'),
);
const { collectTimingMetrics } = require(
  path.join(PROJECT_ROOT, '.sinapse-ai', 'core', 'synapse', 'diagnostics', 'collectors', 'timing-collector.js'),
);
const { safeReadJson } = require(
  path.join(PROJECT_ROOT, '.sinapse-ai', 'core', 'synapse', 'diagnostics', 'collectors', 'safe-read-json.js'),
);
...
function checkCodeIntel() {
  try {
    const codeIntel = require(path.join(PROJECT_ROOT, '.sinapse-ai', 'core', 'code-intel', 'index.js'));
