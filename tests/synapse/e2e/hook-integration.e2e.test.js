...
// .sinapse/ dir may exist but be empty in CI — check for actual runtime content
const SYNAPSE_RUNTIME_READY = fs.existsSync(path.join(PROJECT_ROOT, '.sinapse', 'runtime-capabilities.json'))
  || fs.existsSync(path.join(PROJECT_ROOT, '.sinapse', 'sessions'))
  || (process.env.CI === undefined && fs.existsSync(path.join(PROJECT_ROOT, '.sinapse')));

const describeIfRuntime = SYNAPSE_RUNTIME_READY ? describe : describe.skip;
...
const describeIfHookExists = (HOOK_EXISTS && SYNAPSE_RUNTIME_READY) ? describe : describe.skip;
