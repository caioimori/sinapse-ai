...
// ---------------------------------------------------------------------------
const { SynapseEngine } = require(path.join(PROJECT_ROOT, '.sinapse-ai', 'core', 'synapse', 'engine.js'));
const { parseManifest } = require(path.join(PROJECT_ROOT, '.sinapse-ai', 'core', 'synapse', 'domain', 'domain-loader.js'));
const { estimateTokens } = require(path.join(PROJECT_ROOT, '.sinapse-ai', 'core', 'synapse', 'utils', 'tokens.js'));
const {
  estimateContextPercent,
  calculateBracket,
  getActiveLayers,
  getTokenBudget,
} = require(path.join(PROJECT_ROOT, '.sinapse-ai', 'core', 'synapse', 'context', 'context-tracker.js'));
const GitConfigDetector = require(path.join(PROJECT_ROOT, '.sinapse-ai', 'infrastructure', 'scripts', 'git-config-detector.js'));

let UnifiedActivationPipeline;
try {
  const uapModule = require(path.join(PROJECT_ROOT, '.sinapse-ai', 'development', 'scripts', 'unified-activation-pipeline.js'));
...
try {
  ProjectStatusLoader = require(path.join(PROJECT_ROOT, '.sinapse-ai', 'infrastructure', 'scripts', 'project-status-loader.js'));
} catch (err) {
...
This audit measures every stage of the SINAPSE activation pipeline (UAP + SYNAPSE + Session + Git Detection)
