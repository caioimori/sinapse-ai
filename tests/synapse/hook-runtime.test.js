...
const {
  resolveHookRuntime,
  buildHookOutput,
} = require('../../.sinapse-ai/core/synapse/runtime/hook-runtime');
...
describe('hook-runtime', () => {
  ...
  it('calls cleanStaleSessions on first prompt (prompt_count === 0)', () => {
    ...
    try {
      ...
      const smPath = path.join(cwd, '.sinapse-ai', 'core', 'synapse', 'session', 'session-manager.js');
      const engPath = path.join(cwd, '.sinapse-ai', 'core', 'synapse', 'engine.js');
