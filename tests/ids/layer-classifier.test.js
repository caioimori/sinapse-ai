...
describe('classifyLayer', () => {
  ...
  describe('L1 — Framework Core', () => {
    test('classifies .sinapse-ai/core/ modules as L1', () => {
      expect(classifyLayer('.sinapse-ai/core/ids/index.js')).toBe('L1');
    });

    test('classifies .sinapse-ai/core/ nested modules as L1', () => {
      expect(classifyLayer('.sinapse-ai/core/ids/registry-updater.js')).toBe('L1');
      expect(classifyLayer('.sinapse-ai/core/utils/helpers.js')).toBe('L1');
    });

    test('classifies bin/ executables as L1', () => {
      expect(classifyLayer('bin/sinapse.js')).toBe('L1');
      expect(classifyLayer('bin/sinapse-init.js')).toBe('L1');
    });

    test('classifies constitution.md as L1', () => {
      expect(classifyLayer('.sinapse-ai/constitution.md')).toBe('L1');
    });
  });

  // --- L2: Framework Templates ---
  describe('L2 — Framework Templates', () => {
    test('classifies .sinapse-ai/development/ as L2', () => {
      expect(classifyLayer('.sinapse-ai/development/tasks/create-next-story.md')).toBe('L2');
    });
    ...
    test('classifies .sinapse-ai/infrastructure/ as L2', () => {
      expect(classifyLayer('.sinapse-ai/infrastructure/scripts/deploy.sh')).toBe('L2');
    });

    test('classifies .sinapse-ai/product/ as L2', () => {
      expect(classifyLayer('.sinapse-ai/product/templates/story-tmpl.yaml')).toBe('L2');
    });
  });

  // --- L3: Project Config ---
  describe('L3 — Project Config', () => {
    test('classifies .sinapse-ai/data/ as L3', () => {
      expect(classifyLayer('.sinapse-ai/data/entity-registry.yaml')).toBe('L3');
    });
