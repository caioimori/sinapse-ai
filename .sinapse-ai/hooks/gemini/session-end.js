#!/usr/bin/env node
/**
 * SINAPSE Session End Hook for Gemini CLI
...
async function sessionEnd() {
  ...
  try {
    const sessionDir = path.join(projectDir, '.sinapse', 'sessions');
    ...
    let modifiedFiles = [];
    const sessionFilesPath = path.join(projectDir, '.sinapse', 'session-files.json');
    ...
    let toolCount = 0;
    const toolLogPath = path.join(projectDir, '.sinapse', 'logs', 'tool-results.jsonl');
