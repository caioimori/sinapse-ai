 ...
 * - sinapse-ai: Hook runner (this file) + pro detection
 * - sinapse-pro: Digest extraction logic (memory/session-digest/)
 *
 * @module .sinapse-ai/hooks/unified/runners/precompact-runner
 * @see Story MIS-3 - Session Digest (PreCompact Hook)
 * @see Story PRO-5 - sinapse-pro Repository Bootstrap
 */
 ...
 *
 * Execution flow:
 * 1. Detect sinapse-pro availability via pro-detector
...
async function onPreCompact(context) {
  try {
    // 1. Detect sinapse-pro availability
    const proAvailable = isProAvailable();

    if (!proAvailable) {
      console.log('[PreCompact] sinapse-pro not available, skipping session digest');
    ...
    setImmediate(async () => {
      try {
        // Dynamic import from sinapse-pro
        ...
        if (!digestModule || typeof digestModule.extractSessionDigest !== 'function') {
          console.error('[PreCompact] Digest extractor not found or invalid in sinapse-pro');
          return;
        }
