...
Hook Runners (runners/*.js)
         ↓
Application Logic (sinapse-ai, sinapse-pro)
```
...
- **`precompact-runner.js`** (Story MIS-3)
  ...
  - Open Core architecture (delegates to sinapse-pro)
...
pro/memory/session-digest/extractor.js
         ↓
.sinapse/session-digests/{session-id}-{timestamp}.yaml
```
...
- If sinapse-pro not available: no-op (log and return)
...
```javascript
// .sinapse-ai/hooks/unified/runners/my-runner.js
...
async function main() {
  ...
  const { onMyEvent } = require('../../.sinapse-ai/hooks/unified/runners/my-runner');
...
if (!isProAvailable()) {
  console.log('[Hook] sinapse-pro not available, skipping');
  return;
}
...
- **Story PRO-5:** sinapse-pro Repository Bootstrap (pro-detector pattern)
