 ...
 */
const EVENT_MAPPING = {
  // SINAPSE Event -> { claude: ClaudeEvent, gemini: GeminiEvent }
...
class UnifiedHook {
   ...
   * @param {string} config.event - SINAPSE event name
  ...
  toClaudeConfig() {
    ...
    // Runners not yet implemented - path .sinapse-ai/hooks/unified/runners/ does not exist
    return null;

    // return {
    //   event: claudeEvent,
    //   matcher: this.matcher,
    //   handler: `node .sinapse-ai/hooks/unified/runners/claude-runner.js ${this.name}`,
    // };
  }
  ...
  toGeminiConfig() {
    ...
    //       name: this.name,
    //       type: 'command',
    //       command: `node .sinapse-ai/hooks/unified/runners/gemini-runner.js ${this.name}`,
