...
**Gates de skills/automação (KIT-skills-aplicado):** skill nasce de engenharia reversa de código real (few-shot), nunca à mão · description carrega o gatilho (quando usar/NÃO) · 1 rule = 1 responsabilidade · prefira o determinístico (script idempotente) · prompt Contexto→Objetivos→Regras→Formato→Tarefa · cure o contexto por task.
