...
/**
 * SINAPSE-FullStack Installation Wizard v5 (LEGACY)
 ...
 *
 * Migration path:
 * - Use `npx sinapse-ai` which routes through bin/sinapse.js to the new wizard
...
/**
 * Smart path resolution for SINAPSE Core modules
 */
function resolveSinapseCoreModule(modulePath) {
  const sinapseCoreModule = path.join(__dirname, '..', '.sinapse-ai', modulePath);

  const moduleExists =
    fs.existsSync(sinapseCoreModule + '.js') ||
    fs.existsSync(sinapseCoreModule + '/index.js') ||
    fs.existsSync(sinapseCoreModule);

  if (!moduleExists) {
    throw new Error(
      `Cannot find SINAPSE Core module: ${modulePath}\n` +
        `Searched: ${sinapseCoreModule}\n` +
        'Please ensure sinapse-ai is installed correctly.'
    );
  }

  return require(sinapseCoreModule);
}

// Load SINAPSE Core modules
const { detectRepositoryContext } = resolveSinapseCoreModule(
  'infrastructure/scripts/repository-detector'
);
// PM adapters imported but not used directly (loaded dynamically)
// const { ClickUpAdapter } = resolveSinapseCoreModule('utils/pm-adapters/clickup-adapter');
// const { GitHubProjectsAdapter } = resolveSinapseCoreModule('utils/pm-adapters/github-adapter');
// const { JiraAdapter } = resolveSinapseCoreModule('utils/pm-adapters/jira-adapter');
...
async function main() {
  ...
  if (!context) {
    ...
    if (!fs.existsSync(packageJsonPath)) {
      ...
      const defaultPackage = {
        ...
        version: '1.0.0',
        description: 'SINAPSE-FullStack project',
        main: 'index.js',
  ...
  const installedManifestPath = path.join(projectRoot, '.sinapse-ai', '.installed-manifest.yaml');
  const hasExistingInstall = fs.existsSync(installedManifestPath);

  if (hasExistingInstall && brownfieldUpgrader) {
    console.log(chalk.yellow('🔄 Existing SINAPSE installation detected!'));
    console.log('');

    const sourceDir = path.join(context.frameworkLocation, '.sinapse-ai');
      ...
      if (upgradeChoice === 'upgrade') {
        ...
        if (result.success) {
          ...
          brownfieldUpgrader.updateInstalledManifest(
            projectRoot,
            sourceManifest,
            `sinapse-ai@${packageJson.version}`
          );
  ...
  const { installMode } = await inquirer.prompt([
    {
      type: 'list',
      name: 'installMode',
      message: chalk.white('How are you using SINAPSE-FullStack?'),
      choices: [
        {
          name: '  Using SINAPSE in a project ' + chalk.gray('(Framework files added to .gitignore)'),
          value: 'project-development',
        },
        {
          name:
            '  Developing SINAPSE framework itself ' + chalk.gray('(Framework files are source code)'),
  ...
  const configPath = path.join(context.projectRoot, '.sinapse-installation-config.yaml');
  // INS-2 Performance: Use async write instead of sync
  ...
  // Step 4b: Copy SINAPSE Core files
  console.log('');

  const sourceCoreDir = path.join(context.frameworkLocation, '.sinapse-ai');
  const targetCoreDir = path.join(context.projectRoot, '.sinapse-ai');

  if (fs.existsSync(sourceCoreDir)) {
    // INS-2 Performance: Add spinner for file copy (AC9)
    const copySpinner = ora('Installing SINAPSE Core files...').start();
    await fse.copy(sourceCoreDir, targetCoreDir);
    copySpinner.succeed(
      'SINAPSE Core files installed ' +
    ...
    console.error(chalk.red('✗') + ' SINAPSE Core files not found');
    process.exit(1);
  }
  ...
  if (!ides.includes('none')) {
    ...
    // Step 2: Install SINAPSE CORE agents and tasks for Claude Code
    ...
    if (ides.includes('claude')) {
      ...
      // Create SINAPSE README for Claude Code
      const sinapsesReadme = path.join(
        context.projectRoot,
        '.claude',
        'commands',
        'SINAPSE',
      ...
      await fse.ensureDir(path.dirname(sinapsesReadme));
      await fse.writeFile(
        sinapsesReadme,
        `# SINAPSE Core Commands

This directory contains the core SINAPSE-FullStack agents and tasks.
...
`
  ...
  const possibleSquadsDirs = [
    ...
    path.join(context.projectRoot, 'node_modules', 'sinapse-ai', 'squads'),
    path.join(context.projectRoot, 'node_modules', '@sinapse', 'fullstack', 'squads'),
  ];
  ...
  try {
    ...
    const validator = new PostInstallValidator(context.projectRoot, context.frameworkLocation, {
      ...
      // For production integrity checks, users should run `sinapse validate` which
    ...
    if (
      ...
      console.log(
        chalk.yellow('   Run ') +
          chalk.cyan('sinapse validate --repair') +
    ...
    console.log(chalk.dim('   Run `sinapse validate` to check installation integrity'));
  }
  ...
  console.log(chalk.gray('═'.repeat(80)));
  console.log('');
  console.log(chalk.green.bold('✓ SINAPSE-FullStack installation complete! 🎉'));
  console.log('');
  ...
  console.log('  ' + chalk.dim('.sinapse-ai/') + '           - Framework core files');

  if (ides.includes('claude')) {
    console.log('  ' + chalk.dim('.claude/'));
    console.log('    ' + chalk.dim('├─ CLAUDE.md') + '        - Main configuration');
    console.log('    ' + chalk.dim('└─ commands/'));
    console.log('      ' + chalk.dim('  ├─ SINAPSE/') + '         - Core agents & tasks');
  ...
  console.log('    • Run ' + chalk.yellow('sinapse validate') + ' to verify installation integrity');
  console.log('    • Run ' + chalk.yellow('sinapse validate --repair') + ' to fix any missing files');
  console.log('    • Check .sinapse-ai/user-guide.md for complete documentation');
...
async function updateGitIgnore(mode, projectRoot) {
  ...
  if (mode === 'project-development') {
    const frameworkRules = [
      '',
      '# SINAPSE-FullStack Framework Files (auto-managed - do not edit)',
      '.sinapse-ai/',
      'node_modules/@sinapse/',
      'outputs/minds/',
      '.sinapse-installation-config.yaml',
      '# End SINAPSE-FullStack auto-managed section',
      '',
    ];

    const hasFrameworkSection = gitignore.includes('# SINAPSE-FullStack Framework Files');
...
async function savePMConfig(pmTool, config, projectRoot) {
  ...
  const configPath = path.join(projectRoot, '.sinapse-pm-config.yaml');
  // INS-2 Performance: Use async write
