 ...
 * for SINAPSE installer failures.
 *
...
const ERROR_MESSAGES = {
  ...
  UNKNOWN_ERROR: {
    ...
    recovery: [
      'Check the installation log: .sinapse-install.log',
...
function formatErrorMessage(error, errorCode = null) {
  ...
  lines.push('');
  lines.push(chalk.gray('Installation log: .sinapse-install.log'));
  lines.push('');
...
function formatRollbackMessage(success, failedFiles = []) {
  ...
  if (success) {
    ...
    lines.push(chalk.cyan('  1. Check backup directory: .sinapse-backup/'));
    lines.push(chalk.cyan('  2. Manually restore failed files from backups'));
    lines.push(chalk.cyan('  3. Review installation log: .sinapse-install.log'));
  }
...
function formatSuccessMessage() {
  ...
  lines.push(chalk.green('═══════════════════════════════════════════════════════'));
  lines.push('');
  lines.push(chalk.white('SINAPSE has been installed and configured.'));
  lines.push('');
  lines.push(chalk.cyan.bold('Next Steps:'));
  lines.push(chalk.cyan('  1. Review installation log: .sinapse-install.log'));
  lines.push(chalk.cyan('  2. Verify configuration: sinapse config --check'));
  lines.push(chalk.cyan('  3. Run validation: sinapse validate'));
  lines.push('');
  return lines.join('\n');
}
