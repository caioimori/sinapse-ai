/**
 * SINAPSE-FullStack MCP Installation Module
...
async function installProjectMCPs(options = {}) {
  ...
  // Create .sinapse directory for logs
  const sinapseDir = path.join(projectPath, '.sinapse');
  await fse.ensureDir(sinapseDir);

  const logPath = path.join(sinapseDir, 'install-log.txt');
  const errorLogPath = path.join(sinapseDir, 'install-errors.log');
...
function displayInstallationStatus(results) {
  ...
  console.log(chalk.gray('📋 Installation log: .sinapse/install-log.txt'));
  console.log('');
}
