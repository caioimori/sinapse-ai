/**
 * SINAPSE-FullStack Environment Configuration Module
...
async function generateCoreConfigYAML(projectPath, wizardState) {
  try {
    const sinapseCoreDir = path.join(projectPath, '.sinapse-ai');
    const yamlPath = path.join(sinapseCoreDir, 'core-config.yaml');

    // Ensure .sinapse-ai directory exists
    await fs.ensureDir(sinapseCoreDir);
