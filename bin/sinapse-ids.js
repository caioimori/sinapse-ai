...
/**
 * SINAPSE IDS CLI — Incremental Development System
 * Standalone (no external dependencies for npx compatibility)
 *
 * Commands:
 *   sinapse ids:query {intent}     — Query registry for matching artifacts
 *   sinapse ids:create-review      — Review CREATE decisions for promotion/deprecation
 *   sinapse ids:health             — Run registry health check (self-healing)
 *   sinapse ids:check {intent}     — Pre-check registry (FrameworkGovernor advisory)
 *   sinapse ids:impact {entity-id} — Impact analysis for modifications
 *   sinapse ids:stats              — Registry statistics
 *   sinapse ids:register {path}    — Register entity after creation
 *
 * Flags:
...
const { RegistryLoader } = require(path.resolve(__dirname, '..', '.sinapse-ai', 'core', 'ids', 'registry-loader'));
const { IncrementalDecisionEngine } = require(path.resolve(__dirname, '..', '.sinapse-ai', 'core', 'ids', 'incremental-decision-engine'));
const { RegistryUpdater } = require(path.resolve(__dirname, '..', '.sinapse-ai', 'core', 'ids', 'registry-updater'));
const { FrameworkGovernor } = require(path.resolve(__dirname, '..', '.sinapse-ai', 'core', 'ids', 'framework-governor'));

// Optional: RegistryHealer (IDS-4a — may not exist yet)
let RegistryHealer = null;
try {
  RegistryHealer = require(path.resolve(__dirname, '..', '.sinapse-ai', 'core', 'ids', 'registry-healer')).RegistryHealer;
} catch (_err) {
...
function showHelp() {
  console.log(`
SINAPSE IDS — Incremental Development System
...
Examples:
  sinapse ids:query "validate story drafts"
  sinapse ids:query "template rendering engine" --json
  sinapse ids:query "database migration" --type script
  sinapse ids:create-review
  sinapse ids:create-review --json
  sinapse ids:health
  sinapse ids:health --fix
  sinapse ids:check "validate yaml schema" --type task
  sinapse ids:impact create-doc
  sinapse ids:stats
  sinapse ids:stats --json
  sinapse ids:register .sinapse-ai/development/tasks/my-new-task.md
`);
}
...
function runQuery() {
  ...
  if (!intent) {
    console.error('Error: Intent is required. Usage: sinapse ids:query "your intent here"');
    process.exit(1);
  }
...
async function runCheck() {
  ...
  if (!intent) {
    console.error('Error: Intent is required. Usage: sinapse ids:check "your intent here"');
    process.exit(1);
  }
...
async function runImpact() {
  ...
  if (!entityId) {
    console.error('Error: Entity ID is required. Usage: sinapse ids:impact {entity-id}');
    process.exit(1);
  }
...
async function runRegister() {
  ...
  if (!filePath) {
    console.error('Error: File path is required. Usage: sinapse ids:register {file-path}');
    process.exit(1);
  }
